...
\Guillaume{I think one bigger example would be enough}
...
The number of essential complementary partitions is $|\EC| = 2(n+1)^{n-2}$.
\end{corollary}
...
\end{itemize}
We have refer to the orders in this fashion due to \cite[Theorem 3.16]{LA21} and Corollary \ref{SU from hyperplane arrangement}.
...
There are clearly $4$ ways to order these two pairs, and we can identify these $I,J$ pairs as cases of Lemma \ref{admissible lemma}.
\\\\
...
Then from Lemma \ref{admissible lemma} we know that the only $\{I,J\}$ pair of order $l+1$ that will not decompose (and hence be specified by admissibility) is 
...
The following contradiction is a generalisation of Example \ref{Non-admissible order contradiction example}.
...
\begin{example} \label{Non-admissible order contradiction example}
