...
We denote by 
\begin{itemize}
    \item $\LA(n):=\{(I,J) \ | \ \{I,J\} \in D(n), \ \min(I\cup J) = \min I\}$, and by 
    \item $\SU(n):=\{(I,J) \ | \ \{I,J\} \in D(n), \ \max(I\cup J) = \max J\}$.
\end{itemize}
\end{definition}
...
In what follows, we will be interested in a \emph{coherent} choice of orderings of $D(n)$, for all $n \geq 1$.
That is, a family of sets $\{O(n)\}_{n\geq 1}$ where each $O(n)$ has as elements ordered pairs $(I,J)$ or $(J,I)$, for each $\{I,J\} \in D(n)$.
For an ordered pair $(I,J)$ in $O(n)$, we denote by $\std(I,J)$ the standardisation function, e.g. $\std(\{5,9,10\},\{6,8,12\}) = (\{1,4,5\},\{2,3,6\})$.

\begin{definition}
A choice of orderings $O:=\{O(n)\}_{n \geq 1}$ for $\{D(n)\}_{n\geq 1}$ is \emph{operadic} if for all $(I,J) \in O$, we have that $(I',J') \subsetneq (I,J) \implies \std(I',J') \in O(|I'|)$. 
\end{definition}

\Guillaume{To myself: rework from here}
