...
\begin{align*}
    (I,J) = (\bigcup_{a\in A} I_a, \bigcup_{a \in A} J_a) \iff \forall a \in A, r((I_a, J_a))
\end{align*}
where $A$ is some indexing set, each $(I_a,J_a)$ is an ordered pair of $D(n)$ and $r$ is the standardisation function e.g. $r( (\{5,9,10\},\{6,8,12\})) = (\{1,4,5\},\{2,3,6\})$.
\end{definition}
...
\\\\
Finally, we show that $\OP$ is a coherent extension of the $o_k$.
...
\begin{center}
\begin{tikzcd}
LA \arrow[r,leftrightarrow] \arrow[d,leftrightarrow]&
LA^{op}\arrow[d,leftrightarrow]\\
SU^{op} \arrow[r,leftrightarrow] &
SU
\end{tikzcd}
\end{center}
These orders induce 
\begin{enumerate}
    \item Isomorphism of operads
    \item Isomorphisms of the face poset of the diagonal
\end{enumerate}
\end{lemma}
\begin{proof}
...
