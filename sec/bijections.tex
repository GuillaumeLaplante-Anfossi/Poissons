...
\Kurt{Starting to rewrite as discussed, making whole extra section now so we can check and compare for errors}

\Kurt{Want to check everything works first then will clean up notation}

\Kurt{What do we want subscripts? superscripts? indices and/or sets?}

In this section we show that the $I,J$ description of $\SUD$, referred to as $\SUD$, is equivalent to the original description of Saneblidze-Umble diagonal \cite{SaneblidzeUmble04}, referred to as subset shift $\SUD$. We fully detail the two bijections of the top row of the following commutative diagram, (\cref{prop:subset shift to shift bijection} and ...)
\begin{center}
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
, and together with the vertical arrow $\theta:\LAD \to \SUD$ (\cref{p:IJ level bijection}) this proves the equivalence of the only two diagonals.
...
We make the following observations about SCPs. Firstly, the same argument as in \cref{l:u-well-defined} shows that the underlying pair of partitions is an essential complementary pair.
...
\end{proof}
The key result of the inductive proof was the chain of inequalities \cref{eq:shift subset dominates adj path}. It turns out, if we assume the inequality holds, we can drop the assumption that $k$ is greater than the maximal index used by $\mathbf{M}$. and the assumption that $\min M_k > \max R_{\mathbf{M}}(\sigma)_{k+1}$. 
...
Let $(\sigma,\tau)$ be a pair of ordered partitions, and let $\rho \in (\sigma_i \setminus \min \sigma_i)$ such that $\rho>\max \delta_{i,i+1}$, where $\delta_{i,i+1}$ is the unique path between $\sigma_i$ and $\sigma_{i+1}$. Such an element $\rho$ is said to be \emph{critical}. 
We similarly define the critical elements of $\tau$ to be, $\rho \in (\tau_{i+1} \setminus \min\tau_{i+1})$ such that $\rho > \max \delta_{i,i+1}$. 
...
\end{corollary}
The observation that the maximal element of the path $\sigma_i$ to $\sigma_{i+1}$ in the SCP $(\sigma,\tau)$ is $\sigma_{i+1}$ then yields the following result.
\begin{corollary} \label{cor:the maximal element of shifts of SCPS}
Let $(\sigma',\tau')$ be generated from critical shifts of an SCP $(\sigma,\tau)$, then $\max \delta_{i,i+1} = \max \sigma_{i+1}$.
\end{corollary}
...
Then from \cref{cor:the maximal element of shifts of SCPS}, we know that $\rho>\max \sigma_{j+1} = \max (R_{M_{i_{j-1}}}...R_{M_{i_1}}(\sigma))_{i_j + 1}$, where the equality follows as $i_1<...<i_{j-1}<i_j$. This that proves $R_{M'_{i_j}}$ is well-defined, completing the inductive proof.
\end{proof}

\Kurt{Example!}

\Kurt{From here onwards}
