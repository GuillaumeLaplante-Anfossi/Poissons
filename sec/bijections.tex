...
\Kurt{Happy to change any and all notation, or make other alterations!}

\begin{definition}
Let $D(n) := \{ \{I,J\}:I,J \subset \{1,\dots,n\}, |I|= |J|, I\cap J = \emptyset \}$.
This is a set of unordered pairs, and we can define total orders on all pairs of the same size as follows. We say $I<J \equiv (I,J)$ if,
\begin{itemize}
    \item $\OP : \min(I\cup J) = \min I$
    \item $SU : \max(I\cup J) = \max I $
\end{itemize}
We have refer to the orders in this fashion due to \cite[Theorem 3.16]{LA21} and \cref{SU from hyperplane arrangement}.
We will also use $-^{op}$ to refer to the dual of these orders. We say an $\{I,J\}$ pair is of order $k$ if $|I| = |J|=k$. 
\end{definition}

\begin{definition}
We say an ordering of $D(n)$ is \textbf{coherent} if for all ordered $(I,J)$ pairs 
\begin{align*}
    (I,J) = (\cup_{a\in A} I_a, \cup_{a \in A} J_a) \iff \forall a \in A, r((I_a, J_a))
\end{align*}
where $r$ is the standardisation function e.g. $r( (\{5,9,10\},\{6,8,12\})) = (\{1,4,5\},\{2,3,6\})$.
\end{definition}

\begin{lemma} \label{coherent lemma}
The $\OP$ and $SU$ orderings are coherent, and are coherent extensions of the following $(I,J)$ pairs,
\begin{itemize}
    \item ${\OP} :$ for all $k\geq 1$  $(\{1,k+2,k+3,\dots,2k-1,2k\}, \{2,3,\dots,k+1\})$
    \item ${SU} :$ for all $k\geq 1$  $(\{1,2,3,\dots,k-1,2k\}, \{k,k+1,\dots,2k-1\})$
\end{itemize}
The dual orders $\OP^{op}$ and $SU^{op}$ are also coherent, and coherent extensions of the dual pairs.
\end{lemma}

\begin{proof}
We present the proof for ${\OP}$, the proof for ${SU}$ proceeds similarly, and the dual proofs are obtained from duality.
...
As such, if we tried to decompose it as a union we could pick out one pair $(I_1,J_1)$ such that $\min ( I_1\cup J_1) = \min I_1 = 1$, but the next possible pair must satisfy $\min ( I_2\cup J_2) = \min J_2$, and hence would violate the $\OP$ order.
\\\\
...
However, if $(I,J) = (\cup_{a\in A} I_a, \cup_{a \in A} J_a)$, then the ${\OP}$ order is such that $\forall a \in A, r((I_a, J_a))$.
As such all decompositions of a pair $(I,J)$ order it the same way.
\\\\
Finally, we show that $\OP$ is coherent extension of the $o_k$.
...
\begin{lemma}
The only coherent orderings of $D(n)$ are $\OP,SU,\OP^{op}$ and $SU^{op}$.
\end{lemma}
...
\begin{enumerate}
    ...
    \item $(\{2\},\{1\})$ and $(\{1,4\},\{2,3\})$ which corresponds to $SU$ i.e. $\max(I\cup J) = \max I$
    \item $(\{2\},\{1\})$ and $(\{2,3\},\{1,4\})$ which corresponds to $\OP^{op}$ i.e. $\min(I\cup J) = \min J$
    \item $(\{1\},\{2\})$ and $(\{2,3\},\{1,4\})$ which corresponds to $SU^{op}$ i.e. $\max(I\cup J) = \max J$
\end{enumerate}
...
We will show this through induction for the $\OP$ order, the $SU$ order proceeds similarly, and the dual orders by duality.
\\\\
...
As such, the only way we can vary from $\OP$ is to order this element in the opposite direction i.e.
...
Suppose the $\OP$ order holds for $D(n)$ of order $k\leq 2$ and in order $3$ it is reversed, i.e.
\begin{align*}
...
\begin{thm}
There are only four comultiplicative diagonals of every operahedra, in particular, those induced by the $\OP, LA, \OP^{op}$ and $LA^{op}$ orders on $D(n)$.
\end{thm}

\begin{lemma}
The $\OP$ and LA orders of $D(n)$ are \textbf{not} isomorphic (with respect to coherence). \Kurt{This seems a little strange, maybe I'm doing something off...}
\end{lemma}

\begin{proof}
As each order is a coherent extension of one non-decomposable pair for each $k\geq 1$, the isomorphism must map these pairs to each other, and extend coherently everywhere else. 
Such an isomorphism $\theta:\OP\to SU$ would operate on the non-decomposable pairs as follows. If $r((I,J)) = o_k$ then $\theta((I,J)) := (I',J')$, the unique pair which uses the same numbers as $I,J$ and is such that $r((I',J')) = o'_k$ the unique non-decomposable pair in the $SU$ order. For instance,
\begin{align*}
	o_3 = (\{1,5,6\},\{2,3,4\}) \land o'_3 = (\{1,2,6\},\{3,4,5\}) \implies
	\theta( (\{1,7,8\}, \{4,5,6\} ) ) = (\{1,4,8\},\{5,6,7\})
\end{align*}
More generally we can extend $\theta$ to every other (decomposable) pair through 
\begin{align*}
	\theta((I,J)) = \theta((\{i_2\},\{j_k\})) \cup \theta((\{i_3\},\{j_{k-1}\})) \cup \theta((\{i_{l+1} \},\{j_{k-l-1} \})) \cup \theta((I'_l,J'_l))
\end{align*}
as each pair of singletons reduces to $o_1$, and $r((I'_l,J'_l)) = o_l$, this is well defined. However, in order for $\theta$ to be an isomorphism, it must be compatible with other decompositions, which it is not. Here is an explicit counter example.
\begin{itemize}
	\item $(1378, 2456) = (178,245) \cup (3,6) = (17,25)\cup (38,46)$
	\item the first decomp implies that $\theta((1378, 2456)) = \theta(178,245) \cup \theta(3,6) = (128,457)\cup (6,3) $
	\item the second decomp implies $\theta((1378, 2456)) = \theta (17,25)\cup \theta (38,46) = (17,25)\cup (38,46)$
\end{itemize}
a contradiction. 
\end{proof}
\Kurt{So if this counter example is true, then}
\begin{itemize}
	\item don't think we can operate just with the $D(n)$ conditions
	\item need something more from the definition of the diagonal.
	\item What explicitly is an isomorphism of comultiplicative diagonals?
\end{itemize}
...
Let $\BT$ denote the set of edge labelled ordered bipartite trees on $[n]$.

\begin{proposition} [{\cite[Theorem 3]{kajitani1982number}}] 
\label{EC Graph Bijection}
The sets of essential complementary partitions $\EC$ and edge labelled bipartite trees $\BT$ are in bijection.
\end{proposition}
\begin{proof}
Informally, given a partition pair $(\sigma,\tau)$, the corresponding graph $G((\sigma,\tau))$ has the subpartitions of each partition as its vertices, and two vertices are connected by an edge if the subpartitions share a common element.
...
\end{proof}
\Kurt{ notation wise sigmaL,tauR or PL PR or other?}
\begin{construction} 
\label{Order Lemma}
