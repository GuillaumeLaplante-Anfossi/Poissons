...
\end{enumerate}
From this observation and the ordering function $o':\EC\to \SUD$ (\cref{prop:SUD are ordered EC}) it is immediate that $SCP\subset\SUD$, i.e. the maximal elements are being traversed right to left.
...
Furthermore, once we have dropped the assumption that we need to perform shifts in an increasing order, we can generate all subset shifts by chains of singleton set shifts. This motivates the following definition. \Kurt{What about $\sigma_{i,i+1}$ for notation?}
...
We now wish to prove that the shift $\SUD$ diagonal is equal to the $\SUD$ diagonal, however we will first need to introduce some supporting theory.
We first define inverse critical shift operators, \Kurt{Should tie closure stuff below to this definition.}
\begin{definition} \label{def:critical SU inverse shift}
Let $(\sigma,\tau)$ be a pair of ordered partitions. Let $\rho \in \sigma_{i+1}$ be such that $\rho>\max \sigma_{i,i+1}$, then we informally define the inverse right shift operator $R^{-1}_\rho$ as returning $\rho$ to $\sigma_i$.
Similarly, let $\rho \in \tau_{i} $ such that $\rho > \max \tau_{i,i+1}$, then we informally define the inverse left shift operator $L^{-1}_\rho$ as returning $\rho$ to $\tau_{i+1}$.
\end{definition}
\Kurt{Explain why no condition on $\sigma_{i+1}$...}
We now seek to combinatorially characterise when we can apply an inverse shift operator.

\begin{definition}
Let $(\sigma,\tau)$ be a pair of ordered partitions $[n]$, we say that $(i,j)$, where $i<j$, is an \emph{inversion} if $i$ appears before $j$ in $\sigma$ and $j$ appears before $i$ in $\tau$. 
\end{definition}

\begin{proposition}
\label{p:crossings}
The set of inversions of an $\SUD$ facet is in bijection with its set of edge crossings. 
...
\end{proof}
We now show that the $\SUD$ diagonal is closed under the operators, and the inverse operators.
\begin{lemma} \label{lem:SUD closed under shifts}
Let $(\sigma,\tau)\in \SUD$, then if we apply a critical shift operator to it we remain in $\SUD$.
\end{lemma}
\begin{proof}
...
\end{proof}
The inverse characterisation is a little more subtle.
\begin{lemma} \label{lem:SUD closed under inverse shifts}
Let $(\sigma,\tau)\in \SUD$, and suppose it has an adjacent crossing, which corresponds to the inversion $(\alpha,\rho)$ (with $\alpha<\rho$). Then,
\begin{itemize}
    \item if the blocks of the adjacent crossing are from $\sigma$, then $(R_\rho^{-1}(\sigma),\tau)\in \SUD$.
    \item if the blocks of the adjacent crossing are from $\tau$, then $(\sigma,L_\rho^{-1}(\tau))\in \SUD$.
\end{itemize}
and in either case, the inverse shift operator produces fewer crossings.
\end{lemma}
\begin{proof}
We prove the result for the inverse right shift operator, and the other case proceeds similarly. \Kurt{TODO}
\end{proof}
...
The proof that every facet of $\SUD$ is in shift $\SUD$ follows from \cref{lem:SUD closed under inverse shifts} and induction on the number of crossings. 
The proof that every facet of shift $\SUD$ is a facet of $\SUD$ follows from \cref{lem:SUD closed under shifts}
\end{proof}
...
\end{lemma}
\begin{proof}
Let $(\sigma_1|...|\sigma_k,\tau) \in \SUD$, and let $\rho \in \sigma_{i}$ be a critical element. Then, the shift operator $R_{\rho}$ steps from the facet $(\sigma_1|...|\sigma_i |\sigma_{i+1} |...|\sigma_k, \tau)$, through the facets of facets $(\sigma_1|...|\sigma_i \setminus \{\rho\}|\rho|\sigma_{i+1}|...|\sigma_k, \tau)$, to the facet $(\sigma_1|...|\sigma_i \setminus \{\rho\}|\sigma_{i+1} \sqcup \{\rho\}|...|\sigma_k,\tau)$.
\end{proof}
...
\end{tikzpicture}
\end{center}
We now illustrate all possible $\LAD$ shifts of the left $SCP$ and all possible $\SUD$ shifts of the right $SCP$. We first display how the $\LAD$ left shifts act on $\sigma$, and the $\SUD$ left shifts act on $\tau'$. We indicate shifts by their corresponding critical element, drawing them to avoid crossings, and so they align with their dual shift under $\theta$.
\begin{center}
{\small
\begin{tikzcd}
