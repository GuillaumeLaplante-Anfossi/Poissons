...
As we have omitted the case $M=\emptyset$, we have altered our indexing set from $\{1,2,...,d\}$ to the increasing sequence $i_1<...<i_d$. 
...
Let $\delta_{k,k+1}:= R_{\mathbf{M}}(\sigma)_{k,k+1}$ (i.e. the path between the adjacent blocks in $(R_{\mathbf{M}}(\sigma),L_{\mathbf{N}}(\tau))$ ), and let $\delta_{k+1,k}$ be the same path reversed.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item $\beta$ uses steps of $\delta_{k,k+1}$, in which case $\gamma' = \alpha m (\delta_{k+1,k}\setminus \beta)(\beta \setminus \delta_{k+1,k})$.
...
Let $(\sigma,\tau)$ be a pair of ordered partitions, and let $\rho \in (\sigma_i \setminus \min \sigma_i)$ such that $\rho>\max \sigma_{i,i+1}$.
Such an element $\rho$ is said to be \emph{critical}. 
...
First, we observe that any subset right shift $R_{M_k}(\sigma)$ can be decomposed in a series of right shifts: since $\min M_k > \max \sigma_{k,k+1}$ by the proof of \cref{lem:SU subset conserves maximal element of paths}, we can first shift $\min M_k$ to the right, then $\min (M_k \setminus \min M_k)$, and so on until the entire subset $M_k$ has been shifted to the right. 
...
Then from \cref{cor: SU shift conserves maximal element of paths}, we know that $\rho>\max \sigma_{i_j+1} = \max (R_{M_{i_{j-1}}}...R_{M_{i_1}}(\sigma))_{i_j + 1}$, where the equality follows as $i_1<...<i_{j-1}<i_j$. This proves that $R_{M'_{i_j}}$ is well-defined, completing the inductive proof.
\end{proof}
...
\end{lemma}
\begin{proof}
Let $(\sigma_1|...|\sigma_k,\tau) \in \SUD$, and let $\rho \in \sigma_{i}$ be a critical element. Then, the shift operator $R_{\rho}$ steps from the facet $(\sigma_1|...|\sigma_i |\sigma_{i+1} |...|\sigma_k, \tau)$, to the facet $(\sigma_1|...|\sigma_i \setminus \{\rho\}|\sigma_{i+1} \sqcup \{\rho\}|...|\sigma_k,\tau)$, through the facets of facets $(\sigma_1|...|\sigma_i \setminus \{\rho\}|\rho|\sigma_{i+1}|...|\sigma_k, \tau)$.
\end{proof}
...
Let $(\sigma,\tau)$ be a pair of ordered partitions, and let $\rho \in (\sigma_{i+1} \setminus \max \sigma_{i+1})$ such that $\rho< \min \sigma_{i,i+1}$. Such an element $\rho$ is said to be \emph{critical}. 
