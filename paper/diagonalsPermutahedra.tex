...
The facets of the \defn{original $\SU$ diagonal} (\resp the \defn{$m$-shift $\SU$ diagonal}) is defined by the formula
    \begin{align*}
...
\begin{proof}
    ...
    For the reverse inclusion, we need to show that any $m$-shift can be rexpressed as a path-admissible sequence of $1$-shifts. 
    We proceed by induction.
    For the case $m=1$ there is nothing to prove.
    Let $(\sigma,\tau)$ be a pair of ordered partitions in the $m$-shift diagonal. 
    Suppose now that for an element $\rho \in \sigma_i$, the right $m$-shift $R_\rho$ is path-admissible, \ie $\rho \neq \min \sigma_i$ and $\rho > \max \sigma_{i,i+m}$.
...
\begin{enumerate}
    ...
    Note that in virtue of \cref{eq:shift-subset-dominates}, the edge $m$ is not in $\delta_{k,k+1}$, and this path is unaffected by the shift. 
...
\end{enumerate}
Observe that all (non-trivial or non-contradictory) paths $\gamma'$ contain $m>\min M_k$ and either some addition or deletion by $\delta_{k,k+1}$.
...
\Guillaume{@Kurt, can you align the nodes + put the arrow in the middle of the picture?}
...
Combining \cref{prop:iso-original-shift-diagonals} and \cref{prop:iso-shift-IJ-diagonals} we obtain the desired equivalence between the original $\SU$ diagonal from \cite{SaneblidzeUmble04} and the geometric $\SU$ diagonal from \cref{def:LA-and-SU}.
