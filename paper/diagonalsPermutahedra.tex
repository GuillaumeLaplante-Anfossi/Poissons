...
We show that these are the only two diagonals which respect the product structure on the faces of the permutahedra (\cref{subsec:operadicProperty}), and that they are isomorphic (\cref{subsec:isos-LA-SU}). 
Finally, we describe their facets by a simple combinatorial rule (\cref{subsec:facets-operadic-diags}), and their vertices as pattern-avoiding pairs of permutations (\cref{subsec:vertices-operadic-diags}). 
...
In this section we show that the geometric $\SU$ diagonal $\SUD$ is a topological enhancement of the original Saneblidze-Umble diagonal~\cite{SaneblidzeUmble} (\cref{subsec:topological-SU}).
...
\subsubsection{$\SU$ diagonals}\label{subsec:SU-shifts}
...
\Kurt{$M$ is too similar to $\mathbf{M}$, so didn't alter. Maybe could use $M'$}
...
According to \cref{cor:SU-shift-preserves-max}, $1$-shifts conserve the maximal element of paths between adjacent blocks, and its directions.
Consequently, the following $(m-1)$-shift $R_\rho$ is path-admissible, and we can thus apply the induction hypothesis. 
\end{proof}

This isomorphism provides us with the following analogue of \cref{prop:iso-1-to-m-shift} for $m$-shifts. 
...
\Kurt{Maybe just drop LA/SU like so?}
...
\end{proof}
\Guillaume{Easier or standard notation for $P_{k_1,k_2}^n$?}
\Kurt{Better?}
...
\begin{definition} 
\label{def:subdiv pairs}
Let $F$ be a face of $\Perm$, and let $v$ be a vertex of $F$. 
The \defn{ballot of $F$ at $v$} is the set $\subdivpairsev(F)$ of all pairs $(C^v,C_v)$ of $(k_1,k_2)$-subdivision cubes of $F$ such that
\begin{enumerate}
	\item $C^v$ has maximal vertex $v$,
	\item $C_v$ has minimal vertex $v$, and
	\item $k_1+k_2 = \dim F$.
\end{enumerate}
If $F$ is the maximal face of $\Perm$, then we denote the ballot of $F$ simply by $\subdivpairsv$.
The \defn{maximal pair} of the ballot $\maxsubdivpairsev(F)$ (\resp $\maxsubdivpairsv$) is the unique maximal pair of $\subdivpairsev(F)$ (\resp $\subdivpairsev$) with respect to inclusion.
\end{definition}
...
We clarify the previous section with examples that are visually verifiable from the diagram of $\divcube{3}$ below.
The following sets of faces are all $k$-subdivision cubes of the maximal face $F$ of $\Perm[4]$, (for varying $k$),
\begin{align*}
...
The following three sets of faces are not subdivision cubes of $F$,
\begin{align*}
...
\end{align*}
This defines the ballot $\subdivpairsv$, whose maximal pair of subvision cubes is $\maxsubdivpairsv=\{ 1|234, 13|24, 14|23, 134|2\} \times \{4|3|12\}$.
...
The set of pairs of subdivision cubes in the ballot $\subdivpairsv$ of $v$ satisfies the following properties
\begin{enumerate}
...
\Guillaume{What does generic mean??}
\Kurt{Clearer?}
...
\begin{align} \label{eq:In LA inductive def}
	...
	\end{cases}
	, \text{where } \sigma'_i = \{x+1:x\in \sigma_i\}
\end{align}
\end{definition}
...
The appropriate base definitions for the $\LA$ diagonal of $k$-subdivision cubes (\ref{def:Subdivisions}), ballots of subdivision cubes ${\subdivpairsv}$ (\ref{def:subdiv pairs}), and maximal pairs of subdivision cubes $\maxsubdivpairsv$ are all the same as in the $\SU$ case.
...
\begin{example}
Applying the isomorphism $r$ to \cref{ex:subdivision cubes} yields,
\begin{center}
{\scriptsize
\begin{tikzcd}[sep=0.1cm, scale=0.3]
& 4|2|1|3 \arrow[rr,dash] \arrow[dd,dash,dotted] \arrow[dl,dash] & & 4|2|3|1 \arrow [dd, dash,dotted] \arrow[rr,dash] & & 4|3|2|1 \arrow[dd,dash] \arrow[dl,dash, near end, "\mathbf{\blue{4|3|12}}"']\\
...
In this Section we derive some higher algebaic consequences of the results obtained in \cref{part:diagonalsPermutahedra}.
...
However, they are both homotopic to the usual thing diagonal.
...
We present here the $24$ planar trees with $5$ internal edges for which $|\LAD_t| \neq |\SUD_t|$.
...
We display each tree $t$ with the counts $(|\LAD_t|,|\SUD_t|)$ above it, and arrange them symmetrically to help emphasize notable missing elements. \Kurt{easy way to make centrally symmetric, or alternate arrangement?}
\begin{center}
...
We would like to thank Hugh Thomas, who first asked about the counts of facets of the $\LA$ diagonal and designed the first code to compute them, during a visit of GLA at the LACIM in Montr\'eal, in the summer of 2021.
We are grateful to Sylvie Corteel for pointing out the relevance of the dual perspective for the purpose of counting, and to Samson Saneblidze and Ron Umble for useful correspondence.
\Guillaume{Galashin (asked about characteristic polynomials). 
