...
We first define the thin diagonal of a polytope and its approximations by cellular diagonals.
\vincent{Give with one sentence the motivations for these cellular approximations...}
...
We denote by~$\min_{\b{v}}(P)$ (resp.~$\max_{\b{v}}(P)$) the unique vertex of~$P$ which minimizes (resp.~maximizes) the scalar product with~$\b{v}$. 

\begin{definition}
\label{prop:diagonal}
For any vector $\b{v}$ in generic position with respect to $P$, the tight coherent section~$\triangle_{(P,\b{v})}$ of the projection $P \times P \to P, (\b{x}, \b{y}) \mapsto (\b{x}+\b{y})/2$ selected by the vector~$(-\b{v}, \b{v})$ defines a cellular diagonal of~$P$.
...
\end{array}
\end{align*}
where $\rho_{\b{z}} P \eqdef 2\b{z}-P$ denotes the reflection of $P$ with respect to the point~$\b{z}$. 
\end{definition}
...
\begin{figure}[p]
	...
	\caption{The subdivisions induced by cellular diagonals of the $3$-dimensional simplex (left), cube (middle), and permutahedron (right).}
	\label{fig:examplesDiagonals2}
\end{figure}

\vincent{Include here the discussion about the fundamental hyperplane arrangement and the precise description of the diagonal. Make it precise.}
...
Let us recall here that the \defn{normal cone} of a face~$F$ of a polytope~$P$ in~$\R^d$ is the cone of directions~$\b{c} \in \R^d$ such that the maximum of the scalar product~$\dotprod{\b{c}}{\b{x}}$ over~$P$ is attained for some~$\b{x}$ in~$F$, and that the \defn{normal fan} of~$P$ is the fan formed by the normal cones of all faces of~$P$.
...
Finally, the following statement relates the image of the diagonal~$\triangle_{(P, \b{v})}$ to the intervals of the poset obtained by orienting the $1$-skeleton of~$P$ in direction~$\b{v}$.
...
\begin{remark}
For some polytopes such as the simplices, the cubes, the Hoschild polytopes, and the associahedra, the reverse inclusion also holds.
\vincent{Hochschild?}
The resulting equality enhancing (\ref{eq:magique}) was called \defn{magical formula} by J.-L. Loday.
\vincent{reference}
Polytopes of greater complexity such as the multiplihedra, the operahedra or the permutahedra, do not possess this exceptional property. 
\end{remark}
...
\vincent{To discuss with Guillaume from this point...}
\vincent{I want to make this more precise and have it just after \cref{prop:diagonal}.}
The following \defn{universal formula} \cite[Thm.~1.26]{LA21} expresses combinatorially the cellular image of the diagonal of a polytope $P$.

\begin{definition}
The \defn{fundamental hyperplane arrangement} $\mathcal{H}_P$ of a polytope $P \subset \R^n$ is the set of hyperplanes whose normal vectors are a choice of directions of the edges of $P\cap \rho_z P$, for any $z \in P$. 
\end{definition}

Recall that a choice of diagonal is given by a choice of vector in a chamber of $\mathcal{H}_P$, two vectors living in the same chamber inducing the same diagonal \cite[Prop.~1.23]{LA21}.

\begin{theorem}
\label{thm:universalFormula}
Let $(P,\b{v})$ be a positively oriented polytope in $\R^n$. 
For each $H\in\mathcal{H}_P$, we choose a normal vector $\vec d_H$ such that $\langle \vec d_H, \b{v} \rangle >0$. 
We have 
\begin{eqnarray*}
(F,G) \in \Ima \triangle_{(P,\b{v})} 
...
Let
\[
\Un(n) \eqdef \{ (I,J) \ | \ I,J \subset [n] \text{ with } |I|=|J|>0 \text{ and } I \cap J = \varnothing\}.
\]
\vincent{Change $(I,J)$ to $\{I,J\}$???}
The fundamental hyperplane arrangement of the permutahedron~$\Perm$ is given by the family of hyperplanes
\[
...
\begin{corollary}
\label{cor:choice-inequalities}
Any choice of diagonal of the $(n-1)$-permutahedron is given combinatorially by a choice of inequalities
\[
\sum_{i \in I} x_i > \sum_{j \in J} x_j \quad \text{or} \quad \sum_{i \in I} x_i < \sum_{j \in J} x_j
\]
for all $(I,J) \in \Un(n)$. 
\vincent{This seems imprecise to me. We focus on special diagonals...}
\end{corollary}

\vincent{Can we specialize \cref{thm:universalFormula} as well?}
...
\vincent{This is too vague, I am not sure to keep it.}
\end{remark}

Finally, the permutahedron~$\Perm$ is not magical in the sense of \cref{prop:magicalFormula}.

\begin{proposition}
For any generic vector~$\b{v}$, the diagonal~$\triangle_{(\Perm, \b{v})}$ of the standard permutahedron~$\Perm$ does \emph{not} satisfy the magical formula.
...
\begin{proof}
\Guillaume{On peut montrer ça uniquement par le compte des faces non?}
\vincent{I would not put any proof. Just say that the specialization of \cref{thm:universalFormula} gives less faces than the magical formula would expect.}
\end{proof}
...
This section is devoted to the combinatorics of two specific diagonals of the permutahedra, the LA diagonal and SU diagonal (\cref{subsec:LASUdiagonal}).
We will see later in \cref{sec:operadicDiagonals} that these two diagonals are in fact the only two operadic diagonals, and we will provide in \cref{...} alternative descriptions of these two diagonals in terms of strong complementary partitions and shifts (which will in particular connect the present description of the SU diagonal with the original description of \cite{SaneblitzeUmble04}).

We will see in Part III \Guillaume{Hyperlink} that these choice of orderings induce operadic structure on the operahedra, multiplihedra and associahedra.
...
Recall from \cref{cor:choice-inequalities} that a diagonal of the permutahedron~$\Perm$ is given by a choice of ordering of the sets
\[
...
\begin{definition}
The \defn{$\LA$ diagonal} $\LAD$ (resp. \defn{$\SU$ diagonal} $\SUD$) of the permutahedron~$\Perm$ is given by any vector $\vec v \in \R^n$ satisfying  
\[
...
The goal of this section is to prove in \cref{prop:unique-operad} that the $\LA$ and $\SU$ diagonals are the only two operadic diagonals which induce the weak Bruhat order the vertices of the permutahedra. 

Recall that each face $A_1 | \ldots | A_k$ of the permutahedron $P_{|A_1|+\cdots + |A_k|-1}$ is isomorphic to the product $P_{|A_1|-1} \times \cdots \times P_{|A_k|-1}$ of lower dimensional permutahedra, via the permutation of coordinates
\begin{equation*}
    \begin{matrix}
...
A family of diagonals $\triangle \eqdef \{\triangle_n : P_n \to P_n \times P_n\}_{n\geq 1}$ of the permutahedra is \defn{operadic} if for every face $A_1 | \ldots | A_k$ of the permutahedron $P_{|A_1|+\cdots + |A_k|-1}$, the map $\Theta$ induces a topological cellular isomorphism \[  \triangle(A_1) \times \ldots \times \triangle(A_k) \cong \triangle(A_1 | \ldots | A_k) \ . \]
\end{definition}
...
An ordering $\Or$ of $\Un$ is \defn{operadic} if for every $\{I,J\} \in \Un$, we have that $(I,J) \in \Or$ if and only if
\begin{enumerate}
    \item $\std(I,J) \in \Or$, and
    \item there exists $I'\subset I, J'\subset J$ of the same size $|I'|=|J'|$ such that both $(I',J')$ and~$(I\setminus I', J'\setminus J')$ are in $\Or$. 
\end{enumerate}
\end{definition}

We call \defn{indecomposables} of $\Or$ the pairs $(I,J) \in \Or$ such that $(I,J)=\std(I,J)$ and such that the only subpair $(I',J')$ satisfying condition (2) above is the pair $(I,J)$ itself.

\begin{lemma} 
\label{lem:operadic-ordering}
The $\LA$ and $\SU$ orderings are operadic, and their indecomposables are the pairs $(\{1\},\{2\})$ and 
\begin{align}
...
Condition (1) in \cref{def:operadic-ordering} is straighforward, since standardising a pair preserves its minimal element.
For Condition (2), it suffices to take $I'=I,J'=J$ in the direct implication.
The reverse implication also holds, as it is clear that if $\{I,J\} \in \Un$ can be written as $\{I_a \sqcup I_b, J_a \sqcup J_b\}$ with $(I_a,J_a)$ and $(I_b,J_b)$ in $\LA$, then $(I,J)$ is itself in $\LA$, since $\min(I)=\min\{\min(I_a),\min(I_b)\}$.
Thus $\LA$ is operadic. 

Second, we compute the indecomposables. 
Let $(I_k,J_k)$ denote the pairs of (\ref{eq:std-LA}).
If we try to decompose $(I_k,J_k)$ as a non-trivial union, there is always one pair $(I',J')$ in this union for which $1 \notin I'$, so we have $\min ( I' \cup J') = \min J'$, which implies that $(I',J') \notin \LA$.
Thus, the pairs $(I_k,J_k)$ are indeed indecomposables. 

It remains to show that any pair $(I,J)$ in $\LA$ which is not of the form $(I_k,J_k)$ can be decomposed as a union of such pairs. 
Let $(I,J)$ be in $\LA$ and suppose that $(I,J)=\std(I,J) \neq (I_k,J_k)$.
...
There are exactly four operadic diagonals of the permutahedra, given by the $\LA$ and $\SU$ diagonals, and their opposites. 
The $\LA$ and $\SU$ diagonals are the only operadic diagonals which induce the weak Bruhat order on the vertices.
\end{theorem}

\begin{proof}
    First, by \cite[Thm.~3.9]{LA21} any diagonal in the sense of \cref{prop:diagonal} is given by a choice of ordering of $\Un$.
    Second, every operadic diagonal satisfies \cite[Prop.~4.14]{LA21}, which amounts precisely to an operadic ordering of $\Un$ in the sense of \cref{def:operadic-ordering}.
    We conclude with \cref{prop:operadic-ordering}.
\end{proof}

\begin{remark}
    This answers by the negative a conjecture regarding unicity of diagonals on the permutahedra, raised at the beginning of \cite[Sect.~3]{SaneblidzeUmble04}, and could be seen as an alternative statement. \Guillaume{See the next section where we prove that SU diagonal is SU diagonal}
\end{remark}
...
\label{subsec:LAdiagonal}

Let us now recall the specific choice of chambers from \cite[Thm.~3.16]{LA21}.

\begin{definition}[$\LA$ diagonal]
    Let $n\geq 1$, and let us write $\LA(n) \coloneqq \{(I,J) \in U(n) \ | \ \min(I\cup J)\in I \}$.
    The \emph{$\LA$ diagonal} $\LAD$ of the $(n-1)$-permutahedron is given by any vector $\b{v} \in \R^n$ satisfying  
    \[\sum_{i \in I} v_i > \sum_{j \in J} v_j  \ , \forall (I,J) \in \LA(n) \ . \]
\end{definition}

Applying \cref{thm:universalFormula}, one obtains the following combinatorial description, referred to later as the \emph{$(I,J)$ description}. 
...
\end{eqnarray*}
\end{proposition}
Here, $\LA(\sigma,\tau) \subset \LA(n)$ is a proper subset of $\LA(n)$ which depends on the choice of~$(\sigma,\tau)$, and comes from the geometry of the situation, see \cite[Thm.~1.26]{LA21} for more details.
...
For geometric reasons (see the proof of \cite[Thm.~1.26]{LA21}), the solution of this system is $x=0$. 
...
\begin{proof}
    ...
    By definition $\LA(\sigma, \tau)$ describes the directions of these edges, and the translation is made as follows: for a given pair $(I,J)$, define the corresponding direction $\vec d$ by its coordinates $d_i \eqdef 1$ if $i \in I$, $d_j \eqdef -1$ if $j \in J$, and $d_k \eqdef 0$ otherwise.  
    We refer to \cite[Sect.~1.5]{LA21} for more details.
\end{proof}
...
\vincent{The next two subsections should be integrated in \cref{sec:LASUdiagonals}.}
...
These pairs of partitions were first studied and called essential complementary partitions in \cite{chen1969computer,chen1971tables,kajitani1982number}, as such we will denote the set of all $(2,n)$-trees by $\EC$. \Guillaume{change notation for $(2,n)$-trees}
In this section, we prove that particular orderings of $\EC$ recover the facets of the four operadic diagonals of \cref{prop:unique-operad}.

\begin{construction}
\label{const:orienting-function}
Given an operadic diagonal $\triangle$, we define an ordering function $o:\EC\to \OP$ from the set of $(2,n)$-trees to the set of facets of $\triangle$ as follows. 
Let $(\sigma,\tau)$ be a $(2,n)$-tree, and let $a,b$ be two vertices of $\sigma$ or $\tau$.
We say that $a < b$ if the unique path $\gamma$ from $a$ to $b$ is such that
\begin{itemize}
    \item $\min \gamma$ is oriented from left to right (resp.~right to left) if $\triangle=\LAD$ (resp.~$\triangle=(\LAD)^{\op}$),
    \item $\max \gamma$ oriented from right to left (resp.~left to right) if $\triangle=\SUD$ (resp.~$\triangle=(\SUD)^{\op})$.
\end{itemize}
\end{construction}

We now aim at proving that this ordering function is in fact a bijection in \cref{thm:facets}. 
To this end we need a few intermediary results. 

\begin{lemma}
\label{l:o-provides-total-order}
For any $(2,n)$-tree $(\sigma, \tau)$, the ordering function $o:\EC\to \LAD$ provides a total order of the blocks of the unordered partitions $\sigma$ and $\tau$. 
\end{lemma}
\begin{proof}
The order defined in \cref{const:orienting-function} is clearly total, reflexive (by convention) and anti-symmetric, what remains to be checked is its transitivity.
Let $a,b,c$ all be vertices of $\sigma$ or $\tau$, and let $\gamma_{ab},\gamma_{bc}$ denote the unique minimal paths between them.
Suppose that the paths have been oriented such that $a<b$ and $b<c$, and let us show that this implies $a<c$.
First observe that $\gamma_{ac}= (\gamma_{ab} \cup \gamma_{bc}) \ssm (\gamma_{ab} \cap \gamma_{bc})$.
As $a<b$ and $b<c$ it must be the case that the intersection $(\gamma_{ab} \cap \gamma_{bc})$ is traversed in opposite directions by the paths $\gamma_{ab}$ and $\gamma_{bc}$; this implies that neither of the minima of paths are in $(\gamma_{ab} \cap \gamma_{bc})$. 
Thus, the minimum of $\gamma_{ac}$, which is one of the two preceding minima, is traversed from left to right. 
\end{proof}

The key observation in proving \cref{thm:facets} is that paths between pairs of vertices in a $(2,n)$-tree correspond to $(I,J)$ conditions defining an operadic diagonal (see \cref{sec:cellularDiagonalsPermutahedra}). 

\begin{example}
\label{ex:ECbijection}
The $(2,7)$-tree $\{15,234,6,7\} \times \{13,2,46,57\}$ (left) is ordered according to \cref{const:orienting-function}, giving a facet of $\LAD$ (right).
\begin{center}
...
\end{tikzpicture}
\end{center}
Two paths providing order information are
\begin{align*}
	7 < 234 \text{ as } 7 \xrightarrow{7} 57 \xrightarrow{5} 15 \xrightarrow{1=\min(I\cup J)} 13 \xrightarrow{3} 234 \ \text{corresponding to } (I,J)=(\{1,7\},\{3,5\}), \\
	57 < 46 \text{ as } 57 \xrightarrow{5} 15 \xrightarrow{1=\min(I\cup J)} 13 \xrightarrow{3} 234 \xrightarrow{4} 46 \ \text{corresponding to } (I,J)=(\{1,4\},\{3,5\}).
\end{align*}
\end{example}

\begin{proposition}
    \label{prop:geometrical-IJ}
    Let $(\sigma,\tau)$ be a facet of the $\LA$ diagonal $\LAD$.
    The paths between adjacent vertices of $\sigma$ or $\tau$ are in bijection with the minimal $(I,J)$-pairs.
\end{proposition}
...
\begin{proof}
Let $(\sigma,\tau)$ be a $(2,n)$-tree.
By \cref{l:o-provides-total-order}, we have the blocks of $o(\sigma,\tau)$ a totally ordered. 
It remains to show that the ordered pair $o(\sigma,\tau)$ belongs to the $\LA$ diagonal, i.e. it satisfies every minimal $(I,J)$ condition of \cref{p:minimal}.
By \cref{prop:geometrical-IJ}, these are given by the paths between adjacent vertices of $o(\sigma,\tau)$.
Let us consider two such ordered vertices $\sigma_l<\sigma_{l'}$ in $\sigma$ (the situation for $\tau$ is similar).
The path connecting them is of the form
\begin{align*}
...
\end{align*}
and we have $(I,J) \eqdef (\{i_1,\dots,i_k\},\{j_1,\dots,j_k\}) \in \LA(n)$ since the minimal element points from~$\sigma$ to $\tau$.
...
$
    \forall m, \big|\bigcup_{1\leq k \leq m} \tau_{k} \cap I \big| = \big|\bigcup_{1\leq k \leq m} \tau_{k} \cap J \big| \ .
$
Hence in order for the $(I,J)$ condition to be satisfied it must be the case that
$
    \exists m, \big| \bigcup_{1\leq k \leq m} \sigma_k \cap I \big| > \big|\bigcup_{1\leq k \leq m} \sigma_k \cap J \big| \ .
$
...
\begin{theorem}
\label{thm:facets}
For any operadic diagonal $\triangle$, the ordering function $o:\EC\to \OP$ is a bijection between the set of $(2,n)$-trees and the set of facets of $\triangle$. 
\end{theorem}

\begin{proof}
First, we prove the statement for the $\LA$ diagonal $\LAD$. 
By \cref{l:o-well-defined}, the ordering function $o:\EC \to \LAD$ is well defined.
Now, the intersection graph of a facet of the diagonal $\LAD$ has $n+1$ vertices and $n$ edges, thus it defines a $(2,n)$-tree, and the function $u:\OP \to \EC$ which forgets the order in a pair of partitions is well defined.
It is clear that the two functions $o$ and $u$ are mutual inverses, thus we have a bijection. 
To obtain the $\SU$ ordering, we apply the isomorphism $rs\times rs$ from \cref{thm:bijection-operadic-diagonals} and track what happens to each path.
The facets of $\LA^{op}$ and $\SU^{op}$ are obtained by reversing these orders.
\end{proof}

We end this Section by illustrating how the isomorphism $rs\times rs$ from \cref{thm:bijection-operadic-diagonals} relates the $\LA$ and $\SU$ orders. 

\begin{example}
\label{ex:theta-path-translation}
Consider the facet of $\LAD$ from \cref{ex:ECbijection}.
The isomorphism $rs\times rs$ gives the following facet of $\SUD$
\begin{center}
\begin{tikzpicture}[scale=.7]  
\node (1) at (-1.5, -1) {$15$};
\node (2) at (-1.5, -2) {$7$};
...
\end{tikzpicture} \ .
\end{center}
Observe that the $\LA(7)$ conditions corresponding to paths, such as $( \{1,4\},\{3,5\})$, which implies that
\begin{align*}
    57 < 46 \quad \text{  as  } \quad 57 \xrightarrow{5} 15 \xrightarrow{1=\min(I\cup J)} 13 \xrightarrow{3} 234 \xrightarrow{4} 46 \ ,
\end{align*}
are mapped to $\SU(7)$ conditions corresponding to paths, e.g. the prior $(I,J)$-condition maps to~$(r(J),r(I))=( \{3,5 \},\{4,7\})$, which implies that
\begin{align*}
    24 < 13 \quad \text{ as } 24 \xrightarrow{4} 456 \xrightarrow{5} 57 \xrightarrow{7=\max(r(J)\cup r(I))} 37 \xrightarrow{3} 13 \ .
\end{align*}
\end{example}
...
%\Kurt{A necessary condition for any shift description of $(l,n)$-trees would be having a generalised order for it.
%Not sure what an obvious extension of this order would be.
%Can we compute via code what the ordering must be for $\LA$/$\SU$ $(l,n)$-trees for small $l,n$?}

%%%%%%%%%%%%%%%

\subsection{Vertices of operadic diagonals}

We are now interested in characterizing the vertices (i.e. pairs of permutations) that occur in any operadic diagonal. 
As in the preceding Section, we study $\LAD$ first, and then transport our results to other operadic diagonals via isomorphisms. 

\begin{definition}[Domination]
For two non-overlapping proper subsets $X,Y \subsetneq \sigma$ of an ordered partition $\sigma$ with $m$ blocks, we say that the set $X$ \emph{dominates} $Y$ in $\sigma$ if for all $k \in [m]$, we have that~$\left| \sigma_{[k]} \cap X \right| \geq \left| \sigma_{[k]} \cap Y \right|$.
\end{definition}

The characterization of the $\LA$ diagonal in \cref{p:minimal} can be straightforwardly reformulated through domination. 

\begin{lemma}
\label{l:LA-domination}
For any pair of ordered partitions $(\sigma,\tau)$ we have that
\begin{eqnarray*}
    (\sigma,\tau) \notin \LAD & \iff \exists (I,J) \in \LA(n), & J \text{ dominates } I \text{ in } \sigma \text{ and } I \text{ dominates } J \text{ in } \tau \ . 
\end{eqnarray*}
\end{lemma}

Moreover, as an immediate consequence of the definition, the domination of sets is conserved under deletion of elements in the following situation. 
For $\sigma$ an ordered partition, and $x\in X$, $y \in Y$, we denote by $i_x,i_y$ the indices of the blocks containing each of them in $\sigma$. 

\begin{lemma}[Coherent domination]
\label{lem:Coherent Domination}
Let $X,Y \subsetneq \sigma$ be two non-overlapping proper subsets of an ordered partition $\sigma$, and suppose that $X$ dominates $Y$ in $\sigma$. 
For any $x\in X$, $y \in Y$, we have that $X\setminus x$ dominates $Y\setminus y$ in $\sigma$ if and only if either
\begin{enumerate}
    \item $y$ comes before $x$ in $\sigma$, or 
    \item for all $k$ in the open interval $(i_x,i_y)$, we have that $\left| \sigma_{[k]} \cap X \right| > \left| \sigma_{[k]} \cap Y \right|$.
\end{enumerate}
\end{lemma}

This enables the following characterisation of vertices in the diagonal as pattern-avoiding pairs of permutations.

\begin{proposition}
\label{prop: LAD Fish Pattern}
A pair of permutations of is in the $\LA$ diagonal if and only if for any $k\geq 1$ and for any $(I,J) \in \LA(k)$ it avoids the pattern 
\begin{align}
    (j_1 i_1 j_2 i_2 \cdots j_k i_k,\ i_2 j_1 i_3 j_2 \cdots i_k j_{k-1} i_1 j_k) \ , \label{eq:pattern}
\end{align}
where $I=\{i_1,\dots,i_k\}$, $J=\{j_1,\dots,j_k\}$ and $i_1=\min(I\cup J)$.
\end{proposition}

\begin{remark}
    In standardized form, there are $(k-1)!k!$ avoided patterns.
    For $k=1,2$ they are given by $(21,12),(2134,4213)$ and $(3124,4312)$.
\end{remark}

\begin{proof}
Given the operadic nature of the $\LA$ diagonal (\cref{lem:operadic-ordering}), a pair of permutations $(\sigma,\tau)$ is in~$\LAD$ if and only if it satisfies every $(I,J)$ condition whose standardisation is of the form 
\begin{align}
    (I_k,J_k) \eqdef (\{1,k+2,k+3,\dots,2k-1,2k\}, \{2,3,\dots,k+1\}) \label{eq:standard}
\end{align}
for some $k\geq 1$. Thus, according to \cref{l:LA-domination}, we have that $(\sigma,\tau) \notin \LAD$ if and only if there is an $(I,J)$ of standard form (\ref{eq:standard}) sucht that $J$ dominates $I$ in $\sigma$ and $I$ dominates $J$ in $\tau$.
It remains to show this last condition is equivalent to the patterns (\ref{eq:pattern}).
...
Now suppose that for all $(I,J)$ of size $|I|=|J|\leq m-1$ with standard form (\ref{eq:standard}), we have that if~$J$ dominates $I$ in $\sigma$ and $I$ dominates $J$ in $\tau$, then $(\sigma,\tau)$ contains a pattern of the form~(\ref{eq:pattern}).

We need to show that this is still true for the pairs $(I,J)$ of size $|I|=|J|=m$.
Let $(\sigma,\tau)$ be permutations, and let $(I,J)$ be such a pair.
If there exists $(I',J')$ of cardinality $|I'|=|J'|<m$ satisfying the domination condition, we can conclude by the induction hypothesis. 
Let us consider the case where such a $(I',J')$ does not exist, and suppose that $J$ dominates $I$ in $\sigma$ and $I$ dominates $J$ in $\tau$.
We want to show that $(\sigma,\tau)$ contains a pattern of the form~(\ref{eq:pattern}) made from the pair $(I,J)$.

Firstly, suppose that the leftmost element $i_1$ of $I$ in $\sigma$ is not $\min(I\cup J)$, and let us write $j_1$ for the leftmost element of $J$ in $\tau$.
Since the pair $(I,J)$ is of standard form (\ref{eq:standard}), so is the pair $(I',J') \eqdef (I_k\setminus \{i_1\}, J_k \setminus \{j_1\})$, and one can check using both cases of \cref{lem:Coherent Domination} that $J'$ dominates $I'$ in $\sigma$, and $I'$ dominates $J'$ in $\tau$.
We can thus conclude with the induction hypothesis. 

Secondly, suppose that the leftmost element $i_1$ of $I$ in $\sigma$ is $\min(I\cup J)$.
Then, for $n\geq1$ we prove that,
\begin{enumerate}[(a)]
    \item If $\sigma = j_1 i_1 j_2 i_2 ... j_{n-1} i_{n-1} w i_{n} w'$, $\tau = i_2 j_1 i_3 j_2 ... i_{n}j_{n-1}w''$, and $j_n$ is the leftmost $J\setminus \{j_1,...,j_{n-1}\}$ in $\tau$, then either we have $w = j_n$, or we can conclude with the induction hypothesis.
...
\end{enumerate}
Let $(I',J') \eqdef (I_k\setminus \{i_n\}, J_k \setminus \{j_n\})$.
...
Let $(I_k,J_k) \eqdef (\{k,k+1,\dots,2k-1\},\{1,2,3,\dots,k-1,2k\})$.
A pair of permutations $(\sigma,\tau)\in \SUD$ if, and only if, $\forall k,$ $(\sigma,\tau)$ avoids the set of $(k-1)!k!$ different patterns
\begin{align*}
    P_k \eqdef \{ (j_1 i_1 j_2 i_2 \cdots j_k i_k,\quad  i_1 j_k i_2 j_1 \cdots i_{k-1} j_{k-2}i_k j_{k-1}): j_k = 2k, I = \{i_1,...,i_k\},J = \{j_1,...,j_k\} \}
\end{align*}
...
\section{The lattice of facets}

\Guillaume{A reecrire plus tard}
\vincent{This section should be called Shifts.}
...
\subsection{Second isomorphism: $m$-shift and $(I,J)$ diagonals coincide}\label{sec:Iso m-shifts to IJ}
...
Recall from Definitions \ref{def:cellularDiagonal} and \ref{def:operadic diagonal} that a cellular operadic diagonal of the permutahdra satisfies the following properties.
\begin{itemize}
    \item ...
\end{itemize}
...
$n_j \eqdef \#(\sigma_{k-i+1}\cup...\cup \sigma_k)$, and 
$I_j \eqdef [1 - \frac{1}{2^{n_j}}, 1 - \frac{1}{2^{n_{j+1}}}]$.
We then define boundary cases $I_0 \eqdef [0,1 - \frac{1}{2^{n_1}}]$ and $I_k \eqdef [1 - \frac{1}{2^{n_k}},1]$, to  produce $I(\sigma) \eqdef I_0 \cup ... \cup I_k$, a partition of the interval $[0,1]$.
...
\subsubsection{The Cubical to Shift Link}

\Kurt{We should also give a lattice-shift definition.
In this definition, the only thing that matters are the computed maximal shifts.
i.e. $R_\rho^{k}\sigma$ is well-defined if $h_\rho(\sigma)+k\leq r_\rho$. 
}
A stronger result than this lemma is saying we can treat any $k$-face as if it is either $\sigma$ or $\tau$ in an $SCP$ and consider the consequential lattice structure...\Kurt{Generalised Tamari Order link...}

\begin{lemma}
If $L$ is a maximal $1$-subdivision cube with maximal (minimal) $1$-face $\sigma$ then its faces are generated by $\bigcup_{i=0}^{r_\rho} R_\rho^i (\sigma)$ (dually $\bigcup_{i=0}^{l_\rho} L_\rho^i (\sigma)$ ).
Where, $r_\rho, l_\rho$ are the maximal shift-indices calculated using \cref{prop:maximal m-shift formulae}.

\end{lemma}
\begin{proof}
We note that the formula for the maximal shift-indices of \cref{prop:maximal m-shift formulae} does not depend on the pair of ordered partitions, but just the left (right) partition.
The result then follows from the inductive description of $I_{n}$ (\cref{eq:In inductive def}), applied to lines.
\end{proof}
...
As such, $\bigcup_{\mathbf{M}} R_{\mathbf{M}}(\sigma)$ will be a collection of $(k+1)$-faces.
Furthermore, it is straightforward to see these $(k+1)$ faces are just $\bigcup_{\mathbf{M}} R_{\mathbf{M}}(\sigma_C)$ with the $i$\ordinal{} and $(i+1)$\ordinalst{} block merged.
So, under the projections $\pi_C$ and $\pi_L$ this collection of $(k+1)$-faces are flush with the $k$-cube and the initial portion of the line $L$ \Kurt{What is the best way to say flush with boundary?}
\\\\ 
Let $v'$ be the smallest vertex of the line $L$.
If $v'$ is in $\sigma$, then $r_\rho = 0$, and all that remains is to show that $\bigcup_{\mathbf{M}} R_{\mathbf{M}}(\sigma)$, has a $(k+1)$-cubical shape, i.e. we need to be flush with the end portion of the line $L$ under the projection $\pi_L$. 
From the inductive description of $I_n$ (\cref{eq:In inductive def}), this will occur if every $k$-face in $C$ has a constant number of elements in block $i+1$. 
Suppose, that there exists a $k$-face with one less element in that block.
Then this implies that one of these elements was shifted into the next block.
However, as $\rho$ is larger than all $\rho'$ this implies that we can also shift $\rho$ into the next block, i.e. $r_\rho>0$, a contradiction. 
\\\\
Suppose that $v'$ is not in $\sigma$. 
As $\sigma$ is a $(k+1)$-face, it is a trivial $(k+1)$-cube.
When the line $L$ steps out of $\sigma$ towards $v'$, it must do so through a cubical vertex, with $(k+1)$ incoming edges.
Once again, as $P_n$ is a simply polytope this implies the next face is a $(k+1)$-face.
Through the inductive description of $I_n$, it must have form $R_\rho^1(\sigma)$, and each admissible sequence of shifts $R_{\mathbf{M}}$ will continue to act on it, i.e. $\bigcup_{i=0}^{i=1}\bigcup_{\mathbf{M}} R_\rho^i R_{\mathbf{M}}(\sigma)$ will be a collection of $(k+1)$-faces.
Furthermore, this set of faces remains flush with the boundary of $C$, and the initial part of $L$.
We can proceed inductively from here, either $v' \in R_\rho^1 \sigma$, in which case we can mirror the argument above to show the set of $(k+1)$-faces must have a $(k+1)$-cubical shape, or $v' \not \in R_\rho^1 \sigma$, so we can infer the existence of $R_\rho^2 \sigma$ and proceed from here.
Note that this inductive argument must terminate with maximal shift $r_\rho$ for $\sigma$, and the boundary of the $(k+1)$-faces agreeing with $L\times C$.
\end{proof}
...
\Kurt{Todo for line desc. Max transfering line will become min transferring line, so will be nice.}
