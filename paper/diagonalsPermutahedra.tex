...
\vincent{All this section needs a lot of pictures.}
...
The following statement is the analogue of \cref{prop:flatPosetMultiBraidArrangement}.
...
\begin{figure}
	...
	\caption{Labelings of the faces of the arrangement~$\multiBraidArrangement[3][2](\b{a})$ for~$\b{a} = ???$.}
	\label{fig:B23a}
\end{figure}
...
We now fix a generic translation matrix~$\b{a} \eqdef (a_{i,j})$ and still denote by~$A_{i,s,t} \eqdef \smash{\sum_{s \le j < t} a_{i,j}}$ for all~$1 \le s < t \le n$ and~$i \in [\ell]$.
\vincent{Say that $A_{i,t,s} \eqdef -A_{i,s,t}$ for~$1 \le s < t \le n$.}
The objective of this section is to describe
\begin{itemize}
\item the ordered $(\ell,n)$-partitions forests with a given underlying (unordered) $(\ell,n)$-partition forest (\cref{subsec:PFtoOPF}),
\item a criterion to decide whether a given ordered $(\ell,n)$-partition forest corresponds to a face of~$\multiBraidArrangement(\b{a})$ (\cref{subsec:criterionOPF}).
\end{itemize}
...
In this section, we describe the ordered $(\ell,n)$-partitions forests~$\orient{\b{F}}$ with a given underlying $(\ell,n)$-partition forest~$\b{F}$.
...
Then for any ordered $(\ell,n)$-partition forest~$\orient{\b{F}} \eqdef (\orient{F_1}, \dots, \orient{F_\ell})$ of~$\orientedForestPoset(\b{a})$ with underlying $(\ell,n)$-partition forest~$\b{F}$ and any~$i \in [\ell]$, the order of~$s$ and~$t$ in~$\orient{F}_i$ is given by the sign of~$A_{i,s,t} - \sum_{p \in [q]} A_{i_p, r_{p-1}, r_p}$.
\end{proposition}
...
For each pair~$s,t \in [n]$ in distinct connected components of~$cc(\b{F})$, we define the chain~$<_i$ on the $\ell$ triples~$(i,s,t)$ for~$i \in [\ell]$ given by the order of the values~$A_{i,s,t}$.
The \defn{inversion poset}~$I(\b{F}, \b{a})$ is then the poset obtained by quotienting the disjoint union of the chains~$<_i$ by the equivalence relation~$(i,s,t) \equiv (i,s',t')$ if~$s$ and~$s'$ belong to the same part of~$F_i$ and~$t$ and~$t'$ belong to the same part of~$F_i$.
\end{definition}

\begin{proposition}
The ordered $(\ell,n)$-partition forests with a given underlying $(\ell,n)$-partition forest~$\b{F}$ are in bijection with the lower ideals of the inversion poset~$I(\b{F}, \b{a})$.
\end{proposition}
...
We now consider a given ordered $(\ell,n)$-partition forest and provide a criterion to decide if it belongs to the upper set~$\orientedForestPoset(\b{a})$ corresponding to the faces of~$\multiBraidArrangement(\b{a})$.
...
\begin{proposition}
An ordered $(\ell,n)$-partition forest~$\orient{\b{F}}$ belongs to the upper set~$\orientedForestPoset(\b{a})$ corresponding to the faces of~$\multiBraidArrangement(\b{a})$ if and only if $\sum_{\alpha \in \gamma} A_{i(\alpha), s(\alpha), t(\alpha)} \ge 0$ for any (simple) oriented cycle~$\gamma$ in~$D_{\orient{\b{F}}}$, where each arc~$\alpha \in \gamma$ has label~$i(\alpha)$, source~$s(\alpha)$, and target~$t(\alpha)$.
\end{proposition}
...
\end{itemize}
Then a point~$\b{x} \in \R^n$ belongs to the face of the $i$th braid arrangement corresponding to~$\orient{F}_i$ if and only if it satisfies~$M_i \, \b{x} \le \b{z}_i$.
...
The key idea is the following: any vector $\vec v$ in generic position with respect to $P$ defines a cellular diagonal~$\triangle_{(P,\vec v)}$, via the formula
\begin{align*}
    \begin{array}{rlcl}
...
\Kurt{I think there is a typo, $23|1\times 1|2|3$ on top left edge, should be $23|1\times 3|2|1$.}
\vincent{You are right! thank you. I just corrected it.}
...
\begin{proof}
    The proof is similar to \cref{lem:IJ-closed-under-shifts}: as the inverse shift operators conserve the maxima of paths, and these same maxima continue to be traversed from right to left, it follows by \cref{thm:facets} that the output of an inverse shift operator will remain in $\SUD$, which shows Point (1). 
...
Secondly, as $C$ spans differing dimensions to $L$, every $k$-face in $C$ must have the element $\rho$ in the same singleton block, say the $i$th block.
Furthermore, the number of elements on either side of the $i$th block must be the same.
...
\end{enumerate}
From the inductive description of $I_n$ (\cref{eq:In inductive def}), it must be the case that $\sigma$ is obtained from $\sigma_C$ by merging the $i$th and $(i+1)$th block.
...
Furthermore, it is straightforward to see these $(k+1)$ faces are just $\bigcup_{\mathbf{M}} R_{\mathbf{M}}(\sigma_C)$ with the $i$th and $(i+1)$th block merged.
