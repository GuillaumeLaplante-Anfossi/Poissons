...
For completeness, we recall from \cite{SaneblidzeUmble} and \cite{SaneblidzeUmble-comparingDiagonals} the matrix description of facets of the $\SU$ diagonal.
...
\begin{definition}
	A right shift $R_M$ of a non-empty set of elements of a matrix $M \subset \sigma_i(A)$ (\resp $M_{i}\subset \tau_{i}(A)$) block-admissible if
	\begin{enumerate}
...
\end{proposition}
\begin{proof}
It suffices to work with the original description of the $\SU$ diagonal.
...
Suppose that adn admissible right shift $R_{M}$ is not well-defined as it tries to move a value $m$ into a non-zero matrix entry $n$.
...
The \emph{configuration matrices} \cite[Def. 7]{SaneblidzeUmble} are the matrices corresponding to $\SCP$s and those generated by admissible sequences of shifts.
...
\begin{proposition}[{\cite[Form.~(1)]{SaneblidzeUmble-comparingDiagonals}}]
...
[{\cite[Form.~(3)]{SaneblidzeUmble-comparingDiagonals}}]
...
\begin{proposition}[{\cite[Form.~(1)~\&~(3)]{SaneblidzeUmble-comparingDiagonals}}]
\begin{align*}
