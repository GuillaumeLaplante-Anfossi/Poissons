...
Every face of a Forcey--Loday multiplihedron is isomorphic to a product of lower-dimensional Forcey--Loday multiplihedra and Loday associahedra, via an isomorphism $\Theta$ similar to the one from \cref{subsec:operadicProperty}, see Point (4) of \cite[Prop. 1.10]{LaplanteAnfossiMazuir}.
...
\end{enumerate}
which agree with the Tamari-type order on $2$-colored planar trees. 
\end{theorem}
...
We shall see now that the two operad (\resp operadic bimodule) structures on the operahedra (\resp multiplihedra) are related to one another in the strongest possible sense: they are isomorphic as topological cellular colored operads.
