...
An interval $[u,v] \in P_n$ is a $k$-subdivision cube of $I_{n-1}$, if, and only if, the vertices $u$ and $v$ are connected by all $k!$ extremal edge paths in $I_{n-1}$.
\end{lemma}
\begin{proof}
...
From the inductive description of \cref{eq:In inductive def}, we know that each edge of $I_{n-1}$ is associated to (possibly multiple) shifts of one particular element $i$.
As such, the extremal edge paths correspond to shifting a each element to its final position and then never touching it again. \Kurt{This feels almost tautological, but needs something...}
...
\end{lemma}
\begin{proof}
As $j>i$, shifting $i$ before $j$ will never block $\rightshiftk{j}{s_j}$ or $\leftshiftk{j}{s_j}$. The equality is clear.
\end{proof}
...
\begin{example}
We illustrate on the left (right) all $k$-subdivision cubes of $I_3$ with maximal vertex $4|3|2|1$ (with minimal vertex $1|2|3|4$)
...
In addition, we have directed the edges of each of the cubes so that the unique directed path from $4|3|2|1$ ($1|2|3|4$) to the minimal (maximal) vertex is the path of \cref{lem:subdiv cubes are intervals}.
{\scriptsize
\begin{center}
\begin{tikzcd}[sep=0.15cm,scale=0.8]
...
& \blue{2|1|3|4} \arrow [rr,dash,dotted] \arrow [dl,dash,dotted] & & {2|3|1|4} \arrow [rr,dash,dotted] & & \color{purple}{3|2|1|4} \arrow [dl,leftarrow] \\
...
\end{proposition}
\begin{proof}
Applying Lemmas \ref{lem:unique interval desc of maximal subdiv pair}, \ref{lem:Interchanging shift operators}\Kurt{seems likely I cheated the proof here. Can probably switch prior lemma around to fix this.} and \ref{lem: minimal vertex formulae}, sequentially yields 
\begin{align*}
    \maxsubdivpairsv = [\rightshift{1}...\rightshift{n}v, v] \times [v, \leftshift{1}...\leftshift{n}v] = [\rightshift{n}...\rightshift{1}v, v] \times [v, \leftshift{n}...\leftshift{1}v] = [(R^{r_n}_{n}...R^{r_{1}}_{1}\sigma)_*,v] \times [v,(L^{l_n}_{n}...L^{l_{1}}_{1}\tau)^*]
\end{align*}
