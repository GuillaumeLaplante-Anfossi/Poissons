...
\begin{proof}
\vincent{Todo. See file from Berenice.}
\end{proof}

\vincent{todo: OEIS sequences, see file from Berenice.}
...
We now simplify the expression of \cref{thm:MobiusPolynomialMultiBraidArrangement} using the polynomial
\[
\weirdPol[n] \eqdef \mobPol[\braidArrangement][x][0] = \sum_{k \in [n]} (-1)^{k-1} \, (k-1)! \, S(n,k) \, x^{k-1}.
\]
introduced at the end of \cref{subsec:braidArrangement}.

\begin{corollary}
The M\"obius polynomial of the $(\ell,n)$-braid arrangement~$\multiBraidArrangement$ is given by
\[
\mobPol[\multiBraidArrangement] = \sum_{\b{G} \in \forestPoset} x^{(n-1)(1-\ell)} y^{n-1-\ell n} \prod_{i \in [\ell]} 
\weirdPol[\#G_i].
\]
\end{corollary}
...
Let $(\sigma,\tau) \in \SUD$.
\begin{itemize}
    \item Let $S_{(\sigma,\tau)}^R := \{\rho: R_{\rho}(\sigma,\tau)  \text{ is a well-defined $1$-shift}\}$, and $S_{(\sigma,\tau)}^L := \{\rho: L_{\rho}(\sigma,\tau)  \text{ is a well-defined $1$-shift}\}$.
    \item For $\rho \in S_{(\sigma,\tau)}^R$ (or $\rho \in S_{(\sigma,\tau)}^L$), let $m_\rho:= \max m$ such that $R_\rho(\sigma,\tau)$ (or $L_{\rho}(\sigma,\tau)$) is a well-defined $m$-shift.
\end{itemize}
\end{definition}
Note $S_{(\sigma,\tau)}^R$ and $S_{(\sigma,\tau)}^L$ may not be disjoint.

\begin{lemma}[$m$-shifts commute] \label{lem:m-shifts commute}
Let $(\sigma,\tau) \in \SUD$.
If either side of the following equations are  well-defined sequences of $m_1,m_2$ shift operators, then the other side is a well-defined sequence of $m_2,m_1$ shift operators, and their output is equal.
\begin{align*}
...
\begin{align*}
    L(\sigma,\tau) \cong (\prod_{\rho \in S_{(\sigma,\tau)}^R} [m_\rho])\times 
    (\prod_{\rho \in S_{(\sigma,\tau)}^L} [m_\rho] )
\end{align*}
where $[n] = (0<1...<n)$.
\end{proposition}
...
We identify $(0,0,...,0)$ with the SCP $(\sigma,\tau)$ and every other element $(n_{r_1},...,n_{r_{k_1}}, n_{l_1},...,n_{l_{k_2}})$, with 
$R^{n_{r_1}}_{\rho_{r_1}}...R^{n_{r_{k_1}}}_{\rho_{r_{k_1}}}
L^{n_{l_1}}_{\rho_{l_1}}...L^{n_{l_{k_2}}}_{\rho_{l_{k_2}}}(\sigma,\tau)$.
...
The maximum element of the lattice is thus given by starting with $(\sigma,\tau)$ and shifting all elements of $S^R_{(\sigma,\tau)}$ ($S^L_{(\sigma,\tau)}$) right (resp. left) by their height.
...
Finally, all that remains to be checked is that
\begin{align*}
    t(r,r)(R_{M_{i_1}}(\sigma),\tau)= (r\tau,R_{M_{i_1}}(r\sigma))
\end{align*}
but this is clear as reversing the elements then shifting them, is the same as shifting the elements then reversing them. 
...
\subsection{Matrix and cubical Descriptions}
...
In \cite{SaneblidzeUmble04} Saneblidze and Umble first identified their diagonal through the following subdivision of the $n$-cube by the permutahedra $P_{n+1}$.
Let $1$ denote a trivial subdivision of the $0$-cube (a point), then we can subdivide the $n$-cube by weaving $n+1$ through the previous subdivision of the $n-1$-cube, then connecting all vertices by the weak Bruhat order. 
Here is an illustration of this subdivision for the first few-dimensions.
We indicate, in bold, a natural inclusion of each $n-1$ cube into the $n$-cube.
...
\end{tikzcd}
\end{center}
To avoid cluttering the diagram, we have only labelled the vertices of the subdivision of cube, but its edges, facets and face are also identified with the expected elements of $P_{n+1}$.
We note each vertex of the $n-1$ cube produces an edge of the $n$-cube with $n+1$ vertices through,
\begin{align*}
    x_1|...|x_n \mapsto (x_1|\dots|x_n|n+1, x_1|\dots|n+1|x_n|,\dots ,n+1|x_1|\dots|x_n)
\end{align*}
These successive vertices are totally ordered under the weak-Bruhat order.

\begin{definition} \label{def:Subdivisions}
Define $I_{n-1}$ to be the $n-1$-cube under this subdivision of $P_{n}$. 
Let $e$ be a cell in $P_{n}$ with dimension $|e|$,
\begin{itemize}
    \item a \emph{$k$-subdivision cube} of $e$ is a set of faces whose union is a $k$-subcube of $I_{n-1}$ for some $k\leq |e|$.
    \item the set of vertices in $e$ is denoted $\mathcal{V}_e$.
\end{itemize}
Let $v \in \mathcal{V}_e$, as this vertex corresponds to a permutation let $(\sigma,\tau)$ be its SCP. 
\begin{itemize}
    \item Let $k_1,k_2$ denote the number of blocks in $\sigma,\tau$ respectively.
    \item Let $(I_{v,1}^{k_1}$, $I_{v,2}^{k_2})$ be a pair of $k_i$-subdivision cubes of $e$ such that
    \begin{enumerate}
        \item $k_1+k_2 = |e|$
        \item Under the weak Bruhat order $\max I_{v,1}^{k_1} = \min I_{v,2}^{k_2} = v$
    \end{enumerate}
    \item We denote the set of all such pairs by $e_v$, and denote its unique maximal \Kurt{... hmmm well union of all, but probably better descs} element by $e_v^*$.
\end{itemize}
\end{definition}
\begin{example}
Here is an example that is directly verifiable from the diagram of $I_3$ above.
In $P_4$, let $e$ be the top-dimensional cell $1234$. Then the following sets of faces are all $k$-subdivision cubes of $e$
\begin{align*}
    \{1234\}, \{1|234\}, \{1|234, 14|23\} \{4|13|2\}, \{1|3|24,1|43|2,14|3|2\}.
\end{align*}
The follow two sets of faces are both not $k$-subdivision cubes of $e$,
\begin{align*}
    \{1|234, 14|23,13|24\}, \{1|2|34,1|3|24\}
\end{align*}
For $v = 4|1|3|2$, 
\begin{align*}
    e_v = \{( \{14|23\},\{4|13|2\}), ( \{1|234, 14|23\},\{4|13|2\}) \}, \text{and, } e_v^* =( \{1|234, 14|23\},\{4|13|2\})
\end{align*}
Note there are two elements in $\blue{\mathbf{14|23}}$$\times\blue{\mathbf{4|13|2}}, \red{\mathbf{1|234}}$$\times\blue{\mathbf{4|13|2}}$$ \in e_v^*$, an $SCP$ and a right shift of it.
Similarly, for $v=3|1|4|2$, we compute that there are two elements $\mathbf{\blue{13|24}}$$\times \mathbf{\blue{3|14|2}},\mathbf{\blue{13|24}}$$\times \mathbf{\red{43|1|2}}\in e_v^*$, an SCP and its left shift.
...
The set of $k$-subdivision cubes of $e$ with a unique maximal vertex is a bounded lattice.
\end{lemma}
\begin{proof}
Each $k$-subdivision cube has a minimal and maximal vertex under the weak Bruhat order.
Given two $k$-subdivision cubes $x,y$ with the same maximal vertex $v$, we define the meet of $x\land y$ to be their intersection.
We define the join $x \lor y$ to be the unique $k$ subdivision cube with maximal vertex $v$ and minimal vertex given by taking the join of the minimal vertices of $x$ and $y$ under weak Bruhat lattice.
\begin{itemize}
    \item Something like this is true, but might not need...
\end{itemize}
\end{proof}

\begin{lemma}
The unique maximal pair $e_v^*$ is well-defined, and has the following formula. Let $(\sigma,\tau)$ be the SCP of the vertex $v\in P_n$, then 
\begin{align*}
    e_v^* = L(e_v^*) \times R(e_v^*) := [\overrightarrow{\sigma}, \sigma] \times [\tau, \overleftarrow{\tau}]
\end{align*}
where $[x,y]$ is the interval of the permutahedra of faces of the same dimension, and 
\begin{align*}
   \overrightarrow{\sigma} := \overrightarrow{s_n}\overrightarrow{s_{n-1}}...\overrightarrow{s_1}(v),\quad 
   \overleftarrow{\tau} := \overleftarrow{s_n}\overleftarrow{s_{n-1}}...\overleftarrow{s_1}(v)
\end{align*}
The operators $\overleftarrow{s_i}, \overrightarrow{s_i}$ each shift the element $i$ to the left/right until there are no further blocks, or the next block contains an element larger than $i$.
\end{lemma}
\begin{proof}
If the SCP $(\sigma,\tau)$ has $k_1,k_2$ blocks respectively, then $([\overrightarrow{\sigma}, \sigma], [\tau, \overleftarrow{\tau}])$ are a pair of subdivision cubes of $P_{n}$ which meet conditions $1$ and $2$ of \cref{def:Subdivisions} by construction. 
The fact that we meet conditions $1$ and $2$ is clear, to see that they are subdivision cubes
\begin{itemize}
    \item 
\end{itemize}
...
\begin{example}
For the SCP $ 13|247|5|6 \times 3|17|4|156$ of \cref{ex:shift translation by theta}. We directly compute $R(e_v^*)$ to be the cube..., and $L(e_v^*)$ to be the square...
\end{example}

\begin{lemma}[Formula $(3)$ of \cite{saneblidzeComparingDiagonalsAssociahedra2022}]
Let $e$ be the top-dimensional cell of $P_{n}$, and let $v$ be a permutation of size $n$ and $(\sigma,\tau)$ its SCP.
Then,
\begin{align*}
    e_v^* = \bigcup_{\mathbf{M},\mathbf{N}} R_\mathbf{M}(\sigma) \times L_{\mathbf{N}}(\tau)
\end{align*}
where $R$ and $L$ are shift operators, and $\mathbf{M},\mathbf{N}$ are all admissible sequence of shifts of the appropriate type.
\end{lemma}
\begin{proof}
The reason this works 
\begin{itemize}
    \item $\overrightarrow{\sigma}$ is $\sigma$ after right shifting to maximal height....
\end{itemize}
\end{proof}

\begin{corollary}[Formula (1) and (3) \cite{saneblidzeComparingDiagonalsAssociahedra2022}]
\begin{align*}
...
\Kurt{So this logic would establish it is a formula, given we know the shift description is a formula. 
But I don't know follow why it defines a formula independently of this logic.
I think in the original paper, SU only directly prove the shift formula defines the diagonal...
So would need to pull back their proof... or give alternate.
}
