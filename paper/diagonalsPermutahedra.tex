...
For a $k$-dimensional face $\sigma$ of the cubical permutahedron $\divcube{n}$, we construct the unique maximal $k$-subdivision cube, with respect to inclusion, whose maximal (\resp minimal) $k$-face with respect to the weak order is $\sigma$.
\end{construction}
...
Suppose that we have constructed maximal subdivision cubes up to dimension $k$, and let $\sigma$ be a $(k+1)$-face of $\divcube{n}$, with maximal vertex $v$.
...
\begin{definition} 
\label{def:hourglass}
For a vertex $v$ of $\divcube{n}$, the \defn{hourglass} of $\divcube{n}$ at $v$ is the maximal pair of subdivision cubes $\hour$, with respect to inclusion, within the set of all pairs $(C,C')$ of subdivision cubes such that $C$ has maximal vertex $v$, $C'$ has minimal vertex $v$, and $\dim C +\dim C' = n$.
\end{definition}
...
\cref{thm:cubical-SU} shows that $\hour$ is generated by all shifts of the $\SCP$ corresponding to $v$.
\end{example}
...
\begin{theorem}
	...
	Then, we have 
	\[ \hour = \bigcup_{\b{M},\b{N}}R_{\b{M}}(\sigma) \times L_{\b{N}}(\tau) \ , \]
...
Now consider the unique maximal $(k+1)$-subdivision cube of $\divcube{n}$ with maximal $(k+1)$-face~$\sigma$.
...
This follows from the inductive description of $\divcube{n}$ (\cref{eq:sub}): every $k$-face in $C$ has $\rho$ in a singleton block, and as $\rho$ is larger than all other elements it blocks all other shifts.
