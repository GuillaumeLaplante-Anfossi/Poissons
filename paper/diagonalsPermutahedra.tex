...
\begin{definition}
    A \defn{pattern-avoiding permutation} is ...
    A \defn{pattern-avoiding pair of permutations} is...
\end{definition}

\begin{example}
    Pairs of permutations avoiding $(21,12)$ are precisely intervals of the weak Bruhat order. 
\end{example}

\begin{theorem}
\label{prop: LAD Fish Pattern}
A pair of permutations of $[n]$ is in the $\LA$ (resp. $\SU$) diagonal if and only if for any~$k\geq 1$ and for any $(I,J) \in \LA(n)$ (resp. $\SU(n)$) of size $|I|=|J|=k$ it avoids the patterns 
\begin{align}
