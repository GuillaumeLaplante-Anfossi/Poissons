...
The faces of these two diagonals are characterized combinatorially by the following direct application of \cref{thm:universalFormula}.
...
\end{eqnarray*}
The same holds for the $\SU$ diagonal by replacing $\LAD$ and $\LA(n)$ by $\SUD$ and $\SU(n)$, respectively.
\end{theorem}
...
Let $l\geq 2$ and suppose that for all $k\leq l$, we have ordered $I_k \eqdef \{1,k+2,k+3,\dots,2k-1,2k\} <  \{2,3,\dots,k+1\} \defeq J_k$.
...
We first provide a recursive description of a subdivision of the $(n-1)$-cube by $P_n$.
The $0$-cube (a point), admits a trivial subdivision by the sole element $1$ of $P_1$.
Then, assuming we have subdivided the $(n-2)$ cube with $P_{n-1}$, we can subdivide the $(n-1)$-cube with $P_n$ by weaving $n$ through the previous subdivision.
In particular, we weave $n$ through every face of $\sigma = \sigma_1|...|\sigma_k \in P_{n-1}$, using an interval description.
For $0<j<k$ we define
...
We also emphasise, that an immediate consequence of the construction is that each dimension of the cube corresponds to shifting a particular element of $[n]\setminus \{1\}$.
...
\end{center}
To avoid cluttering the diagram, we have only labelled the vertices of the subdivision of cube, but its edges, facets and outer face are all identified with the expected elements of $P_{n}$.
The cubical formula for the diagonal is constructed through maximal pairings of $k$-subdivision cubes of this presentation of $P_n$, which we now define, and prove the existence of.

\begin{definition} \label{def:Subdivisions}
Define $I_{n-1}$ to be the $(n-1)$-cube under this subdivision of $P_{n}$. 
Let $e$ be a cell in $P_{n}$ with dimension $|e|$, a \defn{$k$-subdivision cube} of $e$ denoted $\subdivcubeofe$ is a set of $k$-faces whose union is a $k$-subcube of $e$ for some $k\leq |e|$.
When $e$ is the outer face, we denote these $k$-subdivision cubes $\subdivcubeofouter$.
\end{definition}
...
A $k$-subdivision cube $\subdivcubeofe$ has unique maximal and minimal vertices under the weak Bruhat order.
\end{lemma}

\begin{proof}
If $\subdivcubeofe$ had multiple maximal/minimal vertices, then it could not be a union of $k$-faces in the shape of a $k$-cube.
\end{proof}
...
Let $e$ be a cell of $P_n$ and $\sigma$ a $k$-face of the cell. Then there exists a unique maximal $k$-subdivision cube $C_e$ whose maximal (minimal) $k$-face is $\sigma$.
\end{lemma}
...
Given the inductive description of $I_{n}$ (\cref{eq:In inductive def}), the maximal vertex $v$ and the face $\sigma$ specifies the element $\rho$ being shifted by the line.
...
In particular, if $v'$ is the minimal vertex of $L$, and $v_\rho, v'_\rho$ are the indices of $\rho$ in $v,v'$, then under the inductive description of $I_{n-1}$ (\cref{eq:In inductive def}) the line $L$ corresponds to $\sigma_L \times I'$ where $I':=[1-\frac{1}{2^{n-v'_\rho}},1-\frac{1}{2^{n-v_\rho}}]$.
\\\\
...
This corresponds to shifting $\rho$ from its current block, into a singleton block above it.
Then, by the inductive assumption, there exists a unique maximal $k$-subdivision cube with maximal $k$-face $\sigma_C$, which we denote $C$.
...
Consequently, as $\rho$ was the maximal element corresponding to dimensions of the $(k+1)$-cube, and the line $\sigma_L \times I'$ was well defined, we know the product $C\times L$ will correspond to the union of intervals $\sqcup_{f\in C}f \times I'$ (\cref{eq:In inductive def}), which spans the $(k+1)$-cube with $(k+1)$-faces resulting from shifting $\rho$ through $C$.
Finally, it is clear that $L\times C$ must be maximal under inclusion.
...
We now define a pairing of subdivision cubes.
...
Finally, we define $\maxsubdivpairsev$ and $\maxsubdivpairsv$ to be the unique maximal pair, with respect to inclusion, of $\subdivpairsv$ and $\subdivpairsv$ respectively.
\end{definition}
We note that the uniqueness of the maximal pairs is a corollary of \cref{lem:Unique maximal subdivcubes}.
...
The following sets of faces are all subdivision cubes of the outer face of $P_4$, $1234$ (for varying $k$),
\begin{align*}
...
\Kurt{Can probably streamline further...}

We now explicitly relate the $\SU$ cubical formula to $\SU$ shifts (\cref{sec:Shift-lattice}).
In particular, we will show that when we specialise the cubical formula for the diagonal to the outer faces, we recover the shift description. 
...
It is thus straightforward to see that $r_{\rho'}$ is conserved, when moving from $C$ to $C\times L$.
\\\\
Finally, we know from the inductive description of $I_n$ (\cref{eq:In inductive def}), and the proof of \cref{lem:Unique maximal subdivcubes}, that to construct the $(k+1)$-faces of $L\times C$ that we simply weave $\rho$ through the $k$-faces of $C$ (stepping through the interval $[1-\frac{1}{2^{n-v_\rho}},1-\frac{1}{2^{n-v'_\rho}}]$).
However, every $k$-face in $C$ has $\rho$ in a fixed singleton set (specified by $1-\frac{1}{2^{n-v_\rho}}$), and every $k$ face on the opposite side of $C\times L$ has $\rho$ in another fixed singleton set (specified by $1-\frac{1}{2^{n-v'_\rho}}$).
Given \cref{eq:In inductive def}, this final singleton block is either the last block, or is followed by a block containing an element larger than $\rho$.
...
Which consequently provides an alternate formula for the $\SU$ diagonal,
...
\end{align}
The appropriate base definitions for the $\LA$ diagonal of $k$-subdivision cubes (\ref{def:Subdivisions}),  ${\subdivpairsv}$ (\ref{def:subdiv pairs}), and $\maxsubdivpairsv$ (\ref{lem:max subdiv pairs}) are all the same.
