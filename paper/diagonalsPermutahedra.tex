...
We can use this lemma to prove,

\begin{lemma}\label{lem:Theta as a product of cubes}
The isomorphism of ordered partitions $\Theta:\sigma_1|...|\sigma_k \mapsto [|\sigma_1|] \times ... \times [|\sigma_k|]$, from \cref{subsec:operadicProperty}, manifests as a product of cubes.
\end{lemma}
\begin{proof}
For a given face $\sigma=\sigma_1|...|\sigma_k \in \Perm[n+1]$, $\min_v(\sigma) = \min_v(|\sigma_1|)_{\sigma_1}|...|\min_v(|\sigma_k|)_{\sigma_k}$, and $\max_v(\sigma) = \max_v(|\sigma_1|)_{\sigma_1}|...|\max_v(|\sigma_k|)_{\sigma_k}$.
Thus, as the unique minimal and maximal vertices of $\sigma$ have a product structure, it is clear that $\sigma$ is a product of cubes.
\end{proof}
...
\begin{proof}
Clearly the faces of $\Perm[n+1]$ and $\divcube{n}^\SU$ are in bijection, it just remains to show that their face posets are isomorphic.
It is clear that under any suitable characterisation of the cover relations of each, that the outer face covers all ordered partitions with only two blocks.
Given \cref{lem:Theta as a product of cubes}, the isomorphism $\Theta:A_1|...|A_k \mapsto [|A_1|] \times ... \times [|A_1|]$ then equates the remaining cover relations. \Kurt{Something like these two results seems cleanest...}
%Thus we need to show that $\sigma$ covers a face $\sigma'$ in $\divcube{n}^\SU$ if, and only if, this holds in $\Perm[n+1]$.
%By definition, $\sigma$ covers $\sigma'$ in $\Perm[n+1]$ if, and only if, $\sigma' = \sigma_1|...|\sigma_i^1|\sigma_i^2|...|\sigma_m$ where $\sigma_i = \sigma_i^1 \cup \sigma_i^2$.
%The inductive construction of the cube implies $\sigma$ covers $\sigma'$ if, and only if, they correspond to the respective products of intervals $I'_1 \times ... \times I'_n$, $I_1 \times ... \times I_n$, where $I'_j \subseteq I_j \subseteq [0,1]$, and $Dim(I'_1 \times ... \times I'_n) +1 = Dim(I_1 \times ... \times I_n)$.
...
\hedge[violet, very thick]{0,5,4,1}{0.5cm}
%\end{pgfonlayer}
\end{tikzpicture}
\qquad
\begin{tikzpicture}[yscale=-1, every node/.style={draw, very thick, circle, inner sep=1pt}, edge from parent path={[very thick, draw] (\tikzparentnode) -- (\tikzchildnode)}]
