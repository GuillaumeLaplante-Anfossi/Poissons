...
Ok,
\begin{itemize}
    \item In their original paper SU $(C_*(P_{n+1},\triangle_P,\partial)$ is a DG-coalebra (Corollary 1)  
    \\\\
    (and it the cellular projection onto the subdivision of the cube induces a DG-coalgebra map)
    \item $\triangle_P$ is defined in definition 11.
    \begin{align*}
        \triangle_P(n+1) = \sum_{F\in C^{q\times n-q+2}} csgn(F)c(F)\otimes r(F)
    \end{align*}
    the sum is over the set of configuration matrix.
...
\begin{definition}
Define $I_n$ to be the $n$-cube under this subdivision of $P_{n+1}$. 
Let $e$ be a cell in $P_{n+1}$ with dimension $|e|$,
\begin{itemize}
    \item a \emph{$k$-subdivision cube} of $e$ is a set of faces whose union is a $k$-subcube of $I_n$ for some $k\leq |e|$.
...
\begin{itemize}
    ...
    \end{enumerate}
    \item We denote the set of all such pairs by $e_v$, and denote its unique maximal element by $e_v^*$.
\end{itemize}
\end{definition}
\begin{example}
In $P_4$ let $e$ be the top-dimensional cell $1234$. Then the following sets of faces are all $k$-division subcubes of $e$
\begin{align*}
    \{1234\}, \{1|234\}, \{1|234, 14|23\} \{4|13|2\}, \{1|3|24,1|43|2,14|3|2\}.
\end{align*}
The follow two sets of faces are both not $k$-division subcubes of $e$,
\begin{align*}
...
\end{align*}
Note there are two elements in $ 14|23 \times 4|13|2, 1|234 \times 4|13|2 \in e_v^*$, an $SCP$ and a right shift of it.

\end{example}

\begin{lemma}
The unique maximal pair $e_v^*$ is well-defined.
\end{lemma}
\begin{proof}
\Kurt{TODO. }
\end{proof}
...
\end{lemma}
\begin{proof}
We first observe that the $SCP$ of $v$ is in $e_v^*$.


\begin{itemize}
    \item $SCP$ is in $e_v^*$
    \item Apply shifts stays within $e_v^*$
    \item ah 1-shifts are not only a lattice, but a cubical lattice... \Kurt{}
\end{itemize}
\end{proof}
...
\begin{example}
Here are two examples shifts illustrated on the subdivision of the cube.
In the left diagram, the permutation $\mathbf{\blue{4|1|3|2}}$ has $SCP$ $\mathbf{\blue{14|23\times 4|13|2}}$ and right shifts to $\mathbf{\red{1|234\times 4|13|2}}$.
In the right diagram, the permutation $\mathbf{\blue{3|1|4|2}}$ has $SCP$ $\mathbf{\blue{13|24\times 3|14|2}}$ and left shifts to $\mathbf{\red{13|24\times 43|1|2}}$.
\begin{center}
{\footnotesize
...
The $LA$ shifts have been drawn so the leftmost arrow shifts the smallest element, and the $SU$ shifts have been drawn so the leftmost arrow shifts the largest element.
As such, the face poset isomorphism $t(r\times r)$ directly translates one diagram to other.
The specific element being shifted can be inferred by by the source and target of the arrow.
\begin{center}
{\small
\begin{tikzcd}
