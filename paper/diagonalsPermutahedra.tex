...
In this shorter third part of the paper, we derive some higher algebraic consequences of the preceding results. 
...
We now transport via this bijection the partial order of the flat lattice on rainbow forests. Before stating the proposition, we introduce the following notations: the set of vertices of colored $c$ in a rainbow forest $F$ is denoted by $V_c(F)$. Moreover, given a vertex $a$ in $F$, the root of the connected component containing $a$ is denoted by $\operatorname{Root(a)}$ and the set of children of $a$ is denoted by $\operatorname{Child}(a)$.
...
\begin{figure}
	\centerline{\includegraphics[scale=.7]{CoverRelRFFig}}
...
As of terminology, note that the datum of a polytope together with a generic vector was called \emph{positively oriented polytope} in \cite{MasudaThomasTonksVallette,LaplanteAnfossi,LaplanteAnfossiMazuir}.
...
For $m\geq 1$, the \defn{right $m$-shift}, moves the subset $M$, $m$ blocks to the right, \ie
\begin{align*}
R_M(\sigma) \eqdef \sigma_1 | \cdots | \sigma_i \ssm M | \cdots | \sigma_{i+m} \cup M | \cdots | \sigma_k
\end{align*}
while the \defn{left $m$-shift}, moves the subset $M$, $m$ blocks to the left, \ie
\begin{align*}
...
Moreover, from the equivalence between $1$-shifts and $m$-shifts (\cref{prop:iso-1-to-m-shift}), there exists a $m$-shift of $\rho$ from $\sigma_i$ if and only if there exists a sequence of $m$ consecutive $1$-shifts, each satisfying $\rho > \max \sigma_{j,j+1}=\max \sigma_{j+1}$.
...
For $k\leq \ell$, a \defn{$k$-subdivision cube of $F$} is a union of $k$-faces of $F$ whose underlying set is a $k$-cube.
\end{definition}
...
The \defn{sandglass}\Kurt{Maybe hourglass instead?} of $F$ at $v$ is the set $\subdivpairsev(F)$ of all pairs $(C^v,C_v)$ of $(k_1,k_2)$-subdivision cubes of $F$ such that
\begin{enumerate}
...
\end{align*}
This defines the sandglass $\subdivpairsv$, whose maximal pair of subdivision cubes is $\maxsubdivpairsv=\{ 1|234, 13|24, 14|23, 134|2\} \times \{4|3|12\}$.
