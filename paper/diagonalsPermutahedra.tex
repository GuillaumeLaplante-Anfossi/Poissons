...
\newcommand{\subdivpairsv}{\raisebox{0.065cm}{\rotatebox[origin=c]{90}{$\bowtie$}}_v}
...
Such diagonals, and in particular coherent families that we call \emph{operadic diagonals} (\cref{def:operadicDiagonal}), are of interest in algebraic geometry and topology: via the theory of Fulton--Sturmfels~\cite{FultonSturmfels}, they give explicit formulas for the cup product on Losev--Manin toric varieties~\cite{LosevManin}; they define universal tensor products of homotopy associative permutative (shuffle) algebras~\cite{LodayRonco-permutads,Markl}; they define a coproduct on permutahedral sets, which are models of two-fold loop spaces~\cite{SaneblidzeUmble}, and their study is needed to pursue the work of H. J. Baues aiming at defining explicit combinatorial models for higher iterated loop spaces~\cite{Baues}; using the canonical projections to the operahedra, associahedra and multiplihedra, they define universal tensor products of homotopy operads, $\Ainf$-algebras and $\Ainf$-morphisms, respectively~\cite{LaplanteAnfossi,LaplanteAnfossiMazuir}.
...
More precisely, the algebraic structure encoded by the permutahedra is that of homotopy associative permutative algebra~\cite{LodayRonco-permutads,Markl}.
...
Their defining partial order generalizes the weak order on permutations, \Guillaume{and is related to the facial order...?}
...
As the associahedra and the permutahedra are part of the family of operahedra, we get analogous results for $\Ainf$-algebras and homotopy associative permutative algebras. 

However, the isomorphism of topological operads takes place a category of polytopes $\PolySub$ for which the morphisms are \emph{not} affine maps, see \cite[Def. 4.13]{LaplanteAnfossi}, and does \emph{not} commute with the diagonal maps (\cref{ex:iso-not-Hopf}).
...
\begin{remark}
We can define a partial order on ordered partitions by saying $\sigma\leq\tau$ if the inversion set of $\sigma$ is a subset of the inversion set of $\tau$.
This partial order refines into the weak facial order of the permutahedra, introduced in \cite{palacios2004weak}, and shown to be a lattice in \cite{dermenjian2018facial}.
Later in \cref{sec:Shift-lattice}, we will show that every SCP and all its admissible sequences of left and right shifts is a sub-lattice of either partial order.
For now, we illustrate the weak facial order for $\mathsf{Perm}(3)$ as a refinement of the defined inversion order, grouping elements with the same inversion sets.
\begin{center}
\begin{tikzpicture}
\node (1) at (-1.5, -1) {$3|12$};
\node (2) at (-2.5, -2) {$3|1|2$};
\node (3) at (-2.5, -3) {$13|2$};
\node (4) at (-2.5, -4) {$1|3|2$};
\node (5) at (-1.5, -5) {$1|23$};
%
\node (6) at (1.5, -1) {$23|1$};
\node (7) at (2.5, -2) {$2|3|1$};
\node (8) at (2.5, -3) {$2|13$};
\node (9) at (2.5, -4) {$2|1|3$};
\node (10) at (1.5, -5) {$12|3$};
%
\node (11) at (0, 0) {$3|2|1$};
\node (12) at (0, -3) {$123$};
\node (13) at (0, -6) {$1|2|3$};
...
\draw[thick] (10) -- (13);
%
\hedge[green, very thick]{1,11,6,12}{0.45cm}
\hedge[green, very thick]{2,3}{0.45cm}
\hedge[green, very thick]{4,5}{0.45cm}
\hedge[green, very thick]{7,8}{0.45cm}
\hedge[green, very thick]{9,10}{0.45cm}
\end{tikzpicture}
\end{center}
...
Let $\sigma = \sigma_1|...|\sigma_k$ denote an ordered partition.
Then the weak facial order lattice of the permutahedra is the transitive closure of the following relations,
\begin{enumerate}
    \item $\max \sigma_i < \min \sigma_{i+1} \implies \sigma < \sigma_1|...|\sigma_i \sqcup \sigma_{i+1}|...|\sigma_k$
    \item $\min \sigma_i > \max \sigma_{i+1} \implies \sigma > \sigma_1|...|\sigma_i \sqcup \sigma_{i+1}|...|\sigma_k$
\end{enumerate}
\end{definition}

\begin{proposition}
The Weak facial order is a refinement of the inversion order.
\end{proposition}
\begin{proof}
This is clear as the inversion table used by the weak facial order (Example 3.6 of \cite{dermenjian2018facial}) is a refinement of our notion of an inversion.
In particular, their notion of an inversion distinguishes when two elements are in the same block.
\end{proof}


\begin{lemma}
Shifts of either diagonal respect the weak facial order.
\end{lemma}
\Kurt{Doing something silly here, might be false...}
\begin{proof}
If $R_\rho$ is a well-defined $1$ shift of $(\sigma,\tau) \in \LAD$, then
\begin{align*}
    \tau_1|...|\tau_k &< \tau_1|...|\tau_i\setminus \rho| \rho|\tau_{i+1}|...|\tau_k &(2) \\
    &< \tau_1|...|\tau_i\setminus \rho|\tau_{i+1}|...|\tau_k &(1) 
\end{align*}
where $(2)$ applies as $\rho < \min \tau_{i,i+1}\leq \min \tau_i\setminus \rho$, and $(1)$ applies as $\rho< \min \tau_{i,i+1} \leq \min \tau_{i+1}$.\Kurt{Both false what is a counter example?}
i.e. we know that $\tau < R_\rho(\tau)$ by stepping through the facet of $P_n$ between these two faces.
We can similarly show that for a well-defined $\LA$ left shift that $L\sigma < \sigma$.
The proof for the $\SU$ diagonal for once proceeds differently.
f $R\rho$ is a well-defined $1$ shift of $(\sigma,\tau) \in \SU$, then 
\begin{align*}
        \sigma_1|...|\sigma_k &< \sigma_1|...|\sigma_{i-1} \sqcup \sigma_{i}|...|\sigma_k &(1) \\
        &< \sigma_1|...|\sigma_{i-1}\sqcup \rho| \sigma_{i}\setminus \rho|...|\sigma_k &(2)
\end{align*}
where $(1)$ applies as $\rho \leq \max \sigma_{i-1}$...
\end{proof}
...
The \defn{hourglass} of $F$ at $v$ is the set $\subdivpairsv(F)$ of all pairs $(C^v,C_v)$ of $(k_1,k_2)$-subdivision cubes of $F$ such that
\begin{enumerate}
	\item $C^v$ has maximal vertex $v$,
	\item $C_v$ has minimal vertex $v$, and
	\item $k_1+k_2 = \dim F$.
\end{enumerate}
If $F$ is the maximal face of $\Perm$, then we denote the hourglass of $F$ simply by $\subdivpairsv$.
The \defn{maximal pair} of the hourglass $\maxsubdivpairsv(F)$ (\resp $\maxsubdivpairsv$) is the unique maximal pair of $\subdivpairsv(F)$ (\resp $\subdivpairsv$) with respect to inclusion.
\end{definition}
...
We clarify the previous section with examples that are visually verifiable from the diagram of $\divcube{3}$ of \cref{fig:hourglass1}.
The sets of faces 
\begin{align*}
...
\end{align*}
This defines the hourglass $\subdivpairsv$, whose maximal pair of subdivision cubes is $\maxsubdivpairsv=\{ 1|234, 13|24, 14|23, 134|2\} \times \{4|3|12\}$.
...
\begin{figure}
	...
	\end{center}
	\caption{The hourglass~$\subdivpairsv(F)$ of $F = \Perm[4]$ at~$v = \blue{\mathbf{4|3|1|2}}$.}
...
The set of pairs of subdivision cubes in the hourglass $\subdivpairsv$ of $v$ satisfies the following properties
\begin{enumerate}
...
The appropriate base definitions for the $\LA$ diagonal of $k$-subdivision cubes (\ref{def:Subdivisions}), hourglasses of subdivision cubes ${\subdivpairsv}$ (\ref{def:hourglass}), and maximal pairs of subdivision cubes $\maxsubdivpairsv$ are all the same as in the $\SU$ case.
...
\begin{figure}
	...
	\caption{The isomorphism~$r$ applied to \cref{fig:hourglass1}.}
