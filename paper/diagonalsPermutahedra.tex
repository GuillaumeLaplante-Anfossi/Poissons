...
\newcommandx{\orient}[1]{\smash{\overrightarrow{#1}}} % ordered
...
\begin{definition}
For any integers~$\ell,n \geq 1$, and a generic matrix~$\b{a} \eqdef (a_{i,j}) \in M_{\ell,n-1}(\R)$ of~$\R^n$, the \defn{$\b{a}$-braid arrangement}~$\multiBraidArrangement(\b{a})$ is the arrangement of hyperplanes~$\set{\b{x} \in \HH}{x_r - x_s = A_{i,r,s}}$ for all~$1 \le r < s \le n$ and~$i \in [\ell]$, where $A_{i,r,s} \eqdef \smash{\sum_{r \le j < s} a_{i,j}}$.
\end{definition}
...
In this section, we describe the flat poset of~$\multiBraidArrangement$ and derive explicit formulas for its $f$-vector.
...
In this section, we start from a partition forest~$\b{F}$ and describe all ordered partition forests~$\orient{\b{F}}$
...
In this section, we start from an ordered partition forest and decides if it is valid for~$\b{A}$.
