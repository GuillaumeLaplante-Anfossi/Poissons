...
We define the set of \emph{configuration matrices} to be those matrices corresponding to $SCP$s and those generated by admissible sequences of shifts.
...
In ... Saneblidze and Umble first identified their diagonal through the following subdivision of the $n$-cube by the permutahedra $P_{n+1}$.
...
Let $\sigma$ be a permutation in $P_{n+1}$, then both partitions of its SCP $(\sigma',\tau')$ are faces of $P_{n+1}$ which are adjacent to $\sigma$, and collectively the sum of their number of blocks satisfies $|\sigma'| + |\tau'| = n+1$.
As we have subdivided the cube through $P_{n+1}$, each vertex of the subdivision corresponds to a permutation $\sigma$, and the vertex is adjacent to its two SCP faces.
\\\\
In their construction of the diagonal of the permutahedra \cite{saneblidzeComparingDiagonalsAssociahedra2022}, Saneblidze and Umble show that the SCP of a vertex $\sigma$ are unique maximal subdivision cubes (in their notation  $(I_{\sigma,1}^{k_1}, I_{\sigma,2}^{k_2})_e$).
They then show in Formula $(3)$ that all other subdivision cubes $(I_{\sigma,1}^{k_1}, I_{\sigma,2}^{k_2})$ are generated by shifts of the maximal subdivision cubes.
This establishes their formula for the diagonal, by showing that Formulae $(1),(2)$ admit the same recursive structure and are hence the same.
\\\\
We won't step through these details in full, however here are two examples shifts illustrated on the subdivision of the cube.
\Kurt{Do $4132$ and $1342$.}
\begin{center}
...
\end{tikzcd}
\end{center}
\Kurt{Then show how $t(r,r)$ acts on the cube and these shifts.}
...
We indicate shifts by the element being shifted, drawing them to avoid crossings, and so they align with their isomorphic shift under $\theta$. 
\begin{center}
{\small
\begin{tikzcd}
& 5|17|4|236 \arrow[ld,"\rho=3"] \arrow[d,"\rho=1"] \arrow[rd,"\rho=2"]\\
5|17|34|26 \arrow[rd,"\rho=1"] \arrow[d,"\rho=2"]&
15|7|4|236  \arrow[rd,"\rho=2"] \arrow[d,"\rho=3"]&
5|17|24|36 \arrow[d,"\rho=1"]\\
5|17|234|6 \arrow[d,"\rho=1"]&
15|7|34|26  \arrow[dl,"\rho=2"]&
15|7|24|36  \\
15|7|234|6 
\end{tikzcd}
\begin{tikzcd}
& 3|17|4|256 \arrow[ld,red,"\rho=5"] \arrow[d,"\rho=7"] \arrow[rd,"\rho=6"]\\
3|17|45|26 \arrow[rd,"\rho=7"] \arrow[d,red,"\rho=6"]&
37|1|4|256 \arrow[rd,"\rho=6"] \arrow[d,"\rho=5"]&
 3|17|46|25 \arrow[d,"\rho=7"]\\
3|17|456|2 \arrow[d,"\rho=7"]&
37|1|56|24 \arrow[dl,"\rho=6"]&
37|1|46|25 \\
37|1|456|2
\end{tikzcd}
}
