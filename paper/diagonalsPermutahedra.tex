...
Building on~\cite{MasudaThomasTonksVallette}, a general theory of topological cellular diagonals was developed in~\cite{LaplanteAnfossi}.
\vincent{I would say geometric here, rather than topological cellular.}
...
An important feature of the operahedra is that they are all obtained from the permutahedra by projection (they are generalized permutahedra in the sense of~\cite{Postnikov}), and it was shown in~\cite[Sect.~1.6]{LaplanteAnfossi} that to understand their diagonals, it suffices to understand operadic diagonals of permutahedra, which is the purpose of the present paper.
\vincent{This is incorrect. A generalized permutahedron is a deformation of the permutahedron, not a projection...}
...
\cref{sec:shifts} is devoted to the study of alternate formulae for the operadic diagonals of the permutahedra.
We prove that the geometric $\SU$ diagonal is a topological enhancement of the original $\SU$ diagonal.
We then transfer all known definitions for both operadic diagonals via isomorphism, and show their facets admit a simple lattice structure.
Our main result is,
\vincent{would say a little more here...}
...
\begin{figure}[h!]
	...
	\end{center}
\end{figure}
\Guillaume{Complete refs}

In \cref{sec:Shift-lattice} we show that the facets of operadic diagonals admit a natural lattice structure that we call the \emph{shift lattice}.\Guillaume{@Kurt, do you want to highlight a feature of this lattice? Should we call it "cubical"?}
We devote the last two subsections of \cref{sec:shifts} to the representation of the facets of operadic diagonals as matrices, and as pairs of subcubes in a cubical representation of the permutahedra.
Both of these descriptions come from~\cite{SaneblidzeUmble}.
Along the way, we provide proofs of some results\Guillaume{Cite them here} that were presented there as facts.
\vincent{To discuss...}
...
In this last Part, we use the results of \cref{sec:operadicDiagonals} 

\begin{theorem*}[\cref{thm:operahedra,thm:top-iso}] 
There are exactly 
\begin{enumerate}
\item two operadic diagonals of the Loday operahedra, given by the $\LA$ and $\SU$ diagonals, 
\item two colored topological cellular operad structures on the Loday operahedra, and
...
Observe however that this isomorphism of topological operads in the category of polytopes (REF) does not commute with the diagonal maps (\cref{ex:iso-not-Hopf}), and that the pairs of faces in the image of the two operadic diagonals are not in bijection, yielding different (but $\infty$-isomorphic) tensor products of homotopy operads (\cref{subsec:tensor-products}).  
