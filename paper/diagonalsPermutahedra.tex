...
\begin{example}
	...
	\end{itemize}
	Consequently, $(\sigma,\tau)$ has inversions $I = I_\tau \ssm I_\sigma = \{(2,5), (2,6), (2,7), (4,7)\}$, and these are precisely the crossings of the ordered $(2,n)$-partition tree.
	\end{example}
...
For Point (2), the $(I,J)$ conditions for $\card{I} = \card{J} = 1$ imply that $\rho$ must appear before $\min \sigma_i$ in $\tau$ (\cref{thm:minimal}).
Therefore, the $m$-shift creates a new crossing between these two edges.
\Guillaume{Why is there no deletion of inversions?}
\end{proof}
...
\end{align*}
where the unions are taken over all $\SCP$s $(\sigma, \tau)$ of $[n]$, and over all block-admissible sequences of subset $1$-shifts $\b{M},\b{N}$ (\resp over all path-admissible sequences of singleton $m_j$-shifts, for various $m_j \ge 1$).
\end{definition}
...
\subsection{The Weak Facial Order}\label{sec:Weah Facial Order}

The weak facial order of the the permutahedra, introduced in \cite{palacios2004weak}, generalizes the weak order on permutations to all ordered partitions.
In this section we recall a definition of the weak facial order, and characterize facets of both the $\LA$ and $\SU$ diagonal as intervals in this order.
...
\begin{align}
    \max \sigma_i < \min \sigma_{i+1} &\implies \sigma < \sigma_1|...|\sigma_i \sqcup \sigma_{i+1}|...|\sigma_k  \label{eq:Weak Facial 1}\\
    \min \sigma_i > \max \sigma_{i+1} &\implies \sigma > \sigma_1|...|\sigma_i \sqcup \sigma_{i+1}|...|\sigma_k \label{eq:Weak Facial 2}
\end{align}
\end{definition}
...
\begin{proposition}
The weak facial order is a refinement\Kurt{check correct term} of the inversion order.
\end{proposition}
\begin{proof}
...
\begin{example}
We illustrate the weak facial order for $\Perm[3]$ as a refinement of the weak inversion order, grouping elements with the same weak inversion sets.
\begin{center}
\begin{tikzpicture}
\node (1) at (-1.5, -1) {$3|12$};
...
\begin{lemma}\label{lem:SCPs are intervals of weak facial order}
Let $(\sigma,\tau)$ be the SCP corresponding to the permutation $v$, then $\sigma<v<\tau$ under the weak facial order.
\end{lemma}
\begin{proof}
Firstly $\sigma<v$, as repeated applications of \cref{eq:Weak Facial 2} shows that a block of elements is smaller than those same elements arranged in decreasing order.
Similarly $v<\tau$, as repeated applications of \cref{eq:Weak Facial 1} shows that a sequence of increasing elements is smaller than those same elements in a block. 
To ground this in \cref{ex:shift translation by theta}, $13|247|5|6 < 3|1|7|4|2|5|6 <3|17|4|256$.
\end{proof}

More generally,
\begin{proposition}
Facets $(\sigma,\tau)$ of the $\LA$ and $\SU$ diagonal correspond to intervals $[\sigma,\tau]$ of the weak facial order.
\end{proposition}
\begin{proof}\
We proceed by induction.
Let $(\sigma,\tau) \in \SUD$ be a facet generated by shifts of the SCP corresponding to the permutation $v$, and suppose that $\sigma < v < \tau$.
Given \cref{lem:SCPs are intervals of weak facial order}, we have a base case for induction.
We then prove that if $R_\rho \tau$ is an admissible right shift of $\rho$, then $\sigma < v < R_\rho \tau$.
As the analogous results for left shifts and $\LA$ shifts proceed similarly, this yields the result.

Let $v'$ be the permutation resulting from writing out the blocks of $R_\rho \tau$ in increasing order, then $v<v'<R_\rho \tau$.
The second equality follows as in the prior proof.
To prove the first equality, we show that $I_<(v)\subset I_<(v')$ (as the the weak facial order on permutations is the weak order).
This follows as \Kurt{Clear, but think of something concise.}
\end{proof}

So given this result and \cref{thm:patterns} (which shows that vertices are intervals in the weak order, and hence the weak facial order), it is suspected that all faces of the $\LA$ and $\SU$ diagonals are intervals of the weak facial order. 
\Kurt{Do we care enough to look? Any quick ideas?}
...
Given that in \cite{dermenjian2018facial} it was shown that the weak facial order is a lattice, it is natural to wonder if the identified lattice structure arises as a sub-lattice of the weak facial order.

\begin{proposition}
The shift lattices are not sub-lattices of the weak facial order.
\end{proposition}

\begin{proof}
We provide a counter example for the $\LA$ diagonal that illustrates the identified lattice structure is not compatible with the weak facial order.
...
\node[anchor=east] (3) at (-1.5, -3) {$34$};
%
\node[anchor=west](4) at (1.5, -1) {$124$};
\node[anchor=west](5) at (1.5, -2) {$3$};
%
\draw[thick] (1.east) -- (4.west); 
\draw[thick] (2.east) -- (4.west);
\draw[thick] (3.east) -- (4.west);
\draw[thick] (3.east) -- (5.west);
\end{tikzpicture}
\raisebox{2.15em}{$\xrightarrow{R_{2}}$}
\begin{tikzpicture}[scale=.6]  
\node[anchor=east] (1) at (-1.5, -1) {$1$};
\node[anchor=east] (2) at (-1.5, -2) {$2$};
\node[anchor=east] (3) at (-1.5, -3) {$34$};
...
\draw[thick] (3.east) -- (5.west);
\end{tikzpicture}
\raisebox{2.15em}{$\xrightarrow{R_{1}}$}
\begin{tikzpicture}[scale=.6]  
\node[anchor=east] (1) at (-1.5, -1) {$1$};
\node[anchor=east] (2) at (-1.5, -2) {$2$};
\node[anchor=east] (3) at (-1.5, -3) {$34$};
%
\node[anchor=west](4) at (1.5, -1) {$4$};
\node[anchor=west](5) at (1.5, -2) {$123$};
%
\draw[thick] (1.east) -- (5.west); 
\draw[thick] (2.east) -- (5.west);
\draw[thick] (3.east) -- (4.west);
...
\end{tikzpicture}
\end{center}
The final right shift does not respect the weak facial order as $14|23$ and $4|123$ are incomparable.
\end{proof}
We note that as the weak inversion order respects shifts (e.g. $14|23 < 4|123$), it is the additional information in the refinement of the weak facial order, over the weak inversion order (\cref{sec:Weah Facial Order}), that is breaking the lattice structure.
...
In the next section, we will show that $\maxsubdivpairsv$ is generated by all shifts of the $\SCP$ corresponding to $v$.
%
\begin{figure}
	\begin{center}
	{\small
...
\begin{enumerate}
	\item The $\SCP$ $(\sigma,\tau)$ corresponding to $v$ is in $\subdivpairsv$,
