...
One initial consequence is given the bijection between SCPs and permutations, and our prior enumeration of the facets of the diagonal, this produces two new statistics on permutations.
...
\begin{remark}
The calculation of each height can also be reformulated directly in terms of the permutation.
...
Recall from \cref{subsec:inversions} that the inversions of an ordered $(2,n)$-partition tree are the set of edge crossings. 
...
\begin{proof}
We define $L_\rho^m$ (\resp $R_\rho^m$) to be a $m$-left (right) shift of $\rho$ for $m>0$, and the identity for $m=0$.
...
We note however that any lattice $L$ on permutations will induce a lattice on the facets.
In particular, for every $v \in L$, we can substitute the lattice $T_v^\LA$ (or $T_v^\SU$) into the permutation $v$.
...
Given a matrix $A$ with all non-zero entries the elements $[n]$, let $\sigma_i(A)$ denote the non-zero entries of the $i$\ordinal{} column, and $\tau_j(A)$ the non-zero entries of the $(l-j+1)$\ordinalst{} row, see the labelling in \cref{fig:step-matrix}.
...
The translation of admissibility of sequence of shifts (\cref{def:SU-admissible,def:LA-admissible}) is also straightforward.
We work here with the original $\SU$ diagonal.

\begin{definition}
	A right shift $R_M$ of a non-empty set of elements of a matrix $M \subset \sigma_i(A)$ (\resp $M_{i}\subset \tau_{i}(A)$) \defn{block-admissible} if
	\begin{enumerate}
...
\begin{proposition}
The matrix shift operators are well-defined.
\end{proposition}
\begin{proof}
...
The \defn{configuration matrices} \cite[Def. 7]{SaneblidzeUmble} are the matrices corresponding to $\SCP$s and those generated by admissible sequences of shifts.
From the definitions we get that the set of $\SU$ configuration matrices are in bijection with the facets of the $\SU$ diagnonal.
...
This means we don't have to worry about generic boundaries imposed by generic enveloping faces $F$. \Guillaume{What does generic mean??}
