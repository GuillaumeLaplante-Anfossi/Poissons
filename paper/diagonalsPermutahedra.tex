...
We shall concentrate on three distinct families of shifts:
\begin{enumerate}[(a)]
    \item $1$-shifts of $1$-block movable subsets of various sizes,
    \item $1$-shifts of $1$-path movable singletons, and
    \item $m$-shifts of $m$-path movable singletons, for various $m\geq 1$,
\end{enumerate}
and show that specific sequences all generate the same diagonal. 
...
\begin{definition}[Admissible sequences of shifts]
    ...
    \item \emph{admissible of $1$-path type} if
    \begin{enumerate}[(1)]
        \item each $M_{i_j}$ is a $1$-path movable element of $R_{M_{i_j-1}}\dots R_{M_{i_1}}(\sigma)_{i_j}$.
    \end{enumerate}
...
\begin{definition}[$\SU$ diagonals]
   ...
   \begin{enumerate}[(a)]
    ...
    \item the \emph{$1$-shift $\SU$ diagonal},
...
\subsubsection{First isomorphism: original and $1$-shift $\SU$ diagonals}
...
It is immediate for cases (1), (2a) and (3); in the case (2b) it follows from the observation that the number of steps of $\beta \cap \delta_{k,k+1}$ and $\delta_{k,k+1} \ssm \beta$ have the same parity. \Guillaume{rewrite}
\end{proof}
...
\subsubsection{Third isomorphism: $m$-shift and $(I,J)$ coincide}
...
\begin{definition} 
\label{def:critical SU inverse shift}
