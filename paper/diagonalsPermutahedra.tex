...
\newcommand{\subdivpairsev}{S_v}
\newcommand{\maxsubdivpairsev}{M_v}
\newcommand{\subdivpairsv}{S_v}
...
Such diagonals, and in particular coherent families that we call \emph{operadic diagonals} (\cref{def:operadicDiagonal}), are of interest in algebraic geometry and topology: via the theory of Fulton--Sturmfels~\cite{FultonSturmfels}, they give explicit formulas for the cup product on Losev--Manin toric varieties~\cite{LosevManin}; they define universal tensor products of homotopy operads, and in particular universal tensor products of homotopy associative permutads (shuffle algebras)~\cite{LaplanteAnfossi}; they define a coproduct on permutahedral sets, which are models of two-fold loop spaces~\cite{SaneblidzeUmble}, and their study is needed to pursue the work of Baues aiming at defining explicit combinatorial models for higher iterated loop spaces~\cite{Baues}; using the canonical projections to the multiplihedra and the associahedra, they define universal tensor product of $\Ainf$-algebras and $\Ainf$-morphisms~\cite{LaplanteAnfossiMazuir}.
...
Second, it satisfies the \emph{magical formula} of J.-L. Loday: the faces in the image of the diagonal are given by the pairs of faces which are comparable under the Tamari order (see \cref{sec:cellularDiagonals,rem:magicalFormula} for a precise statement). 
...
%\begin{center}
%\begin{tabular}{c|c}
%\textbf{Magical formula} & \textbf{Non-magical formula} \\
%\hline
%%Simplices, Cubes, Freehedra, Associahedra & Multiplihedra, Operahedra, Permutahedra 
%Simplices & Multiplihedra \\
%Cubes & Operahedra \\
%Freehedra & Permutahedra \\
%Associahedra 
%\end{tabular}
%\end{center}
%\end{figure}

Since the operahedra are \emph{generalized permutahedra}~\cite{Postnikov}, their operadc diagonals are completely determined by the by the operadic diagonals of permutahedra (see~\cite[Sect.~1.6]{LaplanteAnfossi}), which is the purpose of study of the present paper.
...
More precisely, the algebraic structure encoded by the permutahedra is that of homotopy associative permutad~\cite{LodayRonco-permutads,Markl}.
...
Moreover, there are poset isomorphisms between their cellular images.
\end{theorem*}

It turns out that their facets and vertices admit elegant combinatorial descriptions. 
...
A pair of ordered partitions $(\sigma,\tau)$ forming a partition tree is a facet of the $\LA$ (\resp $\SU$) geometric diagonal if and only if the minimum (\resp maximum) of every directed path between two consecutive blocks of $\sigma$ or $\tau$ is oriented from $\sigma$ to $\tau$, that is from left to right (\resp from $\tau$ to $\sigma$, that is from right to left).
\end{theorem*}
...
The proof of this result, quite technical, procedes by showing the equivalence between $4$ different descriptions of the diagonal: the original, $1$-shift, $m$-shift and geometric $\SU$ diagonals (\cref{subsec:topological-SU}).
This brings a positive answer to~\cite[Rem.~2.19]{LaplanteAnfossi}, showing that the original $\SU$ diagonal can indeed be recovered from a choice of chambers in the fundamental hyperplane arrangement of the permutahedron. 
...
Moreover, in \cref{sec:Shift-lattice} we show that the facets of operadic diagonals admit a natural lattice structure that we call the \emph{shift lattice}.
This lattice is a product of cubical lattices, indexed by the permutations of $[n]$.

Finally, we present the alternative matrix (\cref{subsec:matrix}) and cubical (\cref{sec:Cubical}) descriptions of the $\SU$ diagonal from \cite{SaneblidzeUmble,SaneblidzeUmble-comparingDiagonals}, providing proofs of their equivalence with the other descriptions, and giving their $\LA$ counterparts. 
...
Indeed, this resolution is encoded by the dual subdivision of the permutahedron, which is cubical since $\Perm$ is a simple polytope, and a diagonal can be obtained from the classical Serre diagonal via retraction, see \cite{MarklShnider} and \cite[Sec. 5.1]{LaplanteAnfossiMazuir} for the case of the associahedra.
...
In this shorter third part of the paper, we derive some higher algebaic consequences of the preceding results. 
These higher algebraic considerations were the original motivation for the present study.
...
The faces of an operahedron are in bijection with nestings, or parenthesization, of the corresponding planar tree. 
...
\begin{enumerate}
	...
	\item two geometric colored topological cellular operad structures on the Loday operahedra, and
...
As the associahedra and the permutahedra are part of the family of operahedra, we get analogous results for $\Ainf$-algebras and homotopy associative permutads. 

However, the isomorphism of topological operads, which takes place in the category of polytopes $\PolySub$ (for which the morphisms are \emph{not} affine maps, see \cite[Def. 4.13]{LaplanteAnfossi}) does \emph{not} commute with the diagonal maps (\cref{ex:iso-not-Hopf}).
...
The \defn{faces} of~$\arrangement$ are the closures of the regions of~$\arrangement$ and all their intersections with an hyperplane of~$\arrangement$.
...
Hence, $\pi$ denotes an unordered set partition (the order is irrelevant, neither inside each part, nor between two distinct part), while~$\order{\pi}$ denotes an ordered set partition (the order inside each part is irrelevant, but the order between distinct parts is relevant).
...
(Note that the refinement in~$\orderedPartitionPoset$ and in~$\partitionPoset$ is in opposite direction.)
...
The \defn{$(\ell,n)$-braid arrangement}~$\multiBraidArrangement$ is the arrangement in~$\HH$ obtained as the union of $\ell$ generically translated copies of the braid arrangement~$\braidArrangement$.
\end{definition}
...
The \defn{intersection hypergraph} of an $\ell$-tuple~$\b{F} \eqdef (F_1, \dots, F_\ell)$ of set partitions of~$[n]$ is the $\ell$-regular $\ell$-partite hypergraph on all parts of all the partitions~$F_i$ for~${i \in [\ell]}$, with an hyperedge connecting the parts containing~$j$  for each~$j \in [n]$.
\end{definition}

\begin{definition}
\label{def:partitionForests}
An \defn{$(\ell,n)$-partition forest} (\resp \defn{$(\ell,n)$-partition tree}) is an $\ell$-tuple~$\b{F} \eqdef (F_1, \dots, F_\ell)$ of set partitions of~$[n]$ whose intersection hypergraph is a hyperforest (\resp hypertree).
...
In other words, given an $(\ell,n)$-partition forest $\b{G} \eqdef (G_1, \dots, G_\ell)$, any $\ell$-tuple~$\b{F} \eqdef (F_1, \dots, F_\ell)$ of partitions satisfying~$F_i \le_{\partitionPoset[n]} G_i$ for all~$i \in [\ell]$ is an $(\ell,n)$-partition forest.
...
An \defn{$(\ell,n)$-rainbow forest} (\resp \defn{tree}) is an \mbox{$\ell$-rainbow} colored forest (\resp tree) with $\|F\| = n$ nodes.
...
\begin{proof}
We can transform an $\ell$-rainbow tree~$R$ to an $\ell$-ary tree~$T$ as illustrated in \cref{fig:rainbowTrees}.
...
\begin{definition}
For an $(\ell,n)$-rainbow forest~$F$, we define
\[
...
A \defn{labeling} of an $(\ell,n)$-rainbow forest~$F$ is a bijective map from the nodes of~$F$ to~$[n]$ such that
\begin{enumerate}[(i)]
...
The number~$\lambda(F)$ of labelings of an $(\ell,n)$-rainbow forest~$F$ is given by
\[
...
\begin{proof}
From a labeled $(\ell,n)$-rainbow forest~$F$, we construct an $(\ell,n)$-partition forest~$\b{F} \eqdef (F_1, \dots, F_\ell)$ whose $i$\ordinal{} partition~$F_i$ has a part~$\{N\} \cup C_i(N)$ for each node~$N$ of~$F$ not colored~$i$.
Condition~(i) of \cref{def:rainbowForest} ensures that each $F_i$ is indeed a partition.

Conversely, start from an $(\ell,n)$-partition forest~$\b{F} \eqdef (F_1, \dots, F_i)$.
...
We can can refine the formula of \cref{thm:verticesMultiBraidArrangement} according to the dimension of the flats of the different copies intersected to obtain the vertices of the $(\ell,n)$-braid arrangement~$\multiBraidArrangement$.
...
\begin{definition}
For an ordered partition~$\order{\pi} \eqdef \order{\pi}_1 | \dots | \order{\pi}_k$ of~$[n]$, we denote by~$D_{\order{\pi}}$ the directed graph on~$[n]$ with an arc~$\max(\order{\pi}_j) \to \min(\order{\pi}_{j+1})$ for each~$j \in [k-1]$ and a cycle~${x_1 \to \dots \to x_p \to x_1}$ for each part~$\order{\pi}_j = \{x_1 < \dots < x_p\}$.
...
In this second part, we study of the combinatorics of the diagonals of the permutahedra.
...
\begin{figure}[p]
	...
	\caption{The fundamental hyperplane arrangements of the $3$-dimensional simplex (left), cube (middle), and permutahedron (right). The hyperplanes which are perpendicular to edges of the polytope~$P$ are colored in yellow, while those which are not perpendicular to  edges of the polytope $P$, but only to edges of some intersection $P\cap \rho_z P$ are colored in blue. Left and rightmost illustrations from~\cite[Fig.~12]{LaplanteAnfossi}.}
...
For instance, the number of $k$-dimsensional faces in the diagonal of the $(n-1)$-dimensional simplex, cube, and associahedron are respectively given by
\begin{alignat*}{2}
...
To be more precise, the diagonal~$\triangle_{(\Perm, \b{v})}$ corresponds to the $\b{a}$-braid arrangement~$\multiBraidArrangement[2][n](\b{a})$ where the translation matrix~$\b{a}$ from \cref{def:multiBraidArrangementPrecise} has two rows, with first row~$\b{a}_{1,j} = 0$ and second row~$\b{a}_{2,j} = v_j-v_{j+1}$ for all~$j \in [n-1]$.
\end{remark}
...
This section is devoted to the combinatorics of two specific diagonals of the permutahedra, the $\LA$ and $\SU$ diagonals, which are shown to be the only operadic diagonals of the permutahedra.
...
In this section, we consider in particular the following two orderings and their corresponding diagonals.
...
The faces of the $\LA$ and $\SU$ diagonals are described by the following specialisation of \cref{thm:IJ-description}.
...
A family of diagonals $\triangle \eqdef \{\triangle_n : \Perm \to \Perm \times \Perm\}_{n\geq 1}$ of the permutahedra is \defn{operadic} if for every face $A_1 | \ldots | A_k$ of the permutahedron $\Perm[\card{A_1}+\cdots + \card{A_k}]$, the map $\Theta$ induces a topological cellular isomorphism
\[
...
To illustrate our proof of \cref{prop:operadicOrdering}, suppose that the $\LA$ ordering holds for pairs of order $1$ and $2$, but is reversed for pairs of order $3$, \ie
\begin{align*}
...
The pair~$(\sigma,\tau)$ is a facet of the $\LA$ (\resp $\SU$) diagonal if and only if the minimum (\resp maximum) of every directed path between two consecutive blocks of $\sigma$ or $\tau$ is oriented from $\sigma$ to $\tau$, that is from left to right (\resp from $\tau$ to $\sigma$, that is from right to left).
\end{theorem}
...
\enlargethispage{.5cm}
For each $k \ge 1$, there are $\binom{2k-1}{k-1,k}(k-1)!k!$ avoided standardised patterns.
...
Now suppose that for all $(I,J)$ of size $\card{I} = \card{J} \le m-1$, we have that if~$J$ dominates $I$ in $\sigma$ and $I$ dominates $J$ in $\tau$, then $(\sigma,\tau)$ contains a pattern of the form~(\ref{eq:pattern}).
...
Why?
If we define $(\sigma',\tau') \eqdef \std(\sigma \cap (I\cup J),\tau \cap (I\cup J))$, and $(I',J')\eqdef\std(I',J')$, 
...
We start by analyzing the $1$-shift $\SU$ diagonal, which is given by exactly the same definition as the $m$-shift diagonal, but where only $1$-shifts are performed. 
...
Path-admissible sequences of $1$-shifts, defining the $1$-shift $\SU$ diagonal, conserve 
\begin{enumerate}
	\item the maximal element of any path between two blocks of the same ordered partition,
	\item the direction in which this element is traversed. 
\end{enumerate}
In particular, if a pair of ordered partitions $(\sigma',\tau')$ is generated from a strong complementary pair~$(\sigma,\tau)$ by path-admissible $1$-shifts, then we have
\begin{align*}
...
We are how in position to show that the $1$-shift and $m$-shift descriptions are equivalent. 
...
Since $\sigma_{i,i+m}=\sigma_{i,i+1}\sigma_{i+1,i+m}$ we have that $\max \sigma_{i,i+m}\geq \max \sigma_{i,i+1}$, which means that the $1$-shift $R_\rho$ is path-admissible.
According to \cref{cor:SU-1-shift-preserves-max}, $1$-shifts conserve the maximal element of paths between adjacent blocks, and its directions.
Consequently, the following $(m-1)$-shift $R_\rho$ is path-admissible, and we can thus apply the induction hypothesis. 
\end{proof}

This isomorphism provides us with the following analogue of \cref{cor:SU-1-shift-preserves-max} for $m$-shifts. 
...
\begin{definition}
Let $i,j$ be elements of $[n]$, such that $i<j$.
We define $(i,j)$ to be an \defn{inversion of an ordered partition} $\sigma$ if $\sigma^{-1}(j)\leq\sigma^{-1}(i)$ (\ie $j$ is either before $i$, or in the same block).
We define $(i,j)$ to be an \defn{inversion of a pair of ordered partitions} $(\sigma,\tau)$, if $(i,j)$ is an inversion of $\tau$, and $(i,j)$ is not an inversion of $\sigma$.
\end{definition}

\begin{remark}
Our definition of an inversion of an ordered partition specializes to the classical definition of an inversion.
It is similar to existing definitions, 
\end{remark}
...
For the converse, one needs to check that an anti-inversion, where $y$ appears before $x$ in $\sigma$ and $x$ appears before $y$ in $\tau$, cannot occur in a facet of the geometric $\SUD$; this follows immediately from the $(I,J)$-conditions for $\card{I} = \card{J} = 1$. 
...
\end{itemize}
Consequently, $(\sigma,\tau)$ has inversions $I = I_\tau \ssm I_\sigma = \{(2,5), (2,6), (2,7), (4,7)\}$, and these are precisely the crossings of the ordered $(2,n)$-tree.
\end{example}
...
For Point (2), the $(I,J)$ conditions for $\card{I} = \card{J} = 1$ imply that $\rho$ must appear before $\min \sigma_i$ in $\tau$ (\cref{thm:IJ-description}).
...
\node[anchor=east] (2) at (-1.5, -2) {$\sigma_{i+1}$};
%
\node[anchor=west] (3) at (1.5, -1) {$\tau_{j-m} \ssm \rho$};
...
As $\rho'\eqdef \max \sigma_{i,i-n}$, and apart from the step $i_k$ we have that $\tau_{j-m,j}\subset \sigma_{i,i-n}$, we just need to show that $\rho' > i_k$.
...
The facets of the shift $\LA$ diagonals are defined by the formula
\begin{align*}
...
\end{align*}
where $r\mathbf{M} \eqdef (rM_{1},...,rM_{p})$ and $r\mathbf{N}$ (defined similarly) are admissible sequences of $\SU$ shifts of the same type.
\end{proposition}
\begin{proof}
...
Then, we have that $\max M < \min \sigma_{i,i+1}$ which implies that $\min r M > \max r \sigma_{i,i+1}$, and thus the right shift $R_{rM}$ is path-admissible in the $\SU$ sense (\cref{def:SU-admissible}).
Here, $rM \in r\sigma_i$ is interpreted as being in a block of the right partition ($\tau$ in the notation of the definition).
So, if $\mathbf{M} = (M_{i_1},...,M_{i_p})$, define $r(\mathbf{M}) \eqdef (rM_{1},...,rM_{p})$, and from the prior it is clear this is a path-admissible sequence of $\SU$ shifts, finishing the proof.
\end{proof}
Thus, given $t(r\times r)$ is an isomorphism of the geometric diagonals (\cref{rem:Alternate Isomorphism}), and the geometric $\SU$ diagonal coincides with the shift $\SU$ diagonals (\cref{subsec:SU-shifts}), it is immediate that
\begin{proposition}
...
\subsection{Shift lattice structure}
\label{sec:Shift-lattice}

In this section, we show that the $1$-shifts of the operadic diagonals $\LAD$ and $\SUD$, define a lattice structure on the set of facets.
More precisely, we show each $\SCP$ is the minimal element of a lattice isomorphic to a product of chains, where the partial order is given by shifts, or equivalently inversions.
One initial consequence is that given the bijection between SCPs and permutations, and our prior enumeration of the facets of the diagonal, this produces two new statistics on permutations.
...
Prior to defining the lattice structure, we produce two useful lemmas for characterizing it.
...
\begin{align*}
    l_v(\rho) \eqdef \max (\{0\}\cup \{m>0: L_{\rho}(\sigma) \text{ is a path admissible $\LA$ $m$-shift} \})\\
...
Let $(\sigma,\tau)$ be the SCP corresponding to $v$.
Then, the right $\LA$ height $r_v(\rho)$ (\resp the left $\LA$ height $l_v(\rho)$) of $\rho \in [n]$ is given by the number of consecutive blocks of $\sigma$ ($\tau$) to the right (\resp left) of the one containing $\rho$ whose minima are larger than $\rho$.
...
The $m$-shift operators defining the facets of an operadic diagonal ($\LA$ or $\SU$) commute. 
That is, whenever the successive composition of two (right or left) $m_1$, $m_2$-shifts is defined on a facet of the $\LA$ or $\SU$ diagonal, the reverse order of composition is also defined, and the two yield the same facet. 
\end{lemma}
\begin{proof}
...
We now define our lattices.

\begin{definition}
\label{def:shift-lattice}
For each permutation $v$ of $[n]$, let $T_v^\LA$ (\resp $T_v^\SU$) be the set of ordered $(2,n)$-partition trees, resulting from applying all $\LA$ ($\SU$) shifts to the SCP $(\sigma,\tau)$.
We define the \defn{shift lattices}, also denoted $T_v^\LA$ ($T_v^\SU$), to be these sets partially ordered by the inclusion of their inversions sets. 
\end{definition}

The partial order can also be defined in terms of crossings, given inversions are in bijection with crossings (\cref{subsec:inversions}).
Alternatively, it can be directly defined by shifts, \ie $(\sigma,\tau)\leq (\sigma',\tau')$ if there exists a sequence of admissible path shifts that turns $(\sigma,\tau)$ into $(\sigma',\tau')$.
The following proof makes both these interpretations clear.

\begin{proposition} 
\label{prop:shift lattice}
The shift lattices are indeed lattices, and are isomorphic to products of chains.
\begin{align*}
T_v^\LA \cong \prod_{\rho \in [n]} [0,l_v(\rho)] \times \prod_{\rho \in [n]} [0,r_v(\rho)] \ , 
\quad \text{ and } \quad
T_v^\SU \cong \prod_{\rho \in [n]} [0,r_v(\rho)] \times \prod_{\rho \in [n]} [0,l_v(\rho)] .
\end{align*}
where $[0,k]$ is the chain lattice $0<1<...<k$, for $k\geq 0$.
\end{proposition}

\begin{proof}
We define $L_\rho^m$ (\resp $R_\rho^m$) to be a left (right) $m$-shift of $\rho$ for $m>0$, and the identity for $m=0$.
By the commutativity of $m$-shift operators (\cref{lem:m-shifts commute}), and the existence of unique heights for each element (\cref{prop:maximal m-shift formulae}), every element of $T_v^\LA$ admits a unique shift description $(L^{l_n}_{n}...L^{l_1}_{1}\sigma,
R^{r_n}_{n}\cdots R^{r_{1}}_{1}\tau)$, where $0\leq l_\rho\leq l_v(\rho)$ and $0\leq r_\rho\leq r_v(\rho)$.
Thus, we identify each element with the pair of tuples $(l_1,\ldots,l_n)\times (r_1,\ldots,r_n)$.
As shifts strictly increase the number of crossings, and inverse shifts strictly decrease the number of crossings (\cref{lem:IJ-closed-under-shifts}), this identification is an isomorphism of lattices.
\end{proof}

Note that the maximal element of $T_v$ (for either diagonal) is given by shifting each element of $(\sigma,\tau)$ by its maximal shift.
...
{\small
\begin{align*}
	(l_1,\ldots,l_n,r_1,\ldots,r_n)\land (l'_1,\ldots,l'_n,r'_1,\ldots,r'_n) &= (\min\{l_1,1'_1\},\ldots,\min\{l_n,l'_n\},\min\{r_1,r'_1\},\ldots,\min\{r_n,r'_n\})
\end{align*}
}
...
\begin{remark}
As a consequence of \cref{prop:shift lattice}, the facets of the operadic diagonal are a disjoint union of lattices.
However, any lattice $L$ on permutations (such as the weak order), will induce a lattice on the facets.
For every $v \in L$, we can substitute the lattice $T_v^\LA$ (or $T_v^\SU$) into the permutation $v$.
In particular, every element which was covered by $v$ is now covered by the minimal element of $T_v^\LA$, and every element which was covering $v$ now covers the maximal element of $T_v^\LA$.
...
\begin{align*}
    2(n+1)^{n-2} = \sum_{v \in \mathbb{S}_n} \prod_{\rho \in [n]} (l_v(\rho)+1)(r_v(\rho)+1)
\end{align*}
...
\begin{align*}
	n \binom{n-1}{k_1} (n-k_1)^{k_1-1} (n-k_2)^{k_2-1} = \sum_{v \in \mathbb{S}_n^{k_1}} \prod_{\rho \in [n]} (l_v(\rho)+1)(r_v(\rho)+1)
\end{align*}
\end{corollary}
\begin{proof}
...
Given a matrix $A$ whose only non-zero entries are the elements $[n]$, let $\sigma_i(A)$ denote the non-zero entries of the $i$\ordinal{} column, and $\tau_j(A)$ the non-zero entries of the $(l-j+1)$\ordinalst{} row, see the labelling in \cref{fig:step-matrix}.
...
Recall from \cref{def:movable-subsets}, that a right $\SU$ $1$-shift $R_M$ is \defn{block-admissible} if $\min \sigma_i \notin M$ and $\min M > \max \sigma_{i+1}$ (\resp $\min M > \max \sigma_{i-1}$), and that admissible sequence of right shifts proceed in increasing order \cref{def:SU-admissible}.
...
\begin{example}\label{ex:matrix shifts}
...
We define an analogous encoding the permutahedron as a subdivision of the cube, and transfer the cubical formulae to the $\LA$ diagonal via isomorphism.
...
Let $\divcube{0}$ be the $0$-cube (a point), trivially subdivided by the sole element $1$ of $\Perm[1]$.
Then, assuming we have constructed $\divcube{n-1}$, we construct $\divcube{n}$ by weaving the element $n$ through the ordered partition defining every face $\sigma = \sigma_1|...|\sigma_k \in \Perm[n]$, using an interval description.
For $0<j<k$ we define
$n_j \eqdef \card{\sigma_{k-j+1}\cup...\cup \sigma_k}$, $I_j \eqdef [1 - \frac{1}{2^{n_j}}, 1 - \frac{1}{2^{n_{j+1}}}]$, and boundary cases $I_0\eqdef [0,1 - \frac{1}{2^{n_1}}]$, $I_k\eqdef [1 - \frac{1}{2^{n_k}},1]$.
This defines a partition of the interval $[0,1]$, which we denote $I \eqdef I_0 \cup ... \cup I_k$.
The weaving of $n$ through $\sigma$ is then defined as follows,
\begin{align} \label{eq:SU cube inductive def}
	\sigma \times I \mapsto 
	\begin{cases}
		\sigma_1|...|\sigma_k| n, & \text{if } I = [0, 0], \\
		\sigma_1|...|\sigma_j \cup n|...|\sigma_k, & \text{if } I = I_j, \\
		\sigma_1|...|\sigma_j|n|\sigma_{j+1}|...|\sigma_k, & \text{if } I = I_j \cap I_{j+1}, \\
		n|\sigma_1|...|\sigma_k, & \text{if } I = [1, 1].
	\end{cases}
\end{align}
\end{construction}
...
\begin{figure}
	...
	\end{center}
	\caption{Cubic realization of the permutahedra~$\Perm[2]$, $\Perm[3]$ and $\Perm[4]$ from \cref{constr:cubicPermutahedron1}.}
	\label{fig:cubicPermutahedron1}
\end{figure}

\begin{definition} 
\label{def:Subdivisions}
Let $F$ be an $\ell$-face of the permutahedron $\Perm$.
For $k\leq \ell$, a \defn{$k$-subdivision cube of $F$} is union of $k$-faces of $F$ whose underlying set is a $k$-cube.
\end{definition}
...
A $k$-subdivision cube has unique maximal and minimal vertices under the weak order.
\end{lemma}
...
The \defn{sandglass} of $F$ at $v$ is the set $\subdivpairsev(F)$ of all pairs $(C^v,C_v)$ of $(k_1,k_2)$-subdivision cubes of $F$ such that
\begin{enumerate}
	\item $C^v$ has maximal vertex $v$,
	\item $C_v$ has minimal vertex $v$, and
	\item $k_1+k_2 = \dim F$.
\end{enumerate}
If $F$ is the maximal face of $\Perm$, then we denote the sandglass of $F$ simply by $\subdivpairsv$.
The \defn{maximal pair} of the sandglass $\maxsubdivpairsev(F)$ (\resp $\maxsubdivpairsv$) is the unique maximal pair of $\subdivpairsev(F)$ (\resp $\subdivpairsev$) with respect to inclusion.
\end{definition}
...
We clarify the previous section with examples that are visually verifiable from the diagram of $\divcube{3}$ of \cref{fig:sandglass1}.
The sets of faces 
\begin{align*}
	\{1234\},\quad \{1|234\},\quad \{1|234, 14|23\},\quad \{1|3|24,1|34|2\}
\end{align*}
are all subdivision cubes of the face $F = \Perm[4]$.
Note, that $\{1|3|24,1|34|2\}$ is also a $1$-subdivision cube of the face $1|234$.
In contrast, the sets of faces
\begin{align*}
	\{134|2, 14|23\},\quad \{1|234, 134|2, 14|23\}, \quad \{1|2|34,1|23|4\}
\end{align*}
are not subdivision cubes of $F$.
For $v = \blue{\mathbf{4|3|1|2}}$, the only $1$-subdivision cube with minimal vertex $v$ is $4|3|12$, and the three $2$-subdivision cubes with maximal vertex $v$ are
\begin{align*}
	\{134|2\}, \quad \{13|24, 134|2\}, \quad \{ 1|234, 13|24, 14|23, 134|2\}.
\end{align*}
This defines the sandglass $\subdivpairsv$, whose maximal pair of subvision cubes is $\maxsubdivpairsv=\{ 1|234, 13|24, 14|23, 134|2\} \times \{4|3|12\}$.
Let us observe that the $\SCP$ corresponding to $v$ is $(\sigma,\tau) = (\blue{\mathbf{134|2}},\blue{\mathbf{4|3|12}} )$.
...
\begin{figure}
	...
	\end{center}
	\caption{The sandglass~$S_v(F)$ of $F = \Perm[4]$ at~$v = \blue{\mathbf{4|3|1|2}}$.}
	\label{fig:sandglass1}
\end{figure}
%
\end{example}
...
We now specialise the results of the prior section to subdivision cubes of the maximal face of $\divcube{n}$, and we recover the $\SU$ shift lattice structure identified in \cref{sec:Shift-lattice}.
...
The set of pairs of subdivision cubes in the sandglass $\subdivpairsv$ of $v$ satisfies the following properties
\begin{enumerate}
	\item The strong complementary pair $(\sigma,\tau)$ corresponding to $v$ is in $\subdivpairsv$,
	\item All pairs in $\subdivpairsv$ have the same pair of dimensions $(k_1,k_2)$,
	\item For all pairs $(C^v,C_v) \in \subdivpairsv$, the unique maximal $k_1$-face of $C^v$ is $\sigma$, and the unique minimal $k_2$-face of $C_v$ is $\tau$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
\begin{lemma}
Let $(\sigma,\tau)$ be a strong complementary pair. 
...
For $v$ be a vertex of $\Perm$ with associated $\SCP$ $(\sigma,\tau)$, we have
\begin{align*}
...
Then, assuming we have constructed $\divcube{n-1}^{\LA}$, we construct $\divcube{n}^{\LA}$ by weaving $1$ through every face of $\sigma = \sigma_1|...|\sigma_k \in \Perm[n]$, using an interval description.
For $0<j<k$ we define
$n_j \eqdef \card{\sigma_{k-j+1}\cup...\cup \sigma_k}$, $I_j \eqdef [1 - \frac{1}{2^{n_j}}, 1 - \frac{1}{2^{n_{j+1}}}]$, and boundary cases $I_0\eqdef [0,1 - \frac{1}{2^{n_1}}]$, $I_k\eqdef [1 - \frac{1}{2^{n_k}},1]$.
This defines a partition of the interval $[0,1]$, which we denote $I \eqdef I_0 \cup ... \cup I_k$.
...
\label{eq:In LA inductive def}
	\sigma \times I \mapsto 
	\begin{cases}
		\sigma'_1|...|\sigma'_k| 1, & \text{if } I = [0, 0], \\
		\sigma'_1|...|\sigma'_j \cup 1|...|\sigma'_k, & \text{if } I = I_j, \\
		\sigma'_1|...|\sigma'_j|1|\sigma'_{j+1}|...|\sigma'_k, & \text{if } I = I_j \cap I_{j+1}, \\
		1|\sigma'_1|...|\sigma'_k, & \text{if } I = [1, 1],
	\end{cases}
\end{align}
where~$\sigma'_i = \{x+1:x\in \sigma_i\}$.
\end{construction}

\cref{fig:cubicPermutahedron2} illustrates the first few generations of $\divcube{n}^{\LA}$, indicating in bold, the embedding $\divcube{n-1}^{\LA}\hookrightarrow \divcube{n}^{\LA}$ that occurs when $I = [0, 0]$.
...
\begin{figure}
	...
	\end{center}
	\caption{Cubic realization of the permutahedra~$\Perm[2]$, $\Perm[3]$ and $\Perm[4]$ from \cref{constr:cubicPermutahedron2}.}
	\label{fig:cubicPermutahedron2}
\end{figure}

The appropriate base definitions for the $\LA$ diagonal of $k$-subdivision cubes (\ref{def:Subdivisions}), sandglasses of subdivision cubes ${\subdivpairsv}$ (\ref{def:sandglass}), and maximal pairs of subdivision cubes $\maxsubdivpairsv$ are all the same as in the $\SU$ case.
...
Let $r:P\to P$ be the face poset isomorphism of the permutahedra which acts on $A_1|...|A_k$, by replacing each block $A_j$ by the block $r(A_j)\eqdef\set{n-i+1}{i \in A_j}$.
...
\begin{example}
Applying the isomorphism $r$ to \cref{ex:subdivision cubes} yields the illustration of \cref{fig:sandglass2}.
...
\begin{figure}
	...
	\caption{The isomorphism~$r$ applied to \cref{fig:sandglass1}.}
	\label{fig:sandglass2}
\end{figure}
\end{example}

As the example suggests, we have the following cubical description of the $\LA$ diagonal.

\begin{proposition}
\label{prop:LA-cubical}
For $v$ be a vertex of $\Perm$ with associated $\SCP$ $(\sigma,\tau)$, we have
\begin{align*}
	\maxsubdivpairsv = \bigcup_{\mathbf{M},\mathbf{N}} L_\mathbf{M}(\sigma) \times R_{\mathbf{N}}(\tau)
\end{align*}
where the union is taken over all the admissible sequences of $\LA$ shifts $\mathbf{M},\mathbf{N}$.
Thus, the facets of the $\LA$ diagonal are given by 
\begin{align*}
...
\end{tikzcd}
\end{center}
where the vertical arrows are the defining bijection between SCPs and permutations (\cref{def:strong-complementary-pairs}).
\end{proof}
...
In this third part, we derive some higher algebaic consequences of the results obtained in \cref{part:diagonalsPermutahedra}.
...
\begin{definition}
\label{def:valid}
Let $\sigma=\sigma_1|...|\sigma_k$ be an ordered partition of $[n]$. 
We say that $\sigma$ is \defn{valid} for $t \in \PT_n$, if the subsets of internal edges $\{\sigma_1, \sigma_1 \cup \sigma_2,...,\bigcup_{i \in [k]} \sigma_i\}$ form a nesting of $t$.
...
\begin{example}\label{ex:Validity example}
Consider the following planar tree with $5$ internal edges.
We label each internal edge in infix order by labelling the vertex directly below it.
\begin{center}
\begin{tikzpicture}[every tree node/.style={draw, very thick, circle, inner sep=0.08cm}, level distance=0.9cm,sibling distance=0.3cm, edge from parent/.style={very thick, draw}]
\Tree 
[.\node{0};
    [.\node{1};]
    [.\node{2};
...
\end{tikzpicture}
\end{center}
For this tree, the ordered partitions,
\begin{itemize}
    \item $12|3|45, 2|35|14, 25|13|4, 25|4|13, 3|12|45$ are all valid. 
    \item $12|45|3, 35|1|24$ are both invalid. 
\end{itemize}
Consequently,
\begin{itemize}
    \item $12|3|45 \times 2|35|14 \in \LAD$ and $12|3|45 \times 25|13|4 \in \SUD$ are both valid.
    \item $\mathbf{\red{12|45|3}} \times 25|4|13 \in \LAD$ and $3|12|45 \times \mathbf{\red{35|1|24}} \in \SUD$ are both invalid.
\end{itemize}
...
For every $n\geq 4$, there is a tree $t \in \PT_n$ such that the number of facets of the $\LA$ and $\SU$ diagonals are distinct. 
\end{proposition}
...
This was checked via code and the first few counterexamples are displayed in \cref{ex:SU-LA-operahedra} below.
\end{proof}

\begin{example}
\label{ex:SU-LA-operahedra}
We present in \cref{fig:trees} the $24$ planar trees with $5$ internal edges for which ${\card{\LAD_t} \neq \card{\SUD_t}}$.
Note that $\card{\LAD_t},\card{\SUD_t} \leq \card{\LAD_5}=\card{\SUD_5}=432 $ (\cref{table:enumerationDiagonalPermutahedra1}), the number of facets in the permutahedron diagonals.
We display each tree $t$ with the counts $(\card{\LAD_t},\card{\SUD_t})$ above it, and arrange them symmetrically to help emphasize notable missing elements.

\begin{figure}[p]
	\begin{tabular}{cccccc}
		\imagetop{\begin{tikzpicture}[every tree node/.style={draw, very thick, circle, inner sep=0.08cm}, level distance=0.7cm,sibling distance=0.3cm, edge from parent/.style={very thick, draw}]
		\Tree 
		[.\node{};
		    [.\node{};
		        [.\node{};]
		...
		\end{tikzpicture}}
	\end{tabular}
	\caption{The $24$ planar trees with $5$ internal edges for which $\card{\LAD_t} \neq \card{\SUD_t}$.}
	\label{fig:trees}
\end{figure}
\end{example}

We note that one sufficient condition for a tree $t$ to satisfy $\card{\LAD_t} = \card{\SUD_t}$ is, $N$ is a nesting of $t$ to be a nesting if, and only if, $rN$ is a nesting of $t$.
This includes trees such as the linear tree and

\begin{center}
\begin{tikzpicture}[every tree node/.style={draw, very thick, circle, inner sep=0.08cm}, level distance=0.7cm,sibling distance=0.3cm, edge from parent/.style={very thick, draw}]
\Tree 
[.\node{};
[.\node{};
    [.\node{};]
...
The lack of symmetry in the trees above and $(\card{\LAD_t},\card{\SUD_t})$ arises from the lack of symmetry inherent in the infix order, and in how the $\LA$ and $\SU$ diagonal treat maximal and minimal elements.
It is possible to resolve this breakdown in symmetry by using alternate labellings of trees.

\begin{remark}
Observe that $\sigma$ is a valid nesting for $t$ if, and only if $r\sigma$ is a valid nesting for $rt$, where $rt$ is the relabelling of $t$ with minimal edge label sent to maximal and so on.
