...
\Kurt{Can extend the lattice to all elements by pairing with permutahedra lattice in natural fashion. Probably a term for this type construction.
Surely there is a result that says if you substitute a bounded lattice into a point of another bounded lattice, that there is a natural lattice structure on the composite?
}
...
Then,
\begin{align*}
    (R^{r_n}_{n}...R^{r_{1}}_{1}\sigma)_*= \rightshift{n} \rightshift{n-1}...\rightshift{1} \sigma^*
\end{align*}
where the shift operator $\rightshift{i}$ shifts the element $i$ past all elements to the right which are smaller than $i$. 
Similarly,
\begin{align*}
    (L^{l_n}_{n}...L^{l_{1}}_{1}\tau)^* =\leftshift{n} \leftshift{n-1}...\leftshift{1} \tau_*
\end{align*}
where the shift operator $\leftshift{i}$ shifts $i$ past all elements to the left which are smaller than $i$. 
\end{lemma}
\begin{proof}
...
\begin{align*}
    \overrightarrow{v}:= \rightshiftk{n}{k_{1}}... \rightshiftk{n-1}{k_{n-1}}\rightshiftk{n}{k_{n}}v
\end{align*}
where the $\rightshiftk{i}{k_{i}}$ shifts element $i$ past $k_i$ elements to the right which are smaller than $i$. 
Dually, a $k$-subdivision cube corresponds to an interval $[{v},\overleftarrow{v}]$ of $P_n$, where $v$ is the unique minimal vertex, and
\begin{align*}
    \overleftarrow{v} :=\leftshiftk{n}{k_{n}}\leftshiftk{n-1}{k_{n-1}}...\leftshiftk{n}{k_{1}}v
\end{align*}
where $\leftshiftk{i}{k_{i}}$ shifts element $i$ past $k_i$ elements to the right which are smaller than $i$.
\end{lemma}
\begin{proof}
...
$[\overrightarrow{s_{i+1}}^{k_{i+1}}... \rightshiftk{n-1}{k_{n-1}}\rightshiftk{n}{k_{n}}v, v]$ is a subdivision cube.
...
\begin{align*}
    (R^{r_n}_{n}...R^{r_{1}}_{1}\sigma)_*&= \rightshift{n} \rightshift{n-1}...\rightshift{1} \sigma^*, \quad
    (L^{l_n}_{n}...L^{l_{1}}_{1}\tau)^* =\leftshift{n} \leftshift{n-1}...\leftshift{1} \tau_*
\end{align*}
where the shift operator $\rightshift{i}$, $\leftshift{i}$ shifts the element $i$ past all elements to the right/left which are smaller than $i$. 
...
\begin{align*}
    {e_v}_*=[\rightshift{n} \rightshift{n-1}...\rightshift{1}v, v] \times [v, \leftshift{n} \leftshift{n-1}...\leftshift{1}v]
\end{align*}
\end{lemma}
\begin{proof}
...
\begin{align*}
    \overrightarrow{v}:= \rightshiftk{n}{k_{1}}... \rightshiftk{n-1}{k_{n-1}}\rightshiftk{n}{k_{n}}v, \quad \overleftarrow{v} :=\leftshiftk{n}{k_{n}}\leftshiftk{n-1}{k_{n-1}}...\leftshiftk{n}{k_{1}}v
\end{align*}
...
To see this one can observe from the inductive description of $I_n$ (\cref{eq:In inductive def}) that the path from the minimal vertex of $\rightshift{n} \rightshift{n-1}...\rightshift{1}v$ to any other $\rightshiftk{n}{k_{1}}... \rightshiftk{n-1}{k_{n-1}}\rightshiftk{n}{k_{n}}v$ will remain within the $k_1$-cube $[\rightshift{n} \rightshift{n-1}...\rightshift{1}v,v]$, and similarly for the $k_2$.
\end{proof}
...
From here, there is no need to alter the definition of $k$-subdivision cubes, or ${e_v}$, or ${e_v}_*$.
...
\begin{align*}
    t(r\times r) (e_v^*) = t(r\times r) (\bigcup_{\mathbf{M},\mathbf{N}} R_\mathbf{M}^{\SU}(\sigma) \times L_{\mathbf{N}}^{\SU}(\tau)) = 
...
Let $v$ be a vertex of $P_n$, then
\begin{align*}
    e_v^* = \bigcup_{\mathbf{M},\mathbf{N}} L_\mathbf{M}^{\LA}(\sigma) \times R_{\mathbf{N}}^{\LA}(\tau)
\end{align*}
