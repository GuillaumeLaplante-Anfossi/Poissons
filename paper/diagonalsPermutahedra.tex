...
Then, for any $i \in I,j \in J$, we have $I\ssm i$ dominates $J\ssm j$ in $\sigma$ if and only if either
\begin{enumerate}
...
\vincent{Please be consistent: use either the abbreviation SCP everywhere, or the words strong complementary partition everywhere.}
\end{definition}
...
For any strong complementary pair $(\sigma,\tau)$, we have
\begin{enumerate}
...
\end{enumerate}
As a consequence, all strong complementary pairs are in both $\LAD$ and $\SUD$, and constitute precisely the set of facets $(\sigma,\tau)$ of these diagonals such that $\max_{\b{v}}(\sigma) = \min_{\b{v}}(\tau)$.
\end{proposition}
\begin{proof}
...
Let $\sigma$ denote either one of the two ordered partitions of $[n]$ in a ordered \mbox{$(2,n)$-partition} tree, and let $M \subsetneq \sigma_i$.
...
\end{enumerate}
In particular, for a pair of ordered partitions $(R_{\mathbf{M}}(\sigma),L_{\mathbf{N}}(\tau))$ obtained via a block-admissible sequence of $1$-shifts from a strong complementary pair~$(\sigma,\tau)$, we have
\begin{align}
\label{eq:max-1}
...
Let $(\sigma,\tau)$ be a pair of ordered partitions which has thus far been only generated by $k$ shifts, for $k<m$. 
We wish to show that a right path admissible $m$-shift $R_\rho$ can be decomposed using $1$-shifts.
As $R_\rho$ is path admissible, we know that $\rho > \max \sigma_{i,i+m}$, and we use this fact to show that 
$\rho>\max \sigma_{i,i+m}\geq \max \sigma_{i,i+1}, \max R_\rho(\sigma)_{i+1,i+m}$, which implies that we can decompose it as a path admissible $1$-shift, followed by a subsequent path admissible $(m-1)$-shift which yields the result by induction.
We now prove the required inequality.
\begin{center}
\begin{tikzpicture}[scale=0.7]
\node[anchor=east] (1) at (0,0) {$\sigma_i$};
\coordinate (2) at (1,0);
\node[anchor=south] (3) at (1,1) {$\sigma_{i+1}$};
\node[anchor=west] (4) at (2,0) {$\sigma_{i+m}$}; 
...
Let $\alpha = \sigma_{i,i+1}\setminus \sigma_{i+1,i+m}, \beta = \sigma_{i,i+1}\setminus \sigma_{i,i+m}$, and $\gamma = \sigma_{i,i+m}\setminus \sigma_{i,i+1}$, as illustrated above. 
Then $\sigma_{i,i+m} = \alpha\gamma$, $\sigma_{i,i+1} = \alpha \beta$, and $\sigma_{i+1,i+m}=\beta^{-1} \gamma$, where $\beta^{-1}$ is $\beta$ traversed in the opposite direction.
...
Then we must have that $m:=\max \beta> \max \alpha, \max \gamma$.
However, $m$ cannot be the maximum of both $\sigma_{i,i+1}$ and $\sigma_{i+1,i+m}$.
This follows as $1$ shifts conserve the maximal element of paths between adjacent blocks, and its direction (\cref{cor:SU-shift-preserves-max}).
By induction, $(\sigma,\tau)$ can be generated by $1$-shifts, so $m$ cannot be the maxima of both $\beta$ and $\beta^{-1}$, as it would be traversed in opposite directions.
So $\max \sigma_{i,i+m}\geq \max\sigma_{i,i+1}, \max \sigma_{i+1,i+m}$, and applying \cref{cor:SU-shift-preserves-max} again yields $\max \sigma_{i,i+m}\geq \max R_\rho(\sigma)_{i+1,i+m}$ as required.

\end{proof}

Thus, as the prior proof indicates, an analogue of \cref{cor:SU-1-shift-preserves-max} also holds for $m$-shifts. 
...
\begin{align*}
1 \leq i_1 < \dots < i_j \leq \ell < i_{j+1} <\dots< i_p \leq k-1
\end{align*}
for some $j$. 
If $i_j < \ell$, then we have $R_{\rho}R_{\mathbf{M}}(\sigma) = R_{M_{p}}\dots R_{\{\rho\}}R_{M_{j}}\dots R_{M_{1}}(\sigma)$, and we are done.
Otherwise, if $i_j = \ell$, we set $M'_{j} \eqdef M_{j} \cup \{\rho \}$. 
It is clear that ${R_{\rho}R_{\mathbf{M}}(\sigma) = R_{M_{p}}\dots R_{M'_{j}}\dots R_{M_{1}}(\sigma)}$, however, we need to check that $R_{M'_{j}}$ is block-admissible, \ie that $\min M_{j}' > \max (R_{M_{j-1}}\dots R_{M_{1}}(\sigma))_{i_j+1}$.
...
Since by definition block-admissible shift operators do not move the minimal element of a block, we have $\rho > \min R_{\mathbf{M}}(\sigma)_{i_j}= \min (R_{M_{j-1}}\dots R_{M_{1}}(\sigma))_{i_j}$.
...
\begin{remark}
Our definition of an inversion of an ordered partition specializes to the classical definition of an inversion of permutations.
\end{remark}
...
Thus, all maxima of paths in $(R_{\rho}(\sigma),\tau)$ are traversed from right to left, and hence by \cref{thm:facet-ordering}, we have $(R_{\rho}(\sigma),\tau)$ is in the geometric $\SU$ diagonal, which proves Point (1).
...
More precisely, we show that each $\SCP$ is the minimal element of a lattice isomorphic to a product of chains, where the partial order is given by shifts, or equivalently inversions.
...
As $\sigma$ is a part of a $\SCP$, we have $\max \sigma_{k,k+1} = \max \sigma_{k+1}$ for all $k\geq 1$. 
...
For each permutation $v$ of $[n]$, let $T_v^\LA$ (\resp $T_v^\SU$) be the set of ordered \mbox{$(2,n)$-parti}\-tion trees, resulting from applying all $\LA$ ($\SU$) shifts to the SCP $(\sigma,\tau)$.
...
\begin{align*}
T_v^\LA \cong \prod_{\rho \in [n]} [0,\ell_v(\rho)] \times \prod_{\rho \in [n]} [0,r_v(\rho)]
\quad \text{ and } \quad
T_v^\SU \cong \prod_{\rho \in [n]} [0,r_v(\rho)] \times \prod_{\rho \in [n]} [0,\ell_v(\rho)] .
\end{align*}
...
By the commutativity of $m$-shift operators (\cref{lem:m-shifts commute}), and the existence of unique heights for each element (\cref{prop:maximal m-shift formulae}), every element of $T_v^\LA$ admits a unique shift description $(L^{\ell_n}_{n} \dots L^{\ell_1}_{1}\sigma,
R^{r_n}_{n}\cdots R^{r_{1}}_{1}\tau)$, where $0\leq \ell_\rho\leq \ell_v(\rho)$ and $0\leq r_\rho\leq r_v(\rho)$.
Thus, we identify each element with the pair of tuples $(\ell_1,\ldots,\ell_n)\times (r_1,\ldots,r_n)$.
...
Then, assuming we have constructed $\divcube{n-1}$, we construct $\divcube{n}$ by weaving the element~$n$ through the ordered partition defining every face $\sigma = \sigma_1| \cdots |\sigma_k \in \Perm[n]$, using an interval description.
For $0<j<k$ we define
...
\begin{align} \label{eq:SU cube inductive def}
	\sigma \times I \mapsto 
	\begin{cases}
		\sigma_1| \cdots |\sigma_k| n, & \text{if } I = [0, 0], \\
		\sigma_1| \cdots |\sigma_j \cup n| \cdots |\sigma_k, & \text{if } I = I_j, \\
		\sigma_1| \cdots |\sigma_j|n|\sigma_{j+1}| \cdots |\sigma_k, & \text{if } I = I_j \cap I_{j+1}, \\
		n|\sigma_1| \cdots |\sigma_k, & \text{if } I = [1, 1].
	\end{cases}
\end{align}
\end{construction}
