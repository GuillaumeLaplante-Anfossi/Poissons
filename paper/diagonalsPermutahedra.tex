...
\begin{definition}
For an ordered partition~$\orient{\pi} \eqdef \orient{\pi}_1 | \dots | \orient{\pi}_k$ of~$[n]$, we denote by~$D_{\orient{\pi}}$ the directed graph on~$[n]$ with an edge~$\max(\orient{\pi}_j) \to \min(\orient{\pi}_{j+1})$ for each~$j \in [k-1]$ and a cycle~${x_1 \to \dots \to x_p \to x_1}$ for each part~$\orient{\pi}_j = \{x_1 < \dots < x_p\}$.
For an ordered $(\ell,n)$-partition forest~$\orient{\b{F}} \eqdef (\orient{F_1}, \dots, \orient{F_\ell})$, we denote by~$D_{\orient{\b{F}}}$ the superposition of the directed graphs~$D_{\orient{F}_i}$ for~$i \in [\ell]$, where an edge arising from~$D_{\orient{F}_i}$ is labeled by~$i$.
\end{definition}

\begin{proposition}
An ordered $(\ell,n)$-partition forest~$\orient{\b{F}}$ belongs to the upper set~$\orientedForestPoset(\b{a})$ corresponding to the faces of~$\multiBraidArrangement(\b{a})$ if and only if $\sum_{e \in c} A_{i(e), s(e), t(e)} \ge 0$ for any (simple) oriented cycle in~$D_{\orient{\b{F}}}$, where each edge~$e \in c$ has source~$s(e)$, target~$t(e)$ and label~$i(e)$.
\end{proposition}
...
\begin{remark}
Note that we made some arbitrary choices here by choosing the edge from~$\max(\orient{\pi}_j)$ to~$\min(\orient{\pi}_{j+1})$ between two consecutive parts~$\orient{\pi}_j$ and~$\orient{\pi}_{j+1}$ and a cycle inside each part~$\orient{\pi}_j$ (while we said that the order in each part is irrelevant).
We could instead have considered all edges connecting two elements of two consecutive parts, or two elements inside the same part.
...
We have seen in \cref{subsec:multiBraidArrangement} that facets of any diagonal of the permutahedron are in bijection with $(2,n)$-trees, that is, a pair of unordered partitions of $[n]$ whose intersection graph is a tree.
These pairs of partitions were first studied and called essential complementary partitions in \cite{chen1969computer,chen1971tables,kajitani1982number}, as such we will denote the set of all $(2,n)$-trees by $\EC$.\Kurt{A historic nod, but this way don't need to define.}
In this section, we prove that particular orderings of $\EC$ recover the four operadic diagonals of \cref{prop:unique-operad}.\Guillaume{Simplify and unify exposition with Section 1}
\Kurt{A bit better but feel free to doctor further.}

\begin{theorem}
\label{thm:facets}
There exists a bijective ordering function $o:\EC\to \OP$, for each of the operadic diagonals.
For any $(\sigma,\tau) \in \EC$, we order any two vertices on the same side, i.e. $a,b\in \sigma$, or $a,b\in \tau$, by directing the unique path $\gamma$ connecting them.
If the path points $a\to b$, then $a<b$.
Each diagonal orients every path by extending the orientation of a particular path element,
\begin{enumerate}
    \item $\min \gamma$ oriented left to right (right to left) yields $o(\sigma,\tau) \in \LAD$ (resp. $o(\sigma,\tau) \in (\LAD)^{op})$ )
    \item $\max \gamma$ oriented right to left (left to right) yields $o(\sigma,\tau) \in \SUD$ (resp. $o(\sigma,\tau) \in (\SUD)^{op})$ ).
\end{enumerate}
\end{theorem}

\begin{proof}
In this section, we first prove this theorem for $\LAD$, via the following lemmas,
\begin{itemize}
    \item The function $u:\LAD\to \EC$, which forgets the order of the blocks, is well defined (\cref{l:u-well-defined}).
    \item The ordering function $o:\EC \to \LAD$ is well defined. In particular, 
    \begin{itemize}
        \item it totally orders the vertices on each side (\cref{l:o-provides-total-order}), and
        \item it produces elements of $\LAD$ (\cref{l:o-well-defined}).
    \end{itemize}
    \item The functions $u,o$ are mutual inverses (\cref{l:o and u inverses}).
\end{itemize}
Then, we transfer the result to the other operadic diagonals via isomorphism (\cref{l:Non LA ordering functions}).
\end{proof}
...
We will show that this ordering produces elements of the $\LAD$ diagonal, as each oriented path $\gamma_{ab}$ will correspond to satisfying a specific $(I,J) \in \LA$.
We first present a clarifying example.

\begin{example}
\label{ex:ECbijection}
We display the ordering (right) of the $(2,7)$-tree $\{15,234,6,7\} \times \{13,2,46,57\}$ (left).
\begin{center}
...
\end{tikzpicture}
\end{center}
Here are two examples of paths providing order information, 
\begin{align*}
	7 <_L 234 \text{ as } 7 \xrightarrow{7} 57 \xrightarrow{5} 15 \xrightarrow{1=\min(I\cup J)} 13 \xrightarrow{3} 234 \ \text{corresponds to } (I,J)=(\{1,7\},\{3,5\}), \\
	57 <_R 46 \text{ as } 57 \xrightarrow{5} 15 \xrightarrow{1=\min(I\cup J)} 13 \xrightarrow{3} 234 \xrightarrow{4} 46 \ \text{corresponds to } (I,J)=(\{1,4\},\{3,5\}).
\end{align*}
\end{example}
