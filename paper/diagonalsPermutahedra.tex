...
\vincent{This definition already appeared, we nee to slightly modify this.}
...
The goal of this section is to prove that the $\LA$ and $\SU$ diagonals are the only two operadic diagonals which induce the weak Bruhat order on the vertices of the permutahedra (\cref{prop:unique-operad}). 
...
\begin{definition}
For any $\omega \in \N$, the \defn{weight $\omega$ permutahedron} $\Perm[n,\omega] \subset \R^n$ is the convex hull  of the points ~$\sum_{i \in [n]} (\omega + i-1) \, \b{e}_{\sigma(i)}$  for all permutations $\sigma$ of $[n]$. 
\vincent{I believe it would be nicer to just translate...}
\end{definition}
