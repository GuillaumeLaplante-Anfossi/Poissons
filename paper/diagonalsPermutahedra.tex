...
\newcommand{\resp}{\textit{resp.}~} % id est
...
This yields in particular nice formulas for the number of regions and bounded regions of these arrangements in terms of exponentials of Fuss-Catalan numbers.
...
We prove that there are only two operadic families of diagonals on the permutahedra, and we give a bijection of their facets with planar bipartite trees and characterize their vertices as pattern-avoiding pairs of permutations.
...
\subsection*{\cref{part:multiBraidArrangements}. Combinatorics of multiple braid arrangement}

As the dual of the permutahedron~$\Perm$ is the classical braid arrangement~$\braidArrangement$, the dual of a diagonal of the permutahedron~$\Perm$ is an hyperplane arrangement made of $2$ generically translated copies of the braid arrangement~$\braidArrangement$.
...
We first observe that the flats of~$\multiBraidArrangement$ are in bijection with $(\ell,n)$-partition forests, defined as $\ell$-tuples of unordered partitions whose intersection hypergraph is a hyperforest (\cref{def:partitionForests}).
As the $(\ell,n)$-partition forests are independent of the translations of the different copies (as long as these translations are generic), we obtain by Zaslavsky's theory that the number of $k$-dimensional faces and of bounded faces of~$\multiBraidArrangement$ only depends on~$k$, $\ell$, and~$n$ (and not on the specific translation vectors choosen to construct~$\multiBraidArrangement$).
...
We observe that the faces of~$\multiBraidArrangement$ are in bijection with certain ordered $(\ell,n)$-partition forests, defined as $\ell$-tuples of ordered partitions whose underlying unordered partitions form an (unordered) $(\ell,n)$-partition forest.
...
We provide a combinatorial description of the possible orderings of a $(\ell,n)$-partition forest compatible with some given translations in terms of certain paths in the forest, and a combinatorial characterization of the ordered partition forests which appear for some given translations in terms of the circuits of a certain oriented graph.
...
We finally explain the partial order of the flat lattice in terms of rainbow forests.
\vincent{work this out...}
...
From the characteristic polynomial of~$\multiBraidArrangement$ and \cref{rem:characteristicPolynomial}, we obtain the $f$- and $b$-polynomials of~$\multiBraidArrangement$.
...
\vincent{add picture permutahedron and weak order}
\end{definition}
...
\begin{remark}
\vincent{Connection between~$\b{v}$ and~$\b{a}$.}
\end{remark}
...
See \cref{exm:intervalsWeakOrderNotDiagonals}.
...
We show that these are the only two operadic diagonals which induce the weak order (\cref{subsec:operadicProperty}), and that they are isomorphic (\cref{subsec:isos-LA-SU}). 
Using results from \cref{sec:cellularDiagonals}, we characterize the facets of these diagonals in terms of paths in $(2,n)$-partition trees (\cref{subsec:facets-operadic-diags}), and the vertices of these diagonals as pattern-avoiding pairs of permutations (\cref{subsec:vertices-operadic-diags}).
...
These pairs of partitions were first studied and called \defn{essential complementary partitions} in \cite{chen1969computer,chen1971tables,kajitani1982number}.
...
\begin{tikzpicture}[scale=.7]  
    ...
    %
\end{tikzpicture}
\raisebox{3.5em}{$\xrightarrow{rs\times rs}$}
\begin{tikzpicture}[scale=.7]  
    \node (1) at (1.5, -4) {$13$};
    \node (2) at (1.5, -3) {$24$};
    \node (3) at (1.5, -2) {$57$};
...
For each $k$, there are $\binom{2k-1}{k-1,k}(k-1)!k!$ avoided patterns.
For $k=1$ we find that both diagonals avoid $(21,12)$, so consequently the vertices are a subset of intervals the weak order.
For $k=2$, the $\LA$ diagonal avoid the patterns
\begin{align*}
    (4132,2413), (3142,2314),(2143,3214), (4123,3412),
    (2134,4213), (3124,4312)
\end{align*}
and the $\SU$ diagonal avoids the patterns,
\begin{align*}
    (1243,2431),(1342,3421)
    (2143,1432),(2341,3412),
    (3241,2413),(3142,1423),
\end{align*}
As such, for $n\geq4$, the $\LA$ and $\SU$ are strict subsets of intervals of the weak order.
\Kurt{Picture+Check patterns}
\end{example}
...
\begin{enumerate}[(a)]
    \item if the intersection of $(\sigma,\tau)$ with $I \cup J$ is of the form $$(j_1 i_1 j_2 i_2 ... j_{n-1} i_{n-1} w i_{n} w', \ i_2 j_1 i_3 j_2 ... i_{n}j_{n-1}w'') \ , $$ and $j_n$ is the leftmost element of $J\ssm \{j_1,...,j_{n-1}\}$ in $\tau$, then either we have $w = j_n$, or we can conclude with the induction hypothesis.
    \item if the intersection of $(\sigma,\tau)$ with $I \cup J$ is of the form $$(j_1 i_1 j_2 i_2 ... j_{n-1} i_{n-1} w'', \ i_2 j_1 i_3 j_2 ... i_{n-1}j_{n-2} w j_{n-1} w'') \ , $$ and $i_n$ is the leftmost $I\ssm \{i_1,...,i_{n-1}\}$ in $\sigma$, then either we have $w = i_n$, or we can conclude with the induction hypothesis.
\end{enumerate}
...
Now consider the pair $(I',J') \eqdef (I_k\ssm \{i_n\}, J_k \ssm \{j_n\})$.
...
As is clear from the definition, the intersection graph of an $\SCP$ is a $(2,n)$-partition tree.
...
\node (2) at (-1.5, -2) {$\sigma_{i+1}\ssm \rho$};
%
\node (4) at (1.5, -1) {$\tau_j$};
\node (5) at (1.5, -2) {$\tau_{k}$};
%
...
\node (3) at (-1.5, -3) {$\sigma_{i+1+m}\ssm \rho$};
%
\node (4) at (1.5, -1) {$\hphantom{.}$};
\node (5) at (1.5, -2) {$\hphantom{.}$};
%
...
\node (3) at (-1.5, -3) {$\sigma_{i+1+m}$};
%
\node (4) at (1.5, -1) {$\tau_{j-m}\ssm \rho$};
...
The permutahedra are part of a more general family of polytopes called Loday realizations of the \emph{operahedra} \cite[Def.~2.9]{LA21} which encodes the notion of homotopy operad \cite[Def.~4.11]{LA21}.\Guillaume{Image des trois mousquetaires?}
...
\begin{definition}
    An \emph{operadic diagonal} for the operahedra is a choice of diagonal $\triangle_t$ for each operahedron~$P_t$, such that $\triangle \eqdef \{\triangle_t\}$ commutes with the map $\Theta$, \ie it satisfies \cite[Prop.~4.14]{LA21}.
\end{definition}
...
Here we use the increasing order on nestings \cite[Def. 4.5]{LA21}, and observe that any choice of sequence of $\circ_i$ operations yield the same nested, since these form an operad \cite[Def.~4.7]{LA21}.
...
Note that any choice of sequence of $\circ_i^\LA$ operations yield the same isomorphism, since they form an operad \cite[Thm.~4.18]{LA21}.
...
Since $\circ_i^\LA$ and $\circ_i^\SU$ are the identity both on the interior of $\Perm[4]$ (by \cref{const:top-iso}) and on the $1$-skeleton of $\Perm[4]$ (see the proof of \cite[Prop. 7]{masudaDiagonalAssociahedra2021}), we directly obtain that $\LAD(z)=(\Psi \times \Psi)\LAD(z) \neq \SUD \Psi(z)=\SUD(z)$. 
\end{example}
...
\begin{theorem}
    ...
    However, the morphisms of operads $\LAD$ and $\SUD$ are homotopic, which implies that the two tensor products are isomorphic in the category of homotopy operads and their $\infty$-morphisms.
\end{theorem}

\begin{proof}
Il faut voir qu'il ne peut pas exister un infini-isomorphisme entre les deux?? En Effet! Si c'etait un infini-iso, ce serait l'identite en degre zero
\end{proof}
