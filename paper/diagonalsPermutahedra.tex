...
\thanks{VP was partially supported by the French grants CHARMS (ANR-19-CE40-0017) and by the French--Austrian projects PAGCAP (ANR-21-CE48-0020 \& FWF I 5788).}
...
In practice, we consider the hyperplanes~$\set{\b{x} \in \HH}{x_r - x_s = A_{i,r,s}}$ for all~$1 \le r < s \le n$ and~$i \in [\ell]$, where $A_{i,r,s} \eqdef \smash{\sum_{r \le j < s} a_{i,j}}$ for an arbitrary generic matrix~$(a_{i,j}) \in M_{\ell,n-1}(\R)$.
\end{remark}
...
Consider the partitions~$P \eqdef \big\{ \{1\}, \{2\}, \{3\} \big\}$, $Q_i \eqdef \big\{ \{i\}, [3] \ssm \{i\} \big\}$ for~$i \in [3]$, and~$R \eqdef \big\{ [3] \big\}$.
...
The \emph{subset right shift operator} moves $M_i$ one block to the right 
\begin{align*}
    R_{M_i}(\sigma) \eqdef \sigma_1|\dots|\sigma_i \ssm M_i|\sigma_{i+1} \cup M_i|\dots|\sigma_k
\end{align*}
and its brother the \emph{subset left shift operator} moves it one block to the left 
\begin{align*}
    L_{M_{i}}(\sigma) \eqdef \sigma_1|\dots|\sigma_{i-1} \cup M_{i}|\sigma_{i} \ssm M_{i}|\dots|\sigma_l \ .
\end{align*}
\end{definition}
...
\begin{definition}[Movable subsets]
    ...
    A non-empty subset $M_i \subset \sigma_i$ (resp. $N_{i}\subset \tau_{i}$) is \emph{movable} if 
    \begin{enumerate}
...
\begin{example}
Performing the subset shifts $R_7$ and $L_{5,6}$ on movable subsets of the strong complementary pair~$(\sigma,\tau)$ of \cref{ex:strong-complementary}, one obtains the pair $R_{7}(\sigma) \times L_{5,6}(\tau)$.
\begin{center}
\begin{tikzpicture}[scale=.7]  
    \node (p) at (0, 0) {$13|247|5|6 \times 3|17|4|256$};
    \node (1) at (-1.5, -1) {$13$};
...
\begin{definition}[Admissible sequences of subset shifts]
    Let $(\sigma,\tau)$ be a pair of ordered partitions. 
    For $p\geq 1$, a sequence of right subset shifts $\textbf{M} = (M_{i_1},\dots,M_{i_p}) \in [n]^{p}$ is \emph{admissible} if 
    \begin{enumerate}
        \item we have $1\leq i_1 < i_2 < \dots < i_p \leq k-1$, and
        \item each $M_{i_j}$ is a movable subset of $R_{M_{i_j-1}}\dots R_{M_{i_1}}(\sigma)_{i_j}$.
    \end{enumerate}
    We denote the resulting ordered partition by $R_\mathbf{M}(\sigma) \eqdef R_{M_{i_p}}\dots R_{M_{i_1}}(\sigma)$.
    The result of an admissible sequence of left subset shifts $L_\mathbf{N}(\tau)  \eqdef  L_{N_{i_1}}\dots L_{N_{i_q}}(\tau)$ is defined similarly via the condition $2\leq i_1 < i_2 \dots < i_q \leq k$, with $q\geq 1$.
\end{definition}

By convention, we declare the empty sequence of subset shift operators to be admissible, and to act by the identity, i.e. we have $R_{\mathbf{\emptyset}}(\sigma)  \eqdef  \sigma$ and $L_{\mathbf{\emptyset}} (\tau) \eqdef  \tau$.
...
\begin{definition}[Subset shift $\SU$ diagonal {\cite{SaneblidzeUmble04,saneblidzeComparingDiagonalsAssociahedra2022}}]
   The facets of the \emph{subset shift $\SU$ diagonal} are given by the formula
    ...
    \end{align*}
    where the unions are taken over all strong complementary partitions $(\sigma, \tau)$ of $[n]$, and on all admissible sequences of shift operators $\textbf{M},\textbf{N}$.
\end{definition}

\begin{remark}
    Observe that the left (resp. right) shift operator moves elements to the left (resp. right), but acts on the right (resp. left) ordered partition.
...
\begin{definition}[Critical elements] 
    ...
    Let $(\sigma,\tau)$ be a pair of ordered partitions.
    An element $\rho \in \sigma_i$ (resp. $\rho \in \tau_i$) is \emph{critical} if 
    \begin{enumerate}
        \item $\rho \neq \min \sigma_i$ (resp. $\rho \neq \min \tau_i$), and
        \item $\rho>\max \sigma_{i,i+1}$ (resp. $\rho > \max \tau_{i,i-1}$).
    \end{enumerate}
\end{definition}
...
Note here the two important differences between the subset shift and shift diagonals: a critical element $\rho$ is not required to satisfy $\rho > \max\sigma_{i+1}$, and the numbers indexing a sequence of shifts are not required to be increasing.
