...
Moreover, the following statement relates the image of the diagonal~$\triangle_{(P, \b{v})}$ to the intervals of the poset obtained by orienting the $1$-skeleton of~$P$ in direction~$\b{v}$.
...
We now specialize the results of the preceding Section to the standard permutahedron. 
\vincent{Define here the permutahedron}
\vincent{The next proposition should be stated for $\ell = 2$ and just be a vague remark for bigger~$\ell$.}

\begin{proposition}
The $f$-vector of the $(\ell-1)$\ordinalst{} iteration of a diagonal of the $(n-1)$-permutahedron $\triangle_{(P,\b{v})}$ is given by the opposite of the $f$-vector of the $(\ell,n)$-braid arrangement.
\end{proposition}

\begin{proposition}
A diagonal $\triangle_{(P,\b{v})}$ of the standard permutahedron~$\Perm$ does \emph{not} satisfy the magical formula.
That is, the inclusion      
\begin{equation*}
\Ima\triangle_{(P, \b{v})} \quad \subsetneq \bigcup_{\substack{F,G \text{ faces of } P \\ \max_{\b{v}}(F) \, \le \, \min_{\b{v}}(G)}} F \times G  
\end{equation*}
is strict, for any choice of generic vector $\b{v}$. 
\end{proposition}
...
The fundamental hyperplane arrangement of the permutahedron was computed in \cite[Sect.~3.1]{LA21}.
Let $\Un(n) \eqdef \{ (I,J) \ | \ I,J \subset [n], |I|=|J|>0, I \cap J = \emptyset\}$.

\begin{proposition}
    The fundamental hyperplane arrangement of the $(n-1)$-permutahedron is given by the family of hyperplanes \[ \sum_{i \in I} x_i = \sum_{j \in J} x_j \ ,  \ (I,J) \in \Un(n) \ . \] 
\end{proposition}
...
\begin{corollary}
    ...
    Any choice of diagonal of the $(n-1)$-permutahedron is given combinatorially by a choice of inequalities \[ \sum_{i \in I} x_i > \sum_{j \in J} x_j \quad \text{or} \quad \sum_{i \in I} x_i < \sum_{j \in J} x_j \]
    for all $(I,J) \in \Un(n)$. 
\end{corollary}
...
We now specialize the results of \cref{part:multiBraidArrangements} to the case~$\ell = 2$ to derive enumerative results on the diagonals of the permutahedra.
...
This section is devoted to the combinatorics of two specific diagonals of the permutahedron, the LA diagonal and SU diagonals (\cref{subsec:LAdiagonal}).
...
Recall from \cref{cor:choice-inequalities} that a diagonal of the permutahedra is given by a choice of ordering of the sets $$\Un(n)  \eqdef  \set{\{I,J\}}{I,J \subset [n], |I|= |J|, I\cap J = \emptyset} \ .$$
By \defn{ordering} of $\Un=\{\Un(n)\}_{n \geq 1}$, we mean a family of sets $\Or:=\{\Or(n)\}_{n\geq 1}$ where the elements of each $\Or(n)$ are choices of ordered pairs $(I,J)$ or $(J,I)$, for each $\{I,J\} \in \Un(n)$.

\begin{definition}
The \defn{$\LA$ and $\SU$ orderings} are defined by
\begin{itemize} 
    \item $\LA(n) \eqdef \set{(I,J)}{\{I,J\} \in \Un(n), \ \min(I\cup J) = \min I}$ and
    \item $\SU(n) \eqdef \set{(I,J)}{\{I,J\} \in \Un(n), \ \max(I\cup J) = \max J}$.
\end{itemize}
\end{definition}

\begin{definition}
    The \defn{$\LA$ diagonal} $\LAD$ (resp. \defn{$\SU$ diagonal} $\SUD$) of the $(n-1)$-permutahedron is given by any vector $\vec v \in \R^n$ satisfying  
    \[\sum_{i \in I} v_i > \sum_{j \in J} v_j  \ , \forall (I,J) \in \LA(n) \quad (\text{resp. } \SU(n)) \ . \]
\end{definition}
...
An ordering $\Or$ of $\Un$ is \defn{operadic} if for all~$\{I,J\} \in \Un$, we have that if $\forall \ (I',J')\subset(I,J)$, $\std(I',J') \in \Or$ then we have $(I,J) \in \Or$.
\end{definition}
