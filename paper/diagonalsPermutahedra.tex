...
\Kurt{Also this entire section is enormous for what it is, maybe streamline, cut or move to an appendix? }
\\\\
...
\Kurt{This feels almost tautological, but needs something. Is there a simple non-inductive way to do this?
Observatioin, interval of $P_n$ prevents interior holes, extremal edges prevents exterior holes.}
\\\\
From the inductive description of \cref{eq:In inductive def}, we know that each edge of $I_{n-1}$ is associated to (possibly multiple) shifts of one particular element $i$, i.e. edges of $P_n$.
As such, the extremal edge paths correspond to shifting each element to its final position and then never touching it again.
