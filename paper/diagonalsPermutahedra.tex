...
It is natural to wonder if the lattice structure admits an alternate characterisation in terms of inversion sets.
This is not the case, as the following example illustrates
\begin{example}\label{ex: Inversion and lattice counter example}
\Kurt{TODO: Lattice and illustration of inversion sets.}
\begin{center}
\begin{tikzpicture}[scale=.6]  
\node[anchor=east] (1) at (-1.5, -1) {$1$};
\node[anchor=east] (2) at (-1.5, -2) {$2$};
...
This example illustrates that the shift lattices is not directly related to inclusions of inversions sets.
For instance, it can be shown that the right shift of $1$ from $14|23$ to $4|123$, does not respect the facial weak order (discussed in \cref{sec:facial-weak-order}), as these ordered partitions are incomparable.
\end{example}
...
\end{align}
\end{definition}
We stress that the weak facial order recovers the weak order on permutations as should be apparent in the following example.

\begin{example}
We illustrate the Hasse diagram of the facial weak order for $\Perm[3]$.
\begin{center}
\begin{tikzpicture}
\node (1) at (-1.5, -1) {$3|12$};
...
\begin{proposition}
Let $(\sigma,\tau)\in \hour^\LA$ or $(\sigma,\tau)\in \hour^\SU$,
then $\sigma \leq v \leq \tau$ under the facial weak order.
\end{proposition}
\begin{proof}
Let $\sigma$ be a face of the permutahedra, and let $\sigma^*$ (\resp $\sigma_*)$ denote the unique maximal (minimal) vertex of a face of the permutahedra, under the weak order.
This vertex is constructed by writing out each block of $\sigma$ in decreasing (increasing) order.
It is clear that under the facial weak order
$\sigma<\sigma^*$, as repeated applications of \cref{eq:facial weak 2} shows that a block of elements is smaller than those same elements arranged in decreasing order.
Similarly $\sigma_*<\sigma$, as repeated applications of \cref{eq:facial weak 1} shows that a sequence of increasing elements is smaller than those same elements in a block. 
Then it is clear from the construction of $\hour^{\LA}$ (and $\hour^{\SU}$) (in \cref{sec:Cubical}) that for $(\sigma,\tau)\in \hour^{\LA}$ (or $\in \hour^{\SU}$), $\sigma \leq \sigma^* \leq v \leq \tau_* \leq \tau$, as every vertex in the left (right) maximal subdivision cube is smaller (greater) than $v$ under the weak order.
\end{proof}

\begin{example}
In the context of \cref{ex:subdivision cubes}, $1|234\times 4|3|12 \in \hour$, for $v=4|3|1|2$, and 
\begin{align*}
    1|234 \leq  1|4|3|2 \leq  4|3|1|2 \leq  4|3|1|2 \leq  4|3|12 \ .
\end{align*}
\end{example}

\begin{corollary}
Facets of the $\LA$ and $\SU$ diagonal correspond to intervals of the facial weak order.
\end{corollary}
