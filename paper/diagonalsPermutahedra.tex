...
However, from a homotopical point of view, the two tensor products of homotopy operads (\resp $\Ainf$-morphims) that they define are $\infty$-isomorphic (\cref{thm:infinity-iso,thm:infinity-iso-2}). 
...
\begin{tikzpicture}[yscale=-1, every node/.style={draw, very thick, circle, inner sep=1pt}, edge from parent path={[very thick, draw] (\tikzparentnode) -- (\tikzchildnode)}]
	...
	%\end{pgfonlayer}
\end{tikzpicture}
\caption{Two nestings of a tree with $5$ internal edges.}
\label{fig:nestings}
\end{figure}
...
A sufficient condition for the diagonals of a tree $t$ to have the same number of facets to satisfy that $N$ is a nesting of $t$ if and only if $rN$ is a nesting of $t$.
...
\begin{theorem}
	...
	For any pair of $\Ainf$-morphims, the two universal tensor products defined by the $\LA$ and $\SU$ diagonals are not isomorphic in the category of $\Ainf^2$-algebras and strict morphisms.
