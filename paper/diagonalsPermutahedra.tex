...
\begin{proof}
First, we prove the statement for the $\LA$ diagonal $\LAD$, then transport it to the other diagonals via the isomorphisms between them (\cref{l:Non LA ordering functions}). 
The proof for $\LAD$ consists in first checking that the ordering function $o:\EC \to \LAD$ is well defined, in particular checking that it totally orders the vertices on each side (\cref{l:o-provides-total-order}), and it produces elements of $\LAD$ (\cref{l:o-well-defined}), and second verifying that the function $u:\LAD\to \EC$, which forgets the order of the blocks defines an inverse (\cref{l:o and u inverses}).
\end{proof}
...
The key observation in proving \cref{thm:facets} is that paths between pairs of vertices in a $(2,n)$-tree $(\sigma,\tau)$ correspond to $(I,J)$ conditions defining an operadic diagonal (see \cref{s:diagonals-permutahedra}). 
...
\begin{align*}
	7 < 234 \text{ as } 7 \xrightarrow{7} 57 \xrightarrow{5} 15 \xrightarrow{1=\min(I\cup J)} 13 \xrightarrow{3} 234 \ \text{corresponds to } (I,J)=(\{1,7\},\{3,5\}), \\
	57 < 46 \text{ as } 57 \xrightarrow{5} 15 \xrightarrow{1=\min(I\cup J)} 13 \xrightarrow{3} 234 \xrightarrow{4} 46 \ \text{corresponds to } (I,J)=(\{1,4\},\{3,5\}).
\end{align*}
\end{example}
...
Since the intersection graph of a pair of ordered partitions defining a facet of $\LAD$ has $n+1$ vertices and $n$ edges, and as no vertices can be isolated it must be a tree, The function $u:\OP \to \EC$ that forgets the order in a pair of partitions is well defined.

\begin{lemma}
\label{l:o and u inverses}
...
\begin{lemma} 
\label{l:Non LA ordering functions}
The other ordering functions of \cref{const:orienting-function} are well-defined and bijective.
\end{lemma}
\begin{proof}
...
We end this Section by illustrating how the isomorphism $rs\times rs$ from \cref{thm:bijection-operadic-diagonals} relates the two results.

\begin{example}
\label{ex:theta-path-translation}
We apply $rs\times rs$ to the element of $\LAD$ from \cref{ex:ECbijection}
\begin{center}
\begin{tikzpicture}[scale=.7]  
\node (1) at (-1.5, -1) {$15$};
\node (2) at (-1.5, -2) {$7$};
