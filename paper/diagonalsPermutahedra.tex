...
More precisely, each $\SCP$ is the minimal element of a lattice isomorphic to a product of chains, where the partial order is given by shifts.
Latter, we will use the lattice structure to relate the cubical and shift definitions of the $\SUD$ diagonal in \cref{sec:Cubical}.

\begin{definition}
Let $\sigma$ denote any one of the two ordered partitions in a $(2,n)$-partition tree.
The \defn{right height} (\resp \defn{left height}) of $\rho \in [n]$ for the $\LA$ diagonal is the maximal value $r_\rho^\LA(\sigma)$ (\resp $l_\rho^\LA(\sigma)$) of the natural number $m \geq 1$ for which the right $m$-shift of $\rho$ (\resp the left $m$-shift of $\rho$) is path-admissible in the $\LA$ sense, or $0$ if there are no such $m$-shifts.
The right and left heights for the $\SU$ diagonal are defined similarly. 
\end{definition}
...
Recall from \cref{subsec:inversions} that the inversions of an ordered $(2,n)$-partition tree are the pair of edges that intersect. 

\begin{definition}
The \defn{shift lattice} on the set of facets of the $\LA$ (\resp $\SU$) diagonal is the partial order given by inclusion of their inversion sets. 
\end{definition}

\Kurt{Actually, I had originally defined $L(\sigma,\tau)$, as on the set of all facets, we have a disjoint union of lattices. (There are ways to make this a lattice, [Use weak order lattice to order SCPs, then use this lattice on all facets associated to the SCP. This nested lattice is a lattice], but bit artificial. ) Happy to chat tomorrow.}

Thus the shift lattice structure is by definition compatible with a generalised Bruhat order.\Guillaume{Ref? L'ordre de Ronco?? Si oui mettre dans l'intro!}

\begin{proposition} 
\label{prop:shift lattice}
The shift lattices on the sets of facets of the operadic diagonals are indeed lattices, isomorphic to the product over all strong complementary pairs $(\sigma,\tau)$ of the following lattices
\begin{align*}
	 \prod_{\rho \in [n]} [0,l_\rho^\LA(\sigma)]
	\times \prod_{\rho \in [n]} [0,r_\rho^\LA(\tau)] \ , \quad \text{ and }  \prod_{\rho \in [n]} [0,r_\rho^\SU(\sigma)]
	\times \prod_{\rho \in [n]} [0,l_\rho^\SU(\tau)] \ .
\end{align*}
\end{proposition}
\begin{proof}
We identify $(0,0,\ldots,0)$ with the strong complementary pair $(\sigma,\tau)$ and every other element $(r_1,\ldots,r_n,l_1,\ldots,l_n)$, with 
$R^{r_n}_{n}\cdots R^{r_{1}}_{1}
L^{l_n}_{n} \cdots L^{l_{1}}_{1}(\sigma,\tau)$.
Given \cref{lem:m-shifts commute}, it is straightforward to confirm that the join (\resp meet) of any two elements is given by their element wise maximum (\resp minimum). Explicitly,
{\small
\begin{align*}
	(r_1,\ldots,r_n,l_1,\ldots,l_n)\lor (r'_1,\ldots,r'_n,l'_1,\ldots,l'_n) &= (\max\{r_1,r'_1\},\ldots,\max\{r_n,r'_n\},\max\{l_1,1'_1\},\ldots,\max\{l_n,l'_n\})\\
	(r_1,\ldots,r_n,l_1,\ldots,l_n)\land (r'_1,\ldots,r'_n,l'_1,\ldots,l'_n) &= (\min\{r_1,r'_1\},\ldots,\min\{r_n,r'_n\},\min\{l_1,1'_1\},\ldots,\min\{l_n,l'_n\})
\end{align*}
}
We note that the maximal element of the lattice is given by shifting each element of $(\sigma,\tau)$ by its maximal shift.
\end{proof}
...
\begin{corollary}
We have the following statistics on permutations in terms of heights
\begin{align*}
...
\end{align*}
Moreover, denoting by $P_{k_1,k_2}^n\subseteq \mathbb{S}_n$ the set of permutations with $k_1$ ascending and $k_2$ descending runs ($k_1+k_2=n-1$), we have 
\begin{align*}
...
\begin{remark}
Whilst each height can be calculated from the $\SCP$ they can also be reformulated directly in terms of the permutation.
