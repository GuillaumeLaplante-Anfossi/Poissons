...
The objective of \cref{part:combinatorics} is to explore the combinatorics of these multi braid arrangements.
...
\begin{remark}
Connections to other multi braid arrangements. Shi arrangements (non generic). Discriminantal arrangements (fully generic).
\vincent{todo}
\end{remark}
...
\begin{definition} 
\label{def:cellular diagonal}
    A \defn{cellular diagonal} of a polytope $P$ is a continuous map $P \to P \times P$ such that
    \begin{enumerate}
...
The key idea is that any vector $\vec v$ in generic position with respect to $P$ defines a cellular diagonal of $P$. 

\begin{definition}
    \label{def:diagonal}
    A \defn{cellular diagonal} or simply a \defn{diagonal} of a polytope~$P$ is a tight coherent section~$\triangle_{(P,\vec v)}$ of the projection $P \times P \to P, (x,y) \mapsto (x+y)/2$.
    It is given explicitly by a choice of generic vector $\vec v$ via the formula
    \begin{align*}
        \begin{array}{rlcl}
        ...
        \end{array}
    \end{align*}
    where $\rho_z P := 2z-P$ denotes the reflection of $P$ with respect to the point $z$, and $\min_{\vec v}(P)$ (resp. $\max_{\vec v}(P)$) denotes the unique vertex of $P$ which minimizes (resp. maximizes) the scalar product with $\vec v$. 
\end{definition}

\begin{figure}[h!]
	\includegraphics[scale=.9]{diagonalTriangle3.pdf}
    \quad
    \includegraphics[scale=.9]{diagonalTriangle4.pdf}
    \quad
    \includegraphics[scale=.9]{diagonalSquarre3.pdf}
    \includegraphics[scale=.9]{diagonalSquarre4.pdf}
	\caption{The thin (in blue) and cellular (in red) diagonals of the triangle and the square.}
	\label{fig:diagonal-2simplex}
\end{figure}

The image of $\triangle_{(P,\vec v)}$ is a family of pairs of faces $F \times G \subset P \times P$.
The generic vector $\vec v$ orients the $1$-skeleton of $P$, which defines for each face $F$ of $P$ a unique minimum vertex~$\min_{\vec v}(F)$, and a unique maximum vertex $\max_{\vec v}(F)$.
The following inclusion was proved in \cite[Prop. 1.17]{LA21}.

\begin{proposition}
    For any polytope $P$, and for any generic vector $\vec v$, we have that 
    \begin{equation}
        \Ima\triangle_{(P, \vec v)} \subset \bigcup_{\substack{F,G \subset P \\ \max_{\vec v}(F) \leq \min_{\vec v}(G)}} F \times G \ . \label{eq:magique}
    \end{equation}
\end{proposition}

That is, the pairs of faces in the image of a cellular diagonal are always comparable under the partial order induced by $\vec v$ on the $1$-skeleton. 
...
The diagonal $\triangle_{(P,\vec v)}$ defines a canonical polytopal subdivision of $P$, obtained by drawing the polytopes $(F+G)/2$, for all pairs of faces $(F,G) \in \Ima \triangle_{(P,\vec v)}$.

\begin{figure}[h!]
	\includegraphics[scale=.2]{diagonalSimplexGuillaume.png}
    \quad
    \includegraphics[scale=.2]{diagonalCubeGuillaume.png}
    \quad
    \includegraphics[scale=.2]{diagonalPermutahedronGuillaume.png}
	\caption{The subdivisions induced by cellular diagonals of the $3$-simplex, cube, and permutahedron.}
	\label{fig:diagonal-3simplex}
\end{figure}

\begin{definition}
    The \defn{$f$-vector} of the diagonal $\triangle_{(P,\vec v)}$ is the number of faces of $P\times P$ of given total dimension in its cellular image.
    Alternatively, it is the $f$-vector of the polytopal complex~$\pi (\triangle_{(P,\vec v)})$.
\end{definition}

For the purpose of studying this $f$-vector, one can study the dual of the polytopal complex, see \cref{fig:diagonalPermutahedron1}.
The following was proven in \cite[Corollary 1.4]{LA21}.

\begin{proposition}
    The $f$-vector of the diagonal $\triangle_{(P,\vec v)}$ is given by the opposite of the $f$-vector of the polyhedral complex made of two copies of the normal fan of $P$, one of them perturbed in the direction $\vec v$. 
\end{proposition}

\begin{remark}
    This is in fact precisely the content of the \emph{Fulton--Sturmfels formula} for the intersection product on toric varieties \cite[Theorem 4.2]{fultonIntersectionTheoryToric1997a}, where the definition of cellular diagonals as tight coherent sections first appeared.
\end{remark}

The following \defn{universal formula} \cite[Theorem 1.26]{LA21} expresses combinatorially the cellular image of the diagonal of a polytope $P$.
...
\begin{proposition}
    The $f$-vector of the $(\ell-1)$\ordinalst{} iteration of a diagonal of the $(n-1)$-permutahedron $\triangle_{(P,\vec v)}$ is given by the opposite of the $f$-vector of the $(\ell,n)$-braid arrangement.
\end{proposition}

\begin{proposition}
    A diagonal $\triangle_{(P,\vec v)}$ of the standard permutahedron does \emph{not} satisfy the magical formula.
    That is, the inclusion      
    \begin{equation*}
        \Ima\triangle_{(P, \vec v)} \subsetneq \bigcup_{\substack{F,G \subset P \\ \max_{\vec v}(F) \leq \min_{\vec v}(G)}} F \times G  
    \end{equation*}
    is strict, for any choice of generic vector $\vec v$. 
\end{proposition}
...
The fundamental hyperplane arrangement of the permutahedron was computed in \cite[Section 3.1]{LA21}.
Let $\Un(n):=\{ (I,J) \ | \ I,J \subset [n], |I|=|J|>0, I \cap J = \emptyset\}$.
...
Recall that a choice of diagonal is given by a choice of vector in a chamber of $\mathcal{H}_P$, two vectors living in the same chamber inducing the same diagonal \cite[Proposition 1.23]{LA21}.
...
We now specialize the results of \cref{part:combinatorics} to the case~$\ell = 2$ to derive enumerative results on the diagonals of the permutahedra.
...
\begin{remark}
    This answers by the negative a conjecture regarding unicity of diagonals on the permutahedra, raised at the beginning of \cite[Section 3]{SaneblidzeUmble04}, and could be seen as an alternative statement. \Guillaume{See the next section where we prove that SU diagonal is SU diagonal}
\end{remark}
...
The key observation in proving \cref{thm:facets} is that paths between pairs of vertices in a $(2,n)$-tree correspond to $(I,J)$ conditions defining an operadic diagonal (see \cref{s:diagonals-permutahedra}). 
...
\end{align*}
and we have $(I,J) := (\{i_1,\dots,i_k\},\{j_1,\dots,j_k\}) \in \LA(n)$ since the minimal element points from~$\sigma$ to $\tau$.
...
\begin{align}
    (I_k,J_k) := (\{1,k+2,k+3,\dots,2k-1,2k\}, \{2,3,\dots,k+1\}) \label{eq:standard}
\end{align}
...
Since the pair $(I,J)$ is of standard form (\ref{eq:standard}), so is the pair $(I',J'):=(I_k\setminus \{i_1\}, J_k \setminus \{j_1\})$, and one can check using both cases of \cref{lem:Coherent Domination} that $J'$ dominates $I'$ in $\sigma$, and $I'$ dominates $J'$ in $\tau$.
...
\begin{align*}
    L^{\LA}(\sigma,\tau) := \bigcup_{\mathbf{M}, \mathbf{N}} L_\mathbf{N}(\sigma)\times R_\mathbf{M}(\tau), \quad L^{\SU}(\sigma,\tau) := \bigcup_{\mathbf{M}, \mathbf{N}} R_\mathbf{M}(\sigma)\times L_\mathbf{N}(\tau)
\end{align*}
...
Given our previously obtained formulae for the number of elements in the diagonal (\cref{subsec:enumerationDiagonal}), and the results of this section we obtain the following statistics on permutations.
\begin{corollary}
