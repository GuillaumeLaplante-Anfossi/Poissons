...
We define inductively a subdivision $\divcube{n-1}^\SU$ of the $(n-1)$-dimensional cube  which is combinatorially isomorphic to the permutahedron $\Perm$.
...
\begin{align} \label{eq:sub}
	...
	\tag{I}
\end{align}
We obtain a subdivision $\divcube{n}^\SU$ of the $n$-cube isomorphic to the permutahedron $\Perm[n+1]$.
\end{construction}
...
The cubical formula for the diagonal is constructed through maximal pairings of $k$-subdivision cubes of $\divcube{n}^\SU$, which we now define, and prove the existence of.
...
Every $k$-face of $\divcube{n}^\SU$ is a $k$-subdivision cube; other examples are provided in \cref{ex:subdivision cubes}.
...
\begin{proof}
	...
	Moreover, the vector $\b{v}\eqdef(1,\ldots,1)$ induces the weak order on the vertices of $\divcube{n}^\SU$.  
...
\begin{definition}[{\cite[Def.~2.1 \& 2.22]{LaplanteAnfossi}}]
...
\begin{definition}[{\cite[Def. 3.2]{LaplanteAnfossiMazuir}}]
	A \defn{$2$-colored nesting} is a nesting where each nest $N$ is either blue, red, or blue and red (purple), and which satisfies that if $N$ is blue or purple (\resp red or purple), then all nests contained in $N$ are blue (\resp red). \Guillaume{Add example?}
\end{definition}
...
Via the functor of cellular chains, it defines universal tensor products of $\Ainf$-algebras and $\Ainf$-morphisms \cite[Prop. 4.15 \& 4.18]{LaplanteAnfossiMazuir}.
...
It is easy to find a bigger multiplihedron $\J_{n''}$, for which $\J_n$ and $\J_{n'}$ appear in the produt decomposition of a face of $\J_{n''}$ and by the preceding remark, any choice of diagonal for $\J_{n''}$ will then contradict our initial two choices. 
Thus, these had to be the same from the start, which conclude the proof of Point (1). 

Point (2) then follows from the fact that a choice of diagonal for the Loday associahedra and the Forcey--Loday mutiplihedra \emph{forces} a unique topological cellular colored operad and operadic bimodule structure on them, see~\cite[Thm.~1]{MasudaThomasTonksVallette} and \cite[Thm.~1]{LaplanteAnfossiMazuir}.
...
Finally, since only vectors with strictly descreasing coordinates induce the Tamari-type order on the skeleton of the Loday multiplihedra~\cite[Prop. 2.10]{LaplanteAnfossiMazuir}, we get the last part of the statement. 
\end{proof}
...
Using the $\SU$ diagonal $\LAD$, one defines similarly a topological operadic bimodule structure via the same formula, but with a different transition map $\tr$, which commutes with $\SUD$.

Recall that a face of $\J_n$ is represented by a $2$-colored nested tree $(t,\mathcal{N})$, which can be written uniquely as a sequence of substitution of trivially nested $2$-colored trees $(t,\mathcal{N})=((\cdots((t_1\circ_{i_1} t_2) \circ_{i_2} t_3) \cdots )\circ_{i_k} t_{k+1})$.
Here we use the left-levelwise order on nestings \cite[Def. 4.12]{LaplanteAnfossiMazuir}, and the bijection between $2$-colored trees and $2$-colored nested linear trees from \cite[Lemma~3.4, Fig.~6]{LaplanteAnfossiMazuir} to translate the tree grafting operations $\circ_{p+1}$ and $\gamma_{i_1,\ldots,i_k}$ into $2$-colored nested tree substitution $\circ_{i_j}$.
Note that any choice of $\circ_i$ substitutions yield the same $2$-colored nested tree, since these form an operadic bimodule.
...
For any pair of homotopy operads $\mathcal{P},\mathcal{Q}$, the two universal tensor products defined by the $\LA$ and $\SU$ diagonals are not isomorphic in the category of homotopy operads and strict morphisms.
...
Then, by \cite[Cor.~2]{DotsenkoShadrinVallette} there is an $\infty$-isotopy, that is an $\infty$-isomorphism whose first component is the identity, between the two homotopy operad structures on the tensor product $\mathcal{P} \otimes \mathcal{Q}$.
\end{proof}
...
For the operahedra in general, such a bijection does not exist, as \cref{prop:operahedra-LA-SU} demonstrates. 
...
The datum of a diagonal of the operad $\Ainf$ encoding $\Ainf$-algebras and a diagonal of the operadic bimodule $\mathrm{M}_\infty$ enconding $\Ainf$-morphisms is equivalent to the datum of a morphism of $2$-colored operads $\Ainf^2 \to \Ainf^2 \otimes \Ainf^2$. 

\begin{theorem}
	\label{thm:infinity-iso-2}
	For any pair of $\Ainf$-morphims $F_1,F_2$, the two universal tensor products defined by the $\LA$ and $\SU$ diagonals are not isomorphic in the category of $\Ainf^2$-algebras and strict morphisms.
...
\begin{proof}
	...
	Then, by \cite[Cor.~2]{DotsenkoShadrinVallette} there is an $\infty$-isotopy, that is an $\infty$-isomorphism whose first component is the identity, between the two $\Ainf$-morphisms~$F_1 \otimes_\LA F_2$ and~$F_1\otimes_\SU F_2$.
\end{proof}


We now compute the number of facets in the $\LA$ and $\SU$ diagonals for some operahedra.
...
For $t \in \PT_n$, we denote by $\LAD_t$ (\resp $\SUD_t$) the set of facets of $\LAD$ (\resp $\SUD$) which are valid nests of $t$.
\end{definition}


\begin{example}
\label{ex:Validity example}
...
However, there are 24 planar trees with $5$ internal edges, such that the number of facets of the $\LA$ and $\SU$ diagonals are distinct. 
\end{proposition}
...
Thus, the statement is equivalent to the following: for any $n\leq 4$, and for every tree $t\in \PT_n$, we have $\card{\LAD_t} = \card{\SUD_t}$, while for $n=5$, there are 24 trees $t \in \PT_5$, such that $\card{\LAD_t} \neq \card{\SUD_t}$.
This was computed via code, see \cref{ex:SU-LA-operahedra}.
\end{proof}

\Guillaume{Code available on the author's website?}

\begin{remark}
	It seems likely that for every $n\geq 6$, there are trees with $n$ internal edges for which the diagonals do not have the same number of facets. 
\end{remark}
...
We end by computing the number of facets in the $\LA$ and $\SU$ diagonals for some multiplihedra.

\begin{definition}
\label{def:valid-2}
Let $\sigma=\sigma_1| \cdots |\sigma_k$ be an ordered partition of $[n]$. 
We say that $\sigma$ is \defn{valid} for a $2$-colored linear tree $t \in \PT_n$ if... \Guillaume{TBC}
\end{definition}

We have a 

\begin{proposition}
	\label{prop:multiplihedra-LA-SU}
	The $\LA$ and $\SU$ diagonals of the multiplihedra associated with linear trees that have less than $4$ internal edges have the same number of facets. 
	However, for linear trees with $5$ and $6$ internal edges, the number of facets of the $\LA$ and $\SU$ diagonals differ. 
\end{proposition}

\begin{proof}
	This result follows from computer experiments, which are tabulated in \cref{table:multiplihedra}.
\end{proof}
...
\begin{remark}
	It seems likely that for every $n\geq 6$, the $\LA$ and $\SU$ diagonals of the multiplihedra associated to linear trees with $n$ internal edges have distinct numbers of facets. 
\end{remark}
