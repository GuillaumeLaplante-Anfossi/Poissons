...
Since $\sigma_{i,i+m}=\sigma_{i,i+1}\sigma_{i+1,i+m}$ we have that $\max \sigma_{i,i+m}\geq \max \sigma_{i,i+1}$,
\Kurt{can you see a quick way to prove this? (given that $\sigma_{i,i+m}=\sigma_{i,i+1}\sigma_{i+1,i+m}$ only if you cancel steps traversed in the opposite direction).
(We know the max cannot be in the overlap in the geometric def as it would be traversed in opposite directions.)
}
which means that the $1$-shift $R_\rho$ is path-admissible.
