...
To give the precise meaning of generic with respect~$P$ and the characterization of the faces in the image of~$\triangle_{(P,\b{v})}$, we now consider the following hyperplane arrangement.
\vincent{add figure of fundamental hyperplane arrangements for simplex, cube, permutahedron.}
...
Moreover, the geometric diagonal~$\triangle_{(P,\b{v})}$ only depends on the region of~$\mathcal{H}_P$ containing~$\b{v}$, see~\cite[Prop.~1.23]{LA21}.
...
\begin{remark}
For some polytopes such as the simplices, the cubes, the freehedra, and the associahedra, the reverse inclusion also holds.
\vincent{reference}
...
\begin{definition}
For $I,J \subseteq [n]$ and an ordered partition $\sigma$ of~$[n]$ into $k$ blocks, we say that $I$~\defn{dominates} $J$ in~$\sigma$ if for all $\ell \in [k]$, the first~$\ell$ blocks of~$\sigma$ contain more elements of~$I$ than of~$J$.
\end{definition}
...
\begin{proof}
    First, by \cite[Thm.~3.9]{LA21} any geometric diagonal is given by a choice of ordering of $\Un$.
