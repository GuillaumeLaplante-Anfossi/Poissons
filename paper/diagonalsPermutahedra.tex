...
\begin{figure}[h!]
	...
	Lattice & New & New \\
	Matrix & \cite{SaneblidzeUmble} & New \\
	Cubical & \cite{SaneblidzeUmble} & New
	\end{tabular}
	\end{center}
\end{figure}
...
\label{prop:Maximal Subdiv Pairs and Shifts}
...
\Kurt{The definitions I'll need.}

\begin{definition}
Let $T_n$ be the set of planar trees with $n$ internal edges, which are labelled by $[n]$ using the infix order.
A nest 2.1 of \cite{LaplanteAnfossi}.
A nesting 2.2 of \cite{LaplanteAnfossi}.
Let $\sigma=\sigma_1|...\sigma_k$ be an ordered partition of $n$. 
We say that $\sigma$ is valid for $T$, if $\{\sigma_1, \sigma_1 \cup \sigma_2,...,\bigcup_{i=1}^k \sigma_i\}$ is a nesting of $T$.
We say that a tuple of ordered partitions is valid for $T$, if all the ordered partitions are valid.
For $t \in T_n$, we define $\LAD_t$ (\resp $\SUD_t$) to be the elements of $\LAD$ ($\SUD$) which are valid nests of $t$.
\end{definition}
...
\begin{proposition}
For $n\leq 4$, for every $t\in T_n$, $|\LAD_t| = |\SUD_t|$.
For $n>4$, there exists $t \in T_n$, such that $|\LAD_t| \neq |\SUD_t|$.
\end{proposition}
\begin{proof}
This was checked via code, however, here are the $24$ planar trees with $5$ internal edges for which $|\LAD_t| \neq |\SUD_t|$.
Note that $|\LAD_t|,|\SUD_t|\leq |\LAD_5|=|\SUD_5|=432 $ (\cref{table:enumerationDiagonalPermutahedra1}).
We display each tree $t$ with $(|\LAD_t|,|\SUD_t|)$ above it, and arrange them symmetrically to help emphasize notable missing elements. 
\Kurt{Will align better, but something like this.
Re-checking code, the numbers are bizarre, but might really be what it is.}
...
\begin{proposition}[Proposition 3.20 \cite{LaplanteAnfossi}]
...
\end{proposition}
