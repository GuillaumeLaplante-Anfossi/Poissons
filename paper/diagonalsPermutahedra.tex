...
\thanks{BDO was partially supported by the French ANR grants ALCOHOL (ANR-19-CE40-0006), CARPLO (ANR-20-CE40-0007), HighAGT (ANR-20-CE40-0016) and S3 (ANR-20-CE48-0010). VP was partially supported by the French ANR grant CHARMS (ANR-19-CE40-0017) and by the French--Austrian project PAGCAP (ANR-21-CE48-0020 \& FWF I 5788).}
...
The purpose of the first part of this article is to study the combinatorics of the cellular diagonal of the permutohedra, explained in more details in the second part of the introduction.
 ...
 The hyperplane arrangement associated with  the cellular diagonal is two generically translated copied of the braid arrangement. 
...
\begin{theorem}
The number of vertices of the $(\ell,n)$-braid arrangement~$\multiBraidArrangement$ (or equivalently of $(\ell,n)$-trees) is
\[
f_0(\multiBraidArrangement) = \ell \big( (\ell-1) n + 1)^{n-2}.
\]
\end{theorem}
We end this combinatorial section with a conjecture on edge-colored trees.
...
The flat poset~$\flatPoset[\braidArrangement]$ is thus the refinement poset~$\partitionPoset$ on set partitions of~$[n]$ (where a partition~$\pi$ is smaller than a partition~$\omega$ if each part of~$\pi$ is contained in a part of~$\omega$).
...
\vincent{I copy two tables here, we should explain them in the text.}
...
\begin{center}
\begin{tikzcd}
\text{Subset $1$-shift $\SUD$} \arrow[r,leftrightarrow,"\ref{prop:subset shift to shift bijection}"]&
\text{$1$-shift $\SUD$} \arrow[r,leftrightarrow,"\ref{prop:subset shift to shift bijection}"]&
\text{$m$-shift $\SUD$} \arrow[r,leftrightarrow,"\ref{prop: shift SUD and SUD are equal}"]&
\SUD \\
...
\subsubsection{Subset $1$-shift, $1$-shift and $m$-shift diagonals}

\begin{definition}[Subset $m$-shift operators]
...
For $m\geq 1$, the \emph{subset right $m$-shift operator} moves the subset $M_i$, $m$ blocks to the right  
\begin{align*}
    R_{M_i}(\sigma) \eqdef \sigma_1|\dots|\sigma_i \ssm M_i|\dots | \sigma_{i+m} \cup M_i|\dots|\sigma_k
\end{align*}
while the \emph{subset left $m$-shift operator} moves it $m$ blocks to the left 
\begin{align*}
...
\begin{definition}[Movable subsets]
    ...
    A non-empty subset $M_i \subset \sigma_i$ (resp. $N_{i}\subset \tau_{i}$) is \emph{$1$-movable} if 
    \begin{enumerate}
        \item $\min \sigma_i \notin M_i$ (resp. $\min \tau_{i} \notin N_i$), and
        \item $\min M_i> \max \sigma_{i+1}$ (resp. $\min N_{i}> \max \tau_{i-1}$).
    \end{enumerate}
\end{definition}

\begin{example}
Performing the subset $1$-shifts $R_7$ and $L_{5,6}$ on $1$-movable subsets of the strong complementary pair~$(\sigma,\tau)$ of \cref{ex:strong-complementary}, one obtains the pair $R_{7}(\sigma) \times L_{5,6}(\tau)$.
\begin{center}
\begin{tikzpicture}[scale=.7]  
    \node (p) at (0, 0) {$13|247|5|6 \times 3|17|4|256$};
    \node (1) at (-1.5, -1) {$13$};
...
\begin{definition}[Admissible sequences of subset $1$-shifts]
    Let $(\sigma,\tau)$ be a pair of ordered partitions. 
    For $p\geq 1$, a sequence of right subset $1$-shifts $\textbf{M} = (M_{i_1},\dots,M_{i_p}) \in [n]^{p}$ is \emph{admissible} if 
    \begin{enumerate}
        \item we have $1\leq i_1 < i_2 < \dots < i_p \leq k-1$, and
        \item each $M_{i_j}$ is a $1$-movable subset of $R_{M_{i_j-1}}\dots R_{M_{i_1}}(\sigma)_{i_j}$.
    \end{enumerate}
    We denote the resulting ordered partition by $R_\mathbf{M}(\sigma) \eqdef R_{M_{i_p}}\dots R_{M_{i_1}}(\sigma)$.
    The result of an admissible sequence of left subset $1$-shifts $L_\mathbf{N}(\tau)  \eqdef  L_{N_{i_1}}\dots L_{N_{i_q}}(\tau)$ is defined similarly via the condition $2\leq i_1 < i_2 \dots < i_q \leq k$, with $q\geq 1$.
\end{definition}

\Guillaume{Think about notation: too many indices!! $R_{M_{i_j}}$}

By convention, we declare the empty sequence of subset $1$-shift operators to be admissible, and to act by the identity, i.e. we have $R_{\mathbf{\emptyset}}(\sigma)  \eqdef  \sigma$ and $L_{\mathbf{\emptyset}} (\tau) \eqdef  \tau$.

\begin{definition}[Subset $1$-shift $\SU$ diagonal {\cite{SaneblidzeUmble04,saneblidzeComparingDiagonalsAssociahedra2022}}]
    \label{def:classical-SU}
   The facets of the \emph{subset $1$-shift $\SU$ diagonal} are given by the formula
    \begin{align*}
        \SUD([n]) = \bigcup_{(\sigma,\tau)} \bigcup_{\mathbf{M}, \mathbf{N}} R_\mathbf{M}(\sigma)\times L_\mathbf{N}(\tau)
    \end{align*}
    where the unions are taken over all strong complementary partitions $(\sigma, \tau)$ of $[n]$, and over all admissible sequences of subset $1$-shift operators $\textbf{M},\textbf{N}$.
\end{definition}

\begin{remark}
    Observe that the left (resp. right) subset $1$-shift operator moves elements to the left (resp. right), but acts on the right (resp. left) ordered partition.
    In the analogous description for the diagonal $\LAD$ obtained at the end of the Section \Guillaume{[REF]}, left and right subset $1$-shift operators act on the left and right ordered partitions, respectively. 
\end{remark}

We now restrict the size and condition on the subsets that can be moved, but allow them to move more than one block at a time. 

\begin{definition}[Critical elements] 
    \label{def:critical-elements}
    Let $(\sigma,\tau)$ be a pair of ordered partitions.
    For $m\geq 1$, an element $\rho \in \sigma_i$ (resp. $\rho \in \tau_i$) is \emph{$m$-critical} if 
    \begin{enumerate}
        \item $\rho \neq \min \sigma_i$ (resp. $\rho \neq \min \tau_i$), and
        \item $\rho>\max \sigma_{i,i+m}$ (resp. $\rho > \max \tau_{i,i-m}$).
    \end{enumerate}
\end{definition}

For any $m\geq 1$, let us call \emph{critical} the right and left $m$-shift operators associated with the singleton sets given by $m$-critical elements, and denote them by $R_{\rho}$ and $L_{\rho}$.

\begin{definition}[$1$-shift and $m$-shift $\SU$ diagonals]
    \label{def:m-shift-SU}
    The facets of the \emph{$1$-shift (resp. $m$-shift) $\SU$ diagonal} are given by the formula
    \begin{align*}
        \SUD([n]) = \bigcup_{(\sigma,\tau)} \bigcup_{\mathbf{M}, \mathbf{N}} R_\mathbf{M}(\sigma)\times L_\mathbf{N}(\tau)
    \end{align*}
    where the unions are taken over all strong complementary partitions $(\sigma, \tau)$ of $[n]$, and over all well-defined sequences of critical $1$-shift operators $\textbf{M},\textbf{N}$ (resp. sequences of critical $m$-shift operators, for possibly different $m\geq 1$ at each step in the sequence).
\end{definition}

\begin{remark}
    Note here the two important differences between the subset $1$-shift and $m$-shift $\SU$ diagonals: a $m$-critical element $\rho$ is not required to satisfy $\rho > \max\sigma_{i+m}$ (compare Points (2) in \cref{def:movable-subsets} and \cref{def:critical-elements}), and the numbers indexing a sequence of shifts are not required to be increasing (compare \cref{def:classical-SU} and \cref{def:m-shift-SU}).
\end{remark}


%%%%

\subsubsection{First isomorphism: subset $1$-shift and $1$-shift diagonals}

\begin{proposition} 
\label{prop:SU-preserves-max}
The admissible subset $1$-shift operators conserve the maximal element of any path between two blocks of the same ordered partition.
In particular, for a pair of ordered partitions $(R_{\mathbf{M}}(\sigma),L_{\mathbf{N}}(\tau))$ obtained via an admissible sequence of $1$-shifts from a strong complementary pair~$(\sigma,\tau)$, we have that
\begin{align*}
...
\end{align*}
Moreover, the direction in which this maximal element is traversed in the path is also preserved. 
\end{proposition}

\begin{remark}
    Note that in the fist line of \cref{prop:SU-preserves-max}, the maxima $\max \sigma_{i,j}$ and $\max \tau_{i,j}$ are maxima of \emph{paths}, while in the second line the maxima $\max \sigma_{i+1}$ and $\max \tau_{i}$ are maxima of \emph{blocks}.
\end{remark}

\begin{proof}
For simplicity, in this proof we use ``shift" instead of ``$1$-shift".
...
Moreover, in each case, the maximal element $\max \gamma = \max \gamma'$ are traversed in the same direction.
It is immediate for cases (1), (2a) and (3); in the case (2b) it follows from the observation that the number of steps of $\beta \cap \delta_{k,k+1}$ and $\delta_{k,k+1}$ have the same parity. \Guillaume{rewrite}
\end{proof}

\begin{corollary} 
\label{cor:SU-shift-preserves-max}
The critical left and right shift operators conserve the maximal element of paths.
In particular, if $(\sigma',\tau')$ is generated from a $\SCP$ $(\sigma,\tau)$ by critical shifts, then we have
\begin{align*}
...
\begin{proof}
    By definition of the critical shifts, \cref{eq:shift-subset-dominates} holds and we can run the proof of \cref{prop:SU-preserves-max} \emph{mutatis mutandis}.
\end{proof}
...
\begin{proposition}
\label{prop:subset shift to shift bijection}
...
First, we observe that any subset right shift $R_{M_k}(\sigma)$ can be decomposed into a series of critical elements right shifts: since $\min M_k > \max \sigma_{k,k+1}$ by the proof of \cref{prop:SU-preserves-max}, we can shift the elements of $M_k$ to the right one after the other (in any order!).
...
Then from \cref{cor:SU-shift-preserves-max}, we know that $\rho>\max \sigma_{i_j+1} = \max (R_{M_{i_{j-1}}}\dots R_{M_{i_1}}(\sigma))_{i_j + 1}$, where the equality follows as $i_1<\dots<i_{j-1}<i_j$. This proves that $R_{M'_{i_j}}$ is well-defined, completing the inductive proof.
\end{proof}
%On peut se servir de l'hypothèse de récurrence et de la Proposition 2.45. Pas besoin du corollaire!!

%%%%

\subsubsection{Second isomorphism: $1$-shift and $m$-shift coincide}
...
\cref{cor:SU-shift-preserves-max} shows that the maxima of paths between consecutive vertices in $(R_{\rho}(\sigma),\tau)$ are the same as the ones in $(\sigma,\tau)$. 
