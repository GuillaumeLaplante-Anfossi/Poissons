...
\vincent{This subsection should only be about diagonals of polytopes, not specific to the permutahedron.}
\vincent{This is where I would connect the diagonal~$\triangle_{(P,\vec v)}$ with the oriented skeleton of~$P$ in direction~$\vec v$.}
...
\vincent{All this paragraph is useless... or should appear in \cref{subsec:braidArrangement}...}
...
\vincent{This definition should appear in the previous paragraph...}
\end{definition}
...
\end{eqnarray} 
\vincent{This theorem should appear in the previous paragraph...}
\end{theorem}
...
\Kurt{Lattice Section should be after inversion so can discuss link to inversion order.
Should be after face poset isomorphism so can unpack LA lattice as well.}
...
\Kurt{Happy for all notation to be changed}

In this section, we show that $\SUD$ 1-shifts admit a simple lattice structure, that is to say each $\SCP$ is the minimal element of a lattice isomorphic to a product of chains, where the partial order is given by shifts.
We will discuss how this order is compatible with the inversion partial order, and latter use results of this section to prove the shift definition of the diagonal is equivalent to the $(I,J)$, and cubical definitions of the diagonal (Sections \ref{sec:Iso m-shifts to IJ} and \ref{sec:Cubical}).
...
In particular, the maximal element of the lattice is given by shifting all elements of $\sigma$ ($\tau$) as far to the right (resp. left) as possible.
\end{proof}
