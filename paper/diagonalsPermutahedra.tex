...
\Kurt{Look at one particular tree in detail.}
...
This was checked via code and the few first counterexamples are displayed in \cref{ex:SU-LA-operahedra} below.
\end{proof}

\begin{remark}
	Changing infix order inertwines the counts \Guillaume{@Kurt can you complete?}
\end{remark}

\begin{example}
\label{ex:SU-LA-operahedra}
We present herethe $24$ planar trees with $5$ internal edges for which $|\LAD_t| \neq |\SUD_t|$.
...
A sufficient condition for a tree $t$ to satisfy $|\LAD_t|=|\SUD_t|$ is for $rN$ to be a nesting iff $N$ is a nesting. This includes trees such as the linear tree and
...
\Kurt{Old arrangement of trees which mirror code will remove later.}
\begin{center}
\begin{tikzpicture}[every tree node/.style={draw, very thick, circle, inner sep=0.08cm}, level distance=0.7cm,sibling distance=0.3cm, edge from parent/.style={very thick, draw}]
\Tree 
[.\node{}; 
