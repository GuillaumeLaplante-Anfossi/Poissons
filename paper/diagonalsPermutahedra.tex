...
\Guillaume{Can we align and space these drawings a bit better?}
\begin{center}
...
\subsection{The facial weak order}
\label{sec:facial-weak-order}

The facial weak order is a lattice strucure on the faces of the permutahedra, which generalizes the weak order on permutations to all ordered partitions.
It was introduced in \cite{palacios2004weak}.
...
\begin{example}
We illustrate the facial weak order for $\Perm[3]$ as a refinement of the weak inversion order, grouping elements with the same weak inversion sets.
\begin{center}
\begin{tikzpicture}
\node (1) at (-1.5, -1) {$3|12$};
...
\begin{lemma}\label{lem:SCPs are intervals of facial weak order}
Let $(\sigma,\tau)$ be the SCP corresponding to the permutation $v$, then $\sigma<v<\tau$ under the facial weak order.
\end{lemma}
\begin{proof}
...
More generally,
\begin{proposition}
Facets $(\sigma,\tau)$ of the $\LA$ and $\SU$ diagonal correspond to intervals $[\sigma,\tau]$ of the facial weak order.
\end{proposition}
\begin{proof}\
...
Given \cref{lem:SCPs are intervals of facial weak order}, we have a base case for induction.
...
To prove the first equality, we show that $I_<(v)\subset I_<(v')$ (as the the facial weak order on permutations is the weak order).
This follows as \Kurt{Clear, but think of something concise.}
\end{proof}

So given this result and \cref{thm:patterns} (which shows that vertices are intervals in the weak order, and hence the facial weak order), it is suspected that all faces of the $\LA$ and $\SU$ diagonals are intervals of the facial weak order. 
...
Given that in \cite{dermenjian2018facial} it was shown that the facial weak order is a lattice, it is natural to wonder if the identified lattice structure arises as a sub-lattice of the facial weak order.

\begin{proposition}
The shift lattices are not sub-lattices of the facial weak order.
\end{proposition}

\begin{proof}
We provide a counter example for the $\LA$ diagonal that illustrates the identified lattice structure is not compatible with the facial weak order.
