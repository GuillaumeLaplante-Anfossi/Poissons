...
there exists $I'\subset I, J'\subset J$ of the same size $|I'|=|J'|$ such that both $\std(I',J')$ and~$\std(I\setminus I', J'\setminus J')$ are in $\Or$.
\end{definition}

We call \defn{indecomposables} of $\Or$ the pairs $(I,J) \in \Or$ such that the only subpair $(I',J')$ satisfying condition above is the pair $(I,J)$ itself.
Note that it follows from the definition that $(I,J) \in \Or \iff \std(I,J) \in \Or$, and thus a pair $(I,J) \in \Or$ is decomposable if and only if its standardisation is.

\begin{lemma} 
\label{lem:operadic-ordering}
The $\LA$ and $\SU$ orderings are operadic, and their indecomposables are the pair $(\{1\},\{2\})$ and respectively the pairs 
\begin{align}
...
For the direct implication, it suffices to take~$I'=I,J'=J$ and observe that standardising a pair preserves its minimal element.
The reverse implication also holds, as in this case we have $\min(I)=\min\{\min(I'),\min(I\setminus I')\}$.
...
Let $(I,J)$ be in $\LA$ and suppose that $\std(I,J) \neq (I_k,J_k)$.
