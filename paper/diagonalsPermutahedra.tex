...
\begin{remark}
    There is a second, distinct isomorphism given by $t(r\times r)$ \Guillaume{TBC} \Kurt{See shift iso example. Might be slightly better to use this instead}
\end{remark}
...
We first compare the original and $1$-shift $\SU$ diagonals, then $1$-shift and $m$-shift. 

\Guillaume{On doit pouvoir compresser ca pour faire directement original -- $m$-shift, on parlera des inversions après}
...
\begin{proof}
    By definition of the admissible $m$-path shifts, \cref{eq:shift-subset-dominates} holds and we can run the proof of \cref{prop:SU-preserves-max} \emph{mutatis mutandis}; moreover, there is no harm in replacing the tuple $k,k+1$ by the tuple $k,k+m$ in everything that follows \cref{eq:shift-subset-dominates}.
\end{proof}
...
The original and $1$-shift $\SU$ diagonals coincide.
\end{proposition}
...
\begin{lemma} 
\label{lem:SUD closed under shifts}
...
\begin{proposition} \label{prop:iso-shift-IJ-diagonals}
...
Combinining \cref{prop:iso-original-shift-diagonals} and \cref{prop:iso-shift-IJ-diagonals}, we obtain that
...
\end{tikzpicture}
\end{center}
We now illustrate all possible $\LAD$ shifts of the left $\SCP$ and all possible $\SUD$ shifts of the right $\SCP$. We first display how the $\LAD$ left shifts act on $\sigma$, and the $\SUD$ left shifts act on $\tau'$. We indicate shifts by their corresponding critical element, drawing them to avoid crossings, and so they align with their isomorphic shift under $\theta$. \Kurt{Update to new theta}
\begin{center}
{\small
\begin{tikzcd}
...
\\\\
We note, that the alternate isomorphism identified in Remark ... has the benefit of mapping left shifts to left shifts and right shifts to right shifts. However, this comes at the cost of switching the sides they act on. For instance, the same $\LAD$ $SCP$ is sent to the following $SCP$.
\begin{center}
\begin{tikzpicture}[scale=.7]  
\node (1) at (-1.5, -1) {$5$};
\node (2) at (-1.5, -2) {$17$};
...
\end{tikzpicture}
\end{center}
and for instance, the left $\LAD$ shift 
$5|17|4|236 \xrightarrow{L_3}5|17|34|26 $ is sent to the left $\SUD$ shift
$3|17|4|256 \xrightarrow{L_5} 3|17|45|26$. \Kurt{Hmm maybe it nicer to stick with alternate iso}
\end{example}
