...
We have seen in \cref{subsec:multiBraidArrangement} that facets of any diagonal of the permutahedron are in bijection with $(2,n)$-trees, that is pair of partitions whose intersection graph is a tree.
...
\end{tikzpicture}
\end{center}
Note, as we have chosen minimal illustrate examples, each inverse $m$-shift is an inverse $1$-shift, and after each shift we obtain an $SCP$. 
...
\begin{definition}
Let $(\sigma,\tau) \in \SUD$. For $\rho \in [n]$, let $r_\rho$ (or $l_\rho$) be either $0$, or the maximal value of $m$ such that $R_\rho(\sigma,\tau)$ (resp. $L_\rho(\sigma,\tau)$) is a well-defined $m$-shift.
\end{definition}
...
\begin{lemma} \label{lem:Formula for maximal m-shifts}
Let $(\sigma,\tau)$ be an SCP, then for $\rho \in \sigma_i$,
\begin{align*}
...
As $\sigma,\tau$ is an SCP, $\max \sigma_{k,k+1} = \max \sigma_{k+1}$. 
...
\begin{lemma}
If $(\sigma,\tau)$ is an SCP generated by a vertex $v$ then  $\sigma^* = \tau_*= v$.
\end{lemma}
\begin{proof}
...
Then,
\begin{align*}
    (R^{r_n}_{n}...R^{r_{1}}_{1}\sigma)_*= \overrightarrow{s_n} \overrightarrow{s_{n-1}}...\overrightarrow{s_1} \sigma^*
\end{align*}
where the shift operator $\overrightarrow{s_i}$ shifts the element $i$ past all elements to the right which are smaller than $i$. 
Similarly,
\begin{align*}
    (L^{l_n}_{n}...L^{l_{1}}_{1}\tau)^* =\overleftarrow{s_n} \overleftarrow{s_{n-1}}...\overleftarrow{s_1} \tau_*
\end{align*}
where the shift operator $\overleftarrow{s_i}$ shifts $i$ past all elements to the left which are smaller than $i$. 
\end{lemma}
\begin{proof}
...
The shift $R_\rho^{r_\rho}$ acting on $R^{r_{\rho-1}}_{\rho-1}...R^{r_{1}}_{1}\sigma$ is equivalent to the shift operator  $\overrightarrow{s_\rho}$ acting on $\overrightarrow{s_{\rho-1}}...\overrightarrow{s_1} \sigma^*$.
...
\end{align*}
Can be obtained by forgetting the order of the elements of the block and applying $R_\rho^{r_\rho}$, or by applying $\overrightarrow{s_\rho}$.
We know that $\overrightarrow{s_\rho}$ places $\rho$ at the same position, as by definition $\sigma_{i+m}$ is either the last block, or the maximal (and hence first) element of the next block is larger than $\rho$.
Note, that whenever we apply the shift $\overrightarrow{s_\rho}$, the element $\rho$ cannot be inhibited by elements of its own block. 
This follows, as we firstly ordered the blocks of $\sigma$ in decreasing order through $\sigma^*$, and secondly we apply the shifts $\overrightarrow{s_i}$ in increasing order of elements, so prior shifts will not inhibit later shifts.
Also observe that after applying the shift $\overrightarrow{s_i}$, that the $i$th element will be the last element of its block. Consequently, after applying all shift operators, each block will be in increasing order, hence $(R^{r_n}_{n}...R^{r_{1}}_{1}\sigma)_*= \overrightarrow{s_n} \overrightarrow{s_{n-1}}...\overrightarrow{s_1} \sigma^*$.
...
it is clear that the equality holds if $r\mathbf{M},r\mathbf{N}$ are admissible sequence of $\SU$ shifts.
...
\end{center}
Given a $l\times k$ matrix $C$ with all non-zero entries drawn without replacement from $[n]$, let $\sigma_i(C)$ denote the non-zero entries of the $i$th column, and $\tau_j(C)$ the non-zero entries of the $(l-j+1)$th row. 
...
\begin{definition}
A non-empty set of elements of a matrix $M_i \subset \sigma_i(C)$ (resp. $M_{i}\subset \tau_{i}(C)$) is \emph{$1$-block movable} if
\begin{enumerate}
    \item $\min \sigma_i(C) \notin M_i$ (resp. $\min \tau_{i}(C) \notin M_i$), and
    \item $\min M_i> \max \sigma_{i+1}(C)$ (resp. $\min M_{i}> \max \tau_{i-1}(C)$).
\end{enumerate}
The right matrix subset shift operator $R_{M_i}$ shifts the elements of $M_i$ right one column (i.e. if $C_{k,l} \in M_i$ then $(R_{M_i}C)_{k,l+1} = C_{k,l}$), replacing only elements of value $0$, and leaving $0$ elements in their wake.
...
\begin{example}
Here are some valid subset shifts of the matrix encoding of $6524713$. Every composite of arrows is a valid sequence of shifts.
\begin{center}
\begin{tikzcd}
\begin{bmatrix}
  &   & 1 & 3 \\
...
\end{tikzcd}
\end{center}
Note, that $R_5,R_6$ are also valid right shifts of the left matrices.
\Kurt{Todo.}
...
\Kurt{Need a rethink on notation, have some clashes, but I think the proof works.
Should also rethink organisation, maybe some of the lattice results should be in here.
}

We present and characterise all definitions in full, before giving a protracted example (\cref{ex:subdivision cubes}).
...
We need an explicit way to calculate $k$-subdivision cubes, and it turns out it is easier to calculate the minimal/maximal vertices, as follows.

\begin{lemma} \label{lem:subdiv cubes are intervals}
A $k$-subdivision cube corresponds to an interval $[\overrightarrow{v},v]$ of $P_n$, where $v$ is the unique maximal vertex, and 
\begin{align*}
    \overrightarrow{v}:= \overrightarrow{s_1}^{k_1}... \overrightarrow{s_{n-1}}^{k_{n-1}}\overrightarrow{s_n}^{k_n}v
\end{align*}
where the shift operator $\overrightarrow{s_i}^{k_i}$ shifts element $i$ past $k_i$ elements to the right which are smaller than $i$. 
Dually, a $k$-subdivision cube corresponds to an interval $[{v},\overleftarrow{v}]$ of $P_n$, where $v$ is the unique minimal vertex, and
\begin{align*}
    \overleftarrow{v} :=\overleftarrow{s_n}^{k_n}\overleftarrow{s_{n-1}}^{k_{n-1}}...\overleftarrow{s_1}^{k_1}v
\end{align*}
where the shift operator $\overleftarrow{s_i}^{k_i}$ shifts element $i$ past $k_i$ elements to the right which are smaller than $i$.
\end{lemma}
\begin{proof}
We first show inductively that every interval of the form $[\overrightarrow{v},v]$ is a $k$-subdivision cube.
...
$\overrightarrow{v}$ uses precisely $k$ non-zero shifts ($k_i$), then $[\overrightarrow{v},v]$ is a $k$-subdivision cube.
Let $\overrightarrow{v}$ uses precisely $k+1$ non-zero shifts, and let $i$ be the smallest element shifted.
By the inductive assumption, we know that 
$[\overrightarrow{s_{i-1}}^{k_{i-1}}... \overrightarrow{s_{n-1}}^{k_{n-1}}\overrightarrow{s_n}^{k_n}v, v]$ is a subdivision cube.
Then from \cref{eq:In inductive def} it follows that $[\overrightarrow{v},v]$
must also be a subdivision cube.
\\\\
We now show that every subdivision cube is an interval of the form $[\overrightarrow{v},v]$.
By \cref{eq:In inductive def}, we know that every edge of the $I_{n-1}$ is associated to a shift of a particular element $i$.
Let $I_v$ be the set of elements associated to the edges touching the maximal vertex $v$.
We now construct a formula for the minimal vertex in terms of shift operators, i.e. $\overrightarrow{v}$.
For all $i \not \in I_v$ let $k_i=0$.
Starting at the maximal vertex $v$, let $i$ be the largest element of $I_v$.
Follow the edge associated to $i$ to the next corner of the $k$ subdivision cube, counting the number of edges of $I_{n-1}$ used to determine $k_i$, and then remove $i$ from $I_v$.
Repeat this process until $I_v = \emptyset$, at which point we have reached the minimal vertex.
\end{proof}
...
For a given vertex $v \in \mathcal{V}_e$, we define $e_v$ to be set of all pairs $(C_\sigma,C_\tau)$ of $k_1,k_2$ subdivision cubes such that
\begin{enumerate}
...
The pairs $e_v$ satisfy the following properties,
\begin{enumerate}
    \item The SCP $(\sigma,\tau) \in e_v$.
    \item All pairs $(C_\sigma,C_\tau) \in e_v$ have the same dimensions and these are determined by the number of blocks in the SCP. In particular, $k_1=|\sigma|$ and $k_2 = |\tau|$. 
    \item For all $(C_\sigma,C_\tau) \in e_v$, $\overline{C_\sigma} = \sigma$, and $\underline{C_\tau} = \tau$.
\end{enumerate}
\end{proposition}
\begin{proof}
The proofs in turn are as follows.
\begin{enumerate}
    \item The fact that the SCP is in $e_v$ follows directly from the definition of each.
    ...
    Suppose that exists $(C_1,C_2) \in e_v$ which are $k'_1, k'_2$ subdivision cubes where $k'_1\neq k_1$ (and $k'_2\neq k_2$).
...
\begin{definition}
Let ${e_v}_*$ denote the unique maximal pair of subdivision cubes in $e_v$ under inclusion.
\end{definition}
We provide an alternate characterisation, and prove the uniqueness of this definition in the next section (\cref{lem:unique interval desc of maximal subdiv pair}), but we first clarify all the definitions of this section through example.
...
\begin{align*}
    \{1234\}, \{1|234\}, \{1|234, 14|23\}, \{4|13|2\}, \{1|3|24,1|43|2,14|3|2\}.
\end{align*}
The follow three sets of faces are not subdivision cubes of $e$,
\begin{align*}
    \{134|2, 14|23\}, \{1|234, 134|2, 14|23\}, \{1|2|34,1|23|4\}
\end{align*}
...
\end{align*}
This defines $e_v$, and thus ${e_v}_*=\{ 1|234, 13|24, 14|23, 134|2\} \times \{4|3|12\}$.
Observe that $\sigma = \blue{\mathbf{134|2}}$ admits $3$ different sequences of admissible rights shifts, their images are $\red{\mathbf{13|24}}$ , $\red{\mathbf{14|23}}$, $\red{\mathbf{1|234}}$,
...
Firstly, all shifts are present in ${e_v}_*$.
Secondly, the explicit relationship between shifts and the other $k$-subdivision cubes is subtle, as there are four shifts of $\sigma$ (including the trivial shift), but only three $k_1$ subdivision cubes with maximal vertex $v$.
...
Recall that in \cref{lem: minimal vertex formulae} we found that the minimal vertices of the maximal shifts of a SCP $(\sigma,\tau)$ satisfied the following formulae.
\begin{align*}
    (R^{r_n}_{n}...R^{r_{1}}_{1}\sigma)_*&= \overrightarrow{s_n} \overrightarrow{s_{n-1}}...\overrightarrow{s_1} \sigma^*, \quad
    (L^{l_n}_{n}...L^{l_{1}}_{1}\tau)^* =\overleftarrow{s_n} \overleftarrow{s_{n-1}}...\overleftarrow{s_1} \tau_*
\end{align*}
where the shift operator $\overrightarrow{s_i}$, $\overleftarrow{s_i}$ shifts the element $i$ past all elements to the right/left which are smaller than $i$. 
From this result we obtain the following.
\begin{lemma} \label{lem:unique interval desc of maximal subdiv pair}
The maximal pair of subdivision cubes ${e_v}_*$ is unique, and has the form
\begin{align*}
    {e_v}_*=[\overrightarrow{s_n} \overrightarrow{s_{n-1}}...\overrightarrow{s_1}v, v] \times [v, \overleftarrow{s_n} \overleftarrow{s_{n-1}}...\overleftarrow{s_1}v]
\end{align*}
\end{lemma}
\begin{proof}
...
\begin{align*}
    \overrightarrow{v}:= \overrightarrow{s_1}^{k_1}... \overrightarrow{s_{n-1}}^{k_{n-1}}\overrightarrow{s_n}^{k_n}v, \quad \overleftarrow{v} :=\overleftarrow{s_n}^{k_n}\overleftarrow{s_{n-1}}^{k_{n-1}}...\overleftarrow{s_1}^{k_1}v
\end{align*}
As such, it is clear that $[\overrightarrow{s_n} \overrightarrow{s_{n-1}}...\overrightarrow{s_1}v, v]$ and $[v,\overleftarrow{s_n} \overleftarrow{s_{n-1}}...\overleftarrow{s_1}v]$ are not only $k_1,k_2$-subdivision cubes,
but the particular subdivision cubes for which each shift $k_i$ is maximal.
This tells us that all other subdivision cubes are enveloped by these maximal cubes.
To see this one can observe from the inductive description of $I_n$ (\cref{eq:In inductive def}) that the path from the minimal vertex of $\overrightarrow{s_n} \overrightarrow{s_{n-1}}...\overrightarrow{s_1}v$ to any other $\overrightarrow{s_1}^{k_1}... \overrightarrow{s_{n-1}}^{k_{n-1}}\overrightarrow{s_n}^{k_n}v$ will remain within the $k_1$-cube $[\overrightarrow{s_n} \overrightarrow{s_{n-1}}...\overrightarrow{s_1}v,v]$, and similarly for the $k_2$.
\end{proof}
...
\begin{align*}
    e_v^* = \bigcup_{\mathbf{M},\mathbf{N}} R_\mathbf{M}(\sigma) \times L_{\mathbf{N}}(\tau)
\end{align*}
...
$e_v^* = [(R^{r_n}_{n}...R^{r_{1}}_{1}\sigma)_*,v] \times [v,(L^{l_n}_{n}...L^{l_{1}}_{1}\tau)^*]$, or in terms of $k_1,k_2$ faces,
$e_v^* = [R^{r_n}_{n}...R^{r_{1}}_{1}\sigma,\sigma] \times [\tau, L^{l_n}_{n}...L^{l_{1}}_{1}\tau]$.
...
The $k_2$ faces of ${e_v}_*$ are similarly characterised, and the result follows.
\end{proof}
...
\begin{align*}
    \SUD([n]) = \bigcup_{v \in P_n} e_v^*
\end{align*}
\end{proposition}

\begin{example}
For the SCP $ 13|247|5|6 \times 3|17|4|156$ of \cref{ex:shift translation by theta}. We directly compute $R(e_v^*)$ to be the cube..., and $L(e_v^*)$ to be the cube...
\end{example}

\Kurt{What do the IJ conditions correspond to on the cube?}


To construct the $LA$ subdivision of the cube we first make a useful observation.
\begin{lemma}
...
\end{tikzcd}
\end{center}
Where the vertical arrows are 'read the right-side blocks to identify the permutation'.
\end{lemma}
...
\subsubsection{Old/Unneeded Cubical}

\begin{center}
{\footnotesize
\begin{tikzcd}[sep=0.1cm, scale=0.3]
& 4|2|1|3 \arrow[rr,dash] \arrow[dd,dash,dotted] \arrow[dl,dash] & & 4|2|3|1 \arrow [dd, dash,dotted] \arrow[rr,dash] & & 4|3|2|1 \arrow[dd,dash] \arrow[dl,dash]\\
4|1|2|3 \arrow [rr,dash] \arrow [dd,dash] & & \mathbf{\blue{4|1|3|2}} \arrow[rr,dash,crossing over, "\mathbf{\blue{4|13|2}}" description] \arrow[dd,dash] & & 4|3|1|2 \arrow[dd,dash]  & \\
& \blue{\mathbf{14|23}} \arrow [rr,dash,dotted] \arrow[dd,dash,dotted]  & & 2|4|3|1  \arrow[dd,dash,dotted] & & 3|4|2|1 \arrow [dl,dash] \arrow[dd,dash]\\
1|4|2|3 \arrow [rr,dash] \arrow [dd,dash]& & 1|4|3|2  \arrow[dd,dash] & & 3|4|1|2 \arrow[dd,dash]\\
& 2|1|4|3 \arrow[dd,dash,dotted] \arrow[dl,dash,dotted] & & 2|3|4|1 \arrow [dd, dash,dotted] \arrow [rr, dash,dotted] & & 3|2|4|1 \arrow[dd,dash] \\
1|2|4|3 \arrow [dd,dash] \arrow[rr, phantom, "\red{\mathbf{1|234}}" description,crossing over]& & 1|3|4|2 \arrow[rr,dash] \arrow[dd,dash] & & 3|1|4|2 \arrow[dd,dash] & \\
& 2|1|3|4 \arrow [rr,dash,dotted] \arrow [dl,dash,dotted] & & 2|3|1|4 \arrow [rr,dash,dotted] & & 3|2|1|4 \arrow [dl,dash] \\
1|2|3|4 \arrow [rr,dash] & & 1|3|2|4 \arrow [rr,dash] & & 3|1|2|4 \\
\end{tikzcd}
}
\quad
...
\begin{tikzcd}[sep=0.1cm, scale=0.3]
& 4|2|1|3 \arrow[rr,dash] \arrow[dd,dash,dotted] \arrow[dl,dash] & & 4|2|3|1 \arrow [dd, dash,dotted] \arrow[rr,dash] & & 4|3|2|1 \arrow[dd,dash] \arrow[dl,dash]\\
4|1|2|3 \arrow [rr,dash] \arrow [dd,dash] & & 4|1|3|2 \arrow[rr,dash] \arrow[dd,dash] & & 4|3|1|2 \arrow[dd,dash,crossing over, "\mathbf{\red{43|1|2}}" description]  & \\
& 2|4|1|3 \arrow [rr,dash,dotted] \arrow[dd,dash,dotted]  & & 2|4|3|1  \arrow[dd,dash,dotted] & & 3|4|2|1 \arrow [dl,dash] \arrow[dd,dash]\\
1|4|2|3 \arrow [rr,dash] \arrow [dd,dash]& & 1|4|3|2  \arrow[dd,dash] & & 3|4|1|2 \arrow[dd,dash,crossing over, "\mathbf{\blue{3|14|2}}" description]\\
& 2|1|4|3 \arrow[dd,dash,dotted] \arrow[dl,dash,dotted] & & 2|3|4|1 \arrow [dd, dash,dotted] \arrow [rr, dash,dotted] & & 3|2|4|1 \arrow[dd,dash] \\
1|2|4|3 \arrow [dd,dash] & & 1|3|4|2 \arrow[rr,dash] \arrow[dd,dash] & & \mathbf{\blue{3|1|4|2}} \arrow[dd,dash] & \\
& 2|1|3|4 \arrow [rr,dash,dotted] \arrow [dl,dash,dotted] & & \mathbf{\blue{13|24}} \arrow [rr,dash,dotted] & & 3|2|1|4 \arrow [dl,dash] \\
1|2|3|4 \arrow [rr,dash] & & 1|3|2|4 \arrow [rr,dash] & & 3|1|2|4 \\
\end{tikzcd}
}
\end{center}

\begin{example}[Shifting a subdivision cube does not produce a cube]
\Kurt{...}
Let $v = 4|3|1|2$, then the corresponding SCP is $134|2 \times 4|3|12$. As such, the ordered partition $134|2$ admits three right shifts, these being, $13|24, 14|23$ and $1|234$.
These correspond to the four front most squares of the cubical subdivision below, with $134|2$ being the top-right square.
\begin{center}
 {\small
\begin{tikzcd}[sep=0.15cm,scale=0.8]
& 4|2|1|3 \arrow[rr,dash] \arrow[dd,dash,dotted] \arrow[dl,dash] & & 4|2|3|1 \arrow [dd, dash,dotted] \arrow[rr,dash] & & 4|3|2|1 \arrow[dd,dash] \arrow[dl,dash]\\
...
\end{center}
Under the definition of a $k$-subdivision cube with a maximal vertex $v$, there are three $2$-subdivision cubes (squares) with maximal vertex $4|3|1|2$, the top-right square ($134|2$), the right rectangle ($134|2$, $13|24$), and all four faces.
However $\{14|23, 134|2\}$ is not a $2$-subdivision cube, as combining the top left and right squares is not a square.
\\\\
Thoughts,
\begin{itemize}
    \item This messes with a lot of recursive descriptions of ${e_v}_*$ (including the proof outline described by Samson in his email)
    \item In fact it tells you that thinking about $k$-division cubes with maximal vertices is in general a flawed approach.
    \item i.e. the only subdivision cube that matters is the maximal one, ${e_v}_*$ (in this case the square with four vertices)
    \item i.e. we can only compute lower $k_1$-face/vertex, use this to construct ${e_v}_*$, and argue that all other faces are in it via the lattice description.
\end{itemize}
So,
\begin{itemize}
    \item If $k$-subdivision cubes is not the right approach, then we are free to define $e_v^*$ in another fashion.
    \item The cubical subdivision still gives you something...
    \begin{itemize}
        \item The LHS of ${e_v}^*$ is not the set of $k_1$ faces whose maximal vertex is smaller than $v$
        \item i.e. the cubical subdiv contains useful stopping info..
    \end{itemize}
\end{itemize}
\end{example}  
...
\begin{lemma}
Let $C$ be a $k$-subdivision cube of $I_{n}$,
then the projection of the $n$th dimension of $C$ is either a point, or it is a non-trivial interval of $[0,1]$.
Furthermore,
\begin{itemize}
    \item If it is a point, then the projection of dimensions $1,...,n-1$ is a $k$-subdivision cube of $I_{n-1}$.
    \item If it is a non-trivial interval, then the projection of dimensions $1,...,n-1$ is a $(k-1)$-subdivision cube of $I_{n-1}$.
\end{itemize}

\end{lemma}
\begin{proof}
Immediate from being a $k$-subcube.
\end{proof}


\begin{lemma}
Let $C_\tau$ be a subdivision cube with minimal $k$-face corresponding to the right ordered partition of a $SCP$. Then,
$k = |\sigma|$, the number of blocks in the ordered partition.
\item The maximal $k$-face is of the form 
\begin{align*}
    \overline{C_\tau} =  \overleftarrow{s_n}^{k_n} \overleftarrow{s_{n-1}}^{k_{n-1}}...\overleftarrow{s_1}^{k_1} \sigma
\end{align*}
where the shift operator $\overleftarrow{s_i}^{k_i}$ shifts element $i$,  $k_i$ blocks to the left, if the maximum of all these $k_i$ blocks is smaller than $i$. 
Dually, let $C_\sigma$ be a subdivision cube with maximal $k$-face corresponding to left ordered partition of a $SCP$. Then,
\begin{enumerate}
    \item $k = |\sigma|$, the number of blocks in the ordered partition.
    \item The minimal $k$-face is of the form 
    \begin{align*}
        \underline{C_\sigma} =  \overrightarrow{s_1}^{k_1}... \overrightarrow{s_{n-1}}^{k_{n-1}}\overrightarrow{s_n}^{k_n} \sigma
    \end{align*}
    where the shift operator $\overrightarrow{s_i}^{k_i}$ shifts element $i$,  $k_i$ blocks to the right, if the maximum of all these $k_i$ blocks is smaller than $i$. 
\end{enumerate}
\end{lemma}
