...
\]
where~$S(n,k)$ denotes the Stirling number of second kind \OEIS{?}, \ie the number of partitions of~$n$ into~$k$ parts.
For instance
\begin{align*}
...
\begin{proposition}
\label{prop:multiBraidArrangement}
...
We will now provide more explicit formulas for the number of vertices and regions of the $(\ell,n)$-braid arrangement~$\multiBraidArrangement$.
To this end, we first introduce another combinatorial model for $(\ell,n)$-partition forests and $(\ell,n)$-partition trees which is more adapted to their enumeration.
...
\end{itemize}
A \defn{$(\ell,n)$-rainbow forest} (resp.~\defn{tree}) is a $\ell$-rainbow colored forest (resp.~tree) with $n$ nodes.
The \defn{weight} of a rainbow forest~$F$ is the product
\[
...
\[
F_{\ell,m} = \frac{1}{(\ell-1)m+1} \binom{\ell m}{m} \qquad \text{(\OEIS{A062993})}.
\]
\end{lemma}
...
%\begin{remark}
%The corresponding generating function~$F_\ell(z) \eqdef \sum_{m \ge 1} F_{\ell,m} \, z^m$ is given by
%\[
%F_\ell(z) = \frac{t}{\big( 1-G_\ell(z) \big)^\ell}
%\qquad\text{where}\qquad
%G_\ell(z) = \frac{z}{\big( 1-G_\ell(z) \big)^{\ell-1}}.
%\]
%\end{remark}
...
\begin{definition}
A $(\ell,n)$-rainbow forest is \defn{well-labeled} if its nodes are bijectively labeled by~$[n]$ so that
\begin{enumerate}[(i)]
...
\begin{proposition}
There is a bijection from $(\ell,n)$-partition forests to well-labeled $(\ell,n)$-rainbow forests, which sends the M\"obius function~$|\mu_{\forestPoset}(\hat 0, -)|$ to the weight~$\omega(-)$.
\end{proposition}

\begin{proof}
From a well-labeled $(\ell,n)$-rainbow forest~$F$, we construct a $(\ell,n)$-partition forest~$\b{F} \eqdef (F_1, \dots, F_\ell)$ whose $i$th partition~$F_i$ has a part~$\{N\} \cup C_i(N)$ for each node~$N$ of~$F$.
...
This defines a well-labeled $(\ell,n)$-rainbow forest~$F$.

Finally, $|\mu_{\forestPoset}(\hat 0, \b{F})| = \prod\limits_{i \in [\ell]} \prod\limits_{p \in F_i} (\#p-1)$ is clearly sent to~$\omega(F) = \prod\limits_{i \in [\ell]} \prod\limits_{N \in F} \#C_i(N)!$.
\end{proof}
...
For any~$k_1, \dots, k_\ell$ such that~$0 \le k_i \le n-1$ for all~$i \in [\ell]$ and~$\sum_{i=1}^\ell k_i = n-1$, the number of vertices~$v$ of the $(\ell,n)$-braid arrangement~$\multiBraidArrangement$ such that the smallest flat of the $i$th copy of~$\braidArrangement$ containing~$v$ has dimension~$n-k_i-1$ is given by
\[
...
\begin{theorem}
\label{thm:facetsMultiBraidArrangement}
...
\]
where~$\displaystyle F_{\ell,m} \eqdef \frac{1}{(\ell-1)m+1} \binom{\ell m}{m}$ denotes the \defn{$(\ell,m)$-Fuss-Catalan number}.
\end{theorem}
...
\begin{proof}
As $\fPol[{\multiBraidArrangement[n][2]}] = \mobPol[{\multiBraidArrangement[n][2]}][-x][-1]$ and $\bPol[{\multiBraidArrangement[n][2]}] = \mobPol[{\multiBraidArrangement[n][2]}][-x][1]$ by \cref{thm:Zaslavsky}, we obtain from \cref{thm:facets2BraidArrangement} that
\begin{align*}
