...
\end{equation}
% k = nb of part of \pi'
% l_i=nb de parts de \pi fusionné en la ième part de \pi'
...
\begin{figure}
	...
	\includegraphics{forest2}\hspace{2cm}
	\includegraphics{forest1}
	\caption{A $(3,6)$-forest with its intersection hypergraph (left) and a $(3,6)$-tree with its intersection hypertree (right). \BDO{@Vincent : j'aimerais mettre mes figures avec les tiennes, pourrais-tu juste scinder "forests.pdf" en deux stp?}}
	\label{fig:forests}
\end{figure}
\end{definition}
...
\includegraphics{figures/IntPosB23.pdf}}
...
For any~$\b{k} \eqdef (k_1, \dots, k_\ell)$ such that~$0 \le k_i \le n-1$ for all~$i \in [\ell]$ and~$\sum_{i=1}^\ell k_i = n-1$, the number of vertices~$v$ of the $(\ell,n)$-braid arrangement~$\lBraidArrangement$ such that the smallest flat of the $i$th copy of~$\braidArrangement$ containing~$v$ has dimension~$d_i=n-k_i-1$ is given by
\[
f_{\b{d}}(\lBraidArrangement) = n^{\ell-1}  \binom{n-1}{k_1, ..., k_l} \prod_{i=1}^\ell (n-k_i)^{k_i-1}.
\]
...
\BDO{Isn't there rather a shift in $k_i$? Like a partition with $k$ parts has dimension $k-1$. What about the following formula ?}
\begin{equation}
f_{\b{k}}(\lBraidArrangement) = (n+1)^{\ell-1}  \binom{n}{k_1+1, ..., k_l+1} \prod_{i=1}^\ell (n-k_i)^{k_i}.
\end{equation}
For any~$\b{k} \eqdef (k_1, \dots, k_\ell)$ such that~$0 \le k_i \le n-1$ for all~$i \in [\ell]$ and~$\sum_{i=1}^\ell k_i = n - \ell -1$, the number of vertices~$v$ of the $(\ell,n)$-braid arrangement~$\lBraidArrangement$ such that the smallest flat of the $i$th copy of~$\braidArrangement$ containing~$v$ has dimension~$k_i$ is given by
\begin{equation}
f_{\b{k}}(\lBraidArrangement) = n^{\ell-1}  \binom{n-1}{k_1+1, ..., k_l+1} \prod_{i=1}^\ell (n-1-k_i)^{k_i}.
\end{equation}
\end{conjecture}
