...
\begin{figure}
	\centerline{\includegraphics[scale=.9]{figures/intersectionPosetBraidArrangement31Full}}
...
\vincent{Add partitions.}

\begin{figure}
	\centerline{\includegraphics[scale=.9]{figures/intersectionPosetBraidArrangement32Full}}
	\caption{The $(2,3)$-braid arrangement $\lBraidArrangement[3][2]$ (left), its flat poset~$\flatPoset[{\lBraidArrangement[3][2]}]$ (top right), and its interpretation in terms of $(2,3)$-colored forests (bottom right).}
...
\begin{figure}
	...
	\caption{A $(3,6)$-forest with its intersection hypergraph and the corresponding $(3,6)$-colored forest (top) and a $(3,6)$-tree with its intersection hypertree and the corresponding $(3,6)$-colored tree (bottom).}
	\label{fig:forests}
\end{figure}
\end{definition}
...
\begin{proposition}
The flat poset of the $(\ell,n)$-braid arrangement~$\lBraidArrangement$ is isomorphic to the $(\ell,n)$-forest poset.
\end{proposition}
...
\subsection{Colored trees and forests}
\label{subsec:coloredTreesForests}

In order to obtain more explicit formulas for the number of vertices and regions of the $(\ell,n)$-braid arrangement~$\lBraidArrangement$ (see \cref{subsec:verticesFacetslnBraidArrangement}), we now introduce another combinatorial model for $(\ell,n)$-forests which is more adapted to their enumeration.

\begin{definition}
A \defn{$\ell$-colored tree} (resp.~\defn{almost $\ell$-colored tree}) is a rooted plane tree whose nodes (resp.~non-root nodes) are properly colored by~$[\ell]$, \ie with no monochromatic edge.
\end{definition}

\begin{lemma}
The numbers~$s(\ell,m)$ of $\ell$-colored trees and $t(\ell,m)$ of almost $\ell$-colored trees with $m$ nodes are
\[
r(\ell,m) = \frac{1}{m+1} \binom{\ell (m+1)-2}{m}
\qquad\text{and}\qquad
s(\ell,m) = \frac{1}{(\ell-1)m+1} \binom{\ell m}{m}.
%\qquad\text{and}\qquad
%t(\ell,m) = \frac{(m-1)!}{(\ell-1)m+1} \binom{\ell m}{m}.
\]
\end{lemma}

\begin{proof}
\vincent{Not sure what the best way is.} \newline
The corresponding generating functions~$S_\ell(z) \eqdef \sum_{m \ge 1} s(\ell,m) \, z^m$ and~$T_\ell(z) \eqdef \sum_{m \ge 1} t(\ell,m) \, z^m$ satisfy
\[
S_\ell(z) = \frac{z}{\big( 1-S_\ell(z) \big)^{\ell-1}}
\qquad\text{and}\qquad
T_\ell(z) = \frac{t}{\big( 1-S_\ell(z) \big)^\ell}.
\]
\end{proof}

\begin{example}
\vincent{TODO: numbers}
\end{example}

\begin{definition}
An \defn{almost $\ell$-colored $n$-labeled forest} is a set of almost $\ell$-colored trees with~$n$ nodes in total bijectively labeled by~$[n]$ such that in each tree, the root receives the maximal label.
\vincent{fix the pictures}
See \cref{fig:forests}.
\end{definition}

\begin{proposition}
For any~$\ell,n \in \N$, the $(\ell,n)$-forests (resp.~trees) are in bijection with the almost $\ell$-colored $n$-labeled forests (resp.~trees).
\end{proposition}
...
\begin{proposition}
The exponential generating function~$F_\ell(y,z)$ of the numbers~$f_\ell(k,n)$ of almost $\ell$-colored $n$-labeled forests consisting in~$k$ trees is given by
\[
F_\ell(y,z) \eqdef \sum_{k,n \ge 1} \frac{f_\ell(k,n)}{n!} \, y^k \, z^n = \exp \Bigg( \sum_{m \ge 1} \frac{y \, z^m}{m \big( (\ell-1)m+1 \big)} \binom{\ell m}{m} \Bigg).
\]
\end{proposition}
...
\subsection{Enumeration of vertices and regions of $\lBraidArrangement$}
\label{subsec:verticesFacetslnBraidArrangement}
...
\begin{proof}
\vincent{TODO: work this out again with the def change.}
...
\begin{theorem}
\label{thm:facetslnBraidArrangement}
The coefficient of~$x^{n-1}$ in the M\"obius polynomial~$\mobPol[{\lBraidArrangement}]$ of the $(\ell,n)$-braid arrangement~$\lBraidArrangement$ is given by
\[
[x^{n-1}] \, \mobPol[{\lBraidArrangement}] = \frac{(-1)^n n!}{y} \, [z^n] \, \exp \Bigg( - \sum_{m \ge 1} \frac{y \, z^m}{m \big( (\ell-1)m+1 \big)} \binom{\ell m}{m} \Bigg).
\]
\end{theorem}
...
\begin{corollary}
The numbers of regions and of bounded regions of the $(\ell,n)$-braid arrangement~$\lBraidArrangement$ are given by
\begin{align*}
f_{n-1}(\lBraidArrangement) 
& = n! \, [z^n] \exp \Bigg( \sum_{m \ge 1} \frac{z^m}{m \big( (\ell-1)m+1 \big)} \binom{\ell m}{m} \Bigg) \\
\text{and}\qquad
b_{n-1}(\lBraidArrangement)
& = - n! \, [z^n] \exp \Bigg( - \sum_{m \ge 1} \frac{z^m}{m \big( (\ell-1)m+1 \big)} \binom{\ell m}{m} \Bigg) 
\end{align*}
\end{corollary}
...
\vincent{We can now delete a lot of things in this section.}
