...
We will now provide more explicit formulas for the number of vertices and regions of the $(\ell,n)$-braid arrangement~$\lBraidArrangement$.
For this, we first introduce another combinatorial model for $(\ell,n)$-forests and $(\ell,n)$-trees which is more adapted to their enumeration.
...
\begin{lemma}
For any~$\ell,n \in \N$, the $(\ell,n)$-forests (resp.~trees) are in bijection with the $(\ell,n)$-colored forests (resp.~trees).
\end{lemma}
...
\[
[x^{n-1}] \, \mobPol[{\lBraidArrangement}] = \frac{(-1)^n n!}{y} \, [z^n] \, \exp \Bigg( - \sum_{m \ge 1} \frac{y \, z^m}{m(m+1)} \binom{\ell (m+1)-2}{m} \Bigg).
\]
\end{theorem}
...
& = n! \, [z^n] \exp \Bigg( \sum_{m \ge 1} \frac{z^m}{m(m+1)} \binom{\ell (m+1)-2}{m} \Bigg) \\
\text{and}\qquad
