...
\centerline{
	...
	\caption{The face numbers (top) and the bounded face numbers (bottom) of the $(\ell,n)$-braid arrangements for~$\ell \in [4]$.}
...
\begin{definition}
The \defn{intersection hypergraph}~$I(F_1, \dots, F_\ell)$ of a $\ell$-tuple of partitions of~$[n]$ is...
A \defn{$(\ell,n)$-forest} is a $\ell$-tuple~$\b{F} \eqdef (F_1, \dots, F_\ell)$ of partitions of~$[n]$ whose intersection hypergraph is a forest.
...
\]
As the M\"obius function is multiplicative
\[
...
\[
\fPol[\lBraidArrangement] = ...
\qquad\text{and}\qquad
\bPol[\lBraidArrangement] = ...
\]
\end{corollary}
