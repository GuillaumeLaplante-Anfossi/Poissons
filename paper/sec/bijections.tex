...
Let $\gamma$ be any path between two blocks on the same side of $(R_{\mathbf{M}}(\sigma),L_{\mathbf{N}}(\tau))$, and $\gamma'$ the path between the same blocks, by indices, in $(R_{M_k}R_{\mathbf{M}}(\sigma),L_{\mathbf{N}}(\tau))$. 
We assume for now that the start and end of the path $\gamma$ are distinct from those of $\delta_{k,k+1}$.
There are four cases to consider. 
\begin{enumerate}
    \item $\gamma$ does not contain an element of $M_k$.
    It is thus unaffected by the shift, so $\gamma' = \gamma$.
    In particular, its maximal element stays the same. 
    \item $\gamma$ contains one element $m\in M_k$, i.e. it is of the form $\gamma = \alpha m \beta$.
    ...
    We must have $\alpha \cap \delta_{k,k+1}=\emptyset$ (resp. $\beta \cap \delta_{k,k+1}=\emptyset$) since otherwise there would be an oriented loop from $R_\mathbf{M}(\sigma)_{k}$ to itself.
    For the same reason, $\beta \cap \delta_{k,k+1}$ (resp. $\alpha \cap \delta_{k,k+1}$) must be connected and starting (resp. ending) at $R_\mathbf{M}(\sigma)_{k}$.
    Then there are two cases to consider:
    \begin{enumerate}
        \item $\beta$ (resp. $\alpha$) does not use any steps of $\delta_{k,k+1}$, in which case $\gamma' = \alpha m \delta_{k+1,k} \beta$ (resp. $\gamma'=\alpha\delta_{k,k+1}m\beta$).
        ...
        Since $\max \gamma \geq m$ and $m\geq \max \delta_{k,k+1}$, we deduce that $\max \gamma = \max \gamma'$.
        
        \item $\beta$ (resp. $\alpha$) uses steps of $\delta_{k,k+1}$, in which case $\gamma' = \alpha m (\delta_{k+1,k}\ssm \beta)(\beta \ssm \delta_{k+1,k})$ (resp. $\gamma' = (\alpha\ssm \delta_{k,k+1})(\delta_{k,k+1}\ssm\alpha) m \beta$).
        This follows, as we know that $\beta$ (resp. $\alpha$) must follow the path $\delta_{k,k+1}$ for some time before diverging ($\beta$ -resp. $\alpha$- could also be a subset of $\delta_{k,k+1}$, in which case it will never diverge).
