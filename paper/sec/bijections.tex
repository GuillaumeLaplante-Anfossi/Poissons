...
Let $U(n)  \eqdef  \set{\{I,J\}}{I,J \subset [n], |I|= |J|, I\cap J = \emptyset}$.
...
An \emph{ordering} $O$ of $U$ is a family of sets $\{O(n)\}_{n\geq 1}$ where each $O(n)$ has as elements ordered pairs $(I,J)$ or $(J,I)$, for each $\{I,J\} \in U(n)$.
...
A \emph{diagonal $\triangle:=\{\triangle_n\}$ of the permutahedra} is a family of diagonals $\triangle_n : P_n \to P_n \times P_n$, for each $n$-permutahedron $P_n$, $ n \geq 1$. 

\begin{definition}
    A diagonal $\triangle$ of the permutahedra is \emph{operadic} if for every $n \geq 1$ and every face $A_1 | \ldots | A_k$ of the permutahedron $P_n$ we have a poset isomorphism \[ \triangle(A_1 | \ldots | A_k) \cong \triangle(A_1) \times \ldots \times \triangle(A_k) \ . \]
\end{definition}

This property is also called comultiplicativity in the literature \cite{SaneblidzeUmble04}.
In the definition, we make use of the fact that every face of the permutahedron $P_n$ is isomorphic to a product of lower dimensional permutahedra, via the isomorphism $\Theta$ of Point (5) in \cite[Proposition 2.3]{LA21} which permutes the coordinates. 
...
There are four clear bijective maps between these four orders of $U(n)$, related by the following diagram
\begin{center}
\begin{tikzcd}
