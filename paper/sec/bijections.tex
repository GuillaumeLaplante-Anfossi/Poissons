...
This partial composition $\circ_i^\LA$ is an isomorphism (in the category $\PolySub$ \cite[Def. 4.13]{LA21}) between the product $P_{t'}\times P_{t''}$ and the facet $t' \circ_i t''$ of $P_t$.
...
\begin{definition}
    For any operahedron $P_t$, the map $\Psi_t : P_t \to P_t$ is defined on the face $t'\circ_i t''$ of~$P_t$ by the composition of the two isomorphisms \[ \Psi_{t'\circ_i t''} := \circ_i^\SU (\circ_i^\LA)^{-1} \ . \] 
\end{definition}

\Guillaume{Well-defined?}

\begin{proposition}
    The map $\Psi:=\{\Psi_t\}$ is an isomorphism of topological cellular colored operad in the category $\PolySub$.
\end{proposition}
...
\begin{proof}
    ...
    The composition $\Psi_t \ \circ_i^\LA = \Psi_{t' \circ_i t''} \ \circ_i^\LA$ is equal to $\circ_i^\SU$. 
\end{proof}
...
\begin{example}
    ...
    For instance, it is not the one you would expect... 
\end{example}
